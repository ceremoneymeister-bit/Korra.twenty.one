...
# threshold in `hermes cron status` (korra_cli/cron.py), so the two never
# drift apart.
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if 'T' in schedule or re.match(r'^\d{4}-\d{2}-\d{2}', schedule):
        try:
            ...
            # against `korra_time.now()`, which uses the configured zone. If a
...
def _normalize_reasoning_effort(value: Any) -> Optional[str]:
    ...
    every other effort surface uses (``korra_constants.parse_reasoning_effort``)
...
# execution. Unlike the quick-snapshot store (`korra_cli.backup`, capped at 20)
...
def _prune_job_output(job_output_dir: Path, keep: int) -> int:
    ...
    Mirrors the quick-snapshot retention in ``korra_cli.backup._prune_quick_snapshots``:
