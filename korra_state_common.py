...
Extracted verbatim from korra_state.py so the SessionDB mixin modules
(korra_state_search / korra_state_schema / korra_state_portability) can
reference them without importing korra_state (which would be a cycle).
korra_state re-imports every name here for backward compatibility.
"""
...
#
# Semantics mirror `korra_state._cross_process_repair_lock` (the schema-
...
# rebuilder cannot wedge future rebuilds. It lives here (not korra_state)
# because the search/schema mixins cannot import korra_state (cycle).
#
...
logger = logging.getLogger("korra_state")
