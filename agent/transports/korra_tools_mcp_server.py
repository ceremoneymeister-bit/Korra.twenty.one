...
Run with: python -m agent.transports.korra_tools_mcp_server
...
from korra_constants import korra_env_setdefault
...
def _build_server() -> Any:
    ...
    mcp = MCPServer(
        "hermes-tools",
        instructions=(
            "Korra's tool surface, exposed for use inside a Codex "
    ...
    for name in EXPOSED_TOOLS:
        ...
        description = spec.get("description") or f"Korra {name} tool"
...
def main(argv: Optional[list[str]] = None) -> int:
    """Entry point for `python -m agent.transports.korra_tools_mcp_server`."""
    ...
    korra_env_setdefault(os.environ, "KORRA_QUIET", "1")
    korra_env_setdefault(os.environ, "KORRA_REDACT_SECRETS", "true")
