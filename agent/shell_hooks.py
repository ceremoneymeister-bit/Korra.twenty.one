...
* Python plugins and shell hooks compose naturally: both flow through
  :func:`korra_cli.plugins.invoke_hook` and its aggregators.  Python
...
* Registration is idempotent — safe to invoke from both the CLI entry
  point (``korra_cli/main.py``) and the gateway entry point
  (``gateway/run.py``).
...
from korra_cli._subprocess_compat import IS_WINDOWS, kill_process_tree, windows_hide_flags
...
# korra_cli.plugins.get_pre_tool_call_block_message / _get_pre_tool_call_
...
def register_from_config(
    ...
    *,
    ...
    ``cfg`` is the full parsed config dict (``korra_cli.config.load_config``
...
def _parse_hooks_block(hooks_cfg: Any) -> List[ShellHookSpec]:
    ...
    """
    from korra_cli.plugins import SHELL_UNSUPPORTED_HOOKS, VALID_HOOKS
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    ...
    :func:`korra_cli.plugins.get_pre_tool_call_block_message`.  This is
...
def _prompt_and_record(
    ...
    print(
        f"\n⚠ Korra is about to register a shell hook that will run a\n"
...
def run_once(
    ...
    ``kwargs`` is the same dict that :func:`korra_cli.plugins.invoke_hook`
