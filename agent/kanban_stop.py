...
def kanban_stop_nudge_enabled() -> bool:
    ...
    """
    env = korra_env("HERMES_KANBAN_STOP_NUDGE")
    if env is not None and env.strip().lower() in {"0", "false", "no", "off"}:
        return False
    task = (korra_env("HERMES_KANBAN_TASK") or "").strip()
    return bool(task)
...
def build_kanban_stop_nudge(
    *,
    ...
    tid = (task_id or korra_env("HERMES_KANBAN_TASK") or "").strip() or "this task"
    return (
        "[System: You are a Korra kanban worker. A plain-text reply is NOT a "
