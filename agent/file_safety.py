...
def _hermes_home_path() -> Path:
    ...
    try:
        from korra_constants import get_hermes_home  # local import to avoid cycles
...
def _hermes_root_path() -> Path:
    ...
    try:
        from korra_constants import get_default_hermes_root  # local import to avoid cycles
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    for hd in hermes_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Korra cache file "
    ...
    for hd in hermes_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Korra credential store "
    ...
    for hd in hermes_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Korra MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Korra MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
    ...
    for hd in hermes_dirs:
        ...
        if resolved == browser_profile:
            return (
                f"Access denied: {path} is the Korra real-profile browser "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is inside the Korra real-profile browser "
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Korra process never "
...
def get_container_mirror_warning(
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Korra "
