...
def is_free_tier_model(model: str, base_url: str = "") -> bool:
    ...
    3. A peek into the in-process pricing cache in ``korra_cli.models``
    ...
    if not base_url:
        return False
    try:
        from korra_cli.models import _is_model_free, _pricing_cache
...
def dev_fixture_credits_state() -> Optional[CreditsState]:
    ...
    on a real account.
    """
    if not is_truthy_value(korra_env("KORRA_DEV_CREDITS")):
        return None
    raw = korra_env("KORRA_DEV_CREDITS_FIXTURE", "").strip()
    if not raw:
        return None
    name = raw
