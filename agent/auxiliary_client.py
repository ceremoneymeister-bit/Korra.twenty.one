...
# Absolute monotonic deadline of the host waiting for this auxiliary stream.
# Compression publishes its commit-fence ceiling here so provider consumers
# stop when their result can no longer be committed.
...
def _anthropic_aux_stream_event_hook() -> Callable[[Any], None]:
    """Track Anthropic progress and stop at the host deadline or hard cancel."""
...
def _current_aux_stream_deadline() -> Optional[float]:
    """Return the waiting host's absolute monotonic deadline, if installed."""
    return getattr(_aux_stream_deadline, "value", None)


@contextlib.contextmanager
def aux_stream_deadline(deadline: Optional[float]):
    """Publish a host deadline to every stream consumer in this aux attempt.

    ``None`` inherits an outer deadline, so nested auxiliary work cannot
    accidentally escape the compression host's ceiling. The previous value is
    always restored when the attempt unwinds.
    """
    previous = getattr(_aux_stream_deadline, "value", None)
    effective = deadline if isinstance(deadline, (int, float)) else previous
    if isinstance(previous, (int, float)) and isinstance(effective, (int, float)):
        effective = min(float(previous), float(effective))
    _aux_stream_deadline.value = effective
    try:
        yield
    finally:
...
def _run_protected_sync_provider_call(
    ...
    # Stream consumption happens on the isolated daemon below. Thread-locals
    # do not cross that boundary without this explicit propagation.
    host_deadline = _current_aux_stream_deadline()
...
def _fast_model_from_catalog(provider_id: str) -> str:
    ...
    """
    try:
        from korra_cli.auth import resolve_api_key_provider_credentials
        from korra_cli.models import fetch_models_with_pricing
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = korra_env("KORRA_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = korra_env("KORRA_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
# in lockstep with korra_cli.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    ...
    Computed at call time so a hot-reloaded ``korra_cli.__version__`` is
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    if getattr(entry, "provider", None) == "nous":
        ...
        from korra_cli.auth import _nous_inference_env_override
...
def _nous_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(korra_env("KORRA_NOUS_MIN_KEY_TTL_SECONDS", "1800")))
...
class _CodexCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        # A live stream can continuously re-arm its idle watchdog. Clamp its
        # absolute backstop to the host's commit deadline so it cannot keep a
        # socket open after compression has already abandoned the result.
        _host_deadline = _current_aux_stream_deadline()
        if (
            isinstance(_host_deadline, (int, float))
            and _host_deadline < hard_deadline
        ):
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        response = create_anthropic_message(
            ...
            stream_deadline_monotonic=_current_aux_stream_deadline(),
        )
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``korra_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _nous_api_key(provider: dict) -> str:
    ...
    from korra_cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from korra_cli.auth import _agent_key_is_usable
...
def _resolve_nous_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from korra_cli.auth import resolve_nous_runtime_credentials
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    Falls back to ``korra_cli.auth``'s singleton runtime resolver for older
    ...
    try:
        from korra_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    ...
    try:
        from korra_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from korra_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from korra_cli.auth import is_provider_explicitly_configured
...
def _aux_openrouter_settings() -> Tuple[bool, str]:
    ...
    """
    try:
        from korra_cli.config import cfg_get, load_config_readonly
...
def _try_azure_foundry(
    *,
    ...
    :func:`korra_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from korra_cli.runtime_provider import _resolve_azure_foundry_runtime
...
def _fallback_entry_api_key(entry: Dict[str, Any]) -> Optional[str]:
    ...
    gateway multiplexing (see ``korra_cli.fallback_config.resolve_entry_api_key``).
    """
...
def _resolve_auto_route(
    ...
    if (main_provider and main_model
        ...
        if runtime_base_url and main_provider == "custom":
            ...
            try:
                from korra_cli.runtime_provider import _get_named_custom_provider
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        from korra_cli.copilot_auth import copilot_request_headers
...
def resolve_provider_client(
    ...
    if provider == "nous":
        ...
        from korra_cli.providers import nous_api_mode
    ...
    try:
        ...
        logger.debug("korra_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        try:
            from korra_cli.models import opencode_zen_free_runtime as _oc_free_rt
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from korra_cli.models import _should_use_copilot_responses_api
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`korra_cli.plugins.PluginContext.register_auxiliary_task`) the
...
def _fast_lane_config_fields(
    ...
    """
    from korra_constants import parse_reasoning_effort
