...
# methods, not a growing archive of incident retellings or per-session files.
_LESSON_LAYER_BLOCK = (
    "What a skill IS: instructions for doing a class of task efficiently and "
    "correctly, to THIS user's specifications — the procedure, tools and "
    "commands that work, their order, the user's preferences for the result, "
    "and the pitfalls that cost time. A future session should be able to "
    "follow it and produce what the user wants on the first try. Everything "
    "below is about writing that well:\n"
    "  • Procedure first: steps in execution order, with concrete commands, "
    "tool calls, and decision points. Attach lessons and pitfalls to the step "
    "they affect.\n"
    "  • A pitfall is a generalizable imperative rule plus one clause of WHY "
    "(the mechanism). State the working rule, not a narrative of this session.\n"
    "  • Do not use PR/issue numbers, dates, ticket IDs, or quoted user text as "
    "the lesson itself — the rule must stand without the incident behind it. "
    "Keep a short quote only when it is the clearest statement of a durable "
    "user preference.\n"
    "  • The same lesson learned twice is ONE rule. Search SKILL.md and its "
    "references/ first; strengthen the existing rule instead of appending a "
    "duplicate.\n"
    "  • Do not duplicate always-loaded context such as AGENTS.md or tool "
    "schemas. A skill carries workflow and pitfalls, not the repository map "
    "or parameter lists.\n"
    "  • Always-on rules and standing user preferences live in SKILL.md. "
    "references/ holds a small set of reusable topical depth such as a "
    "decision table, recipe, or domain note — never '<date>-<incident>.md'. "
...
_SKILL_REVIEW_PROMPT = (
    ...
    "Target shape of the library: CLASS-LEVEL skills, each with a SKILL.md "
    "of always-on rules and a small `references/` set of topical depth. Not "
    "a flat list of narrow one-session skills, and not an umbrella hoarding "
    "one references/ file per session. This shapes HOW you update, not "
    "WHETHER you update.\n\n"
    + _LESSON_LAYER_BLOCK
    ...
    "directory per kind:\n"
    "     • `references/<topic>.md` — topical depth needed only sometimes: "
    "a decision table, reproduction recipe, provider quirk, condensed domain "
    "note, or API excerpt. Name it by TOPIC and extend an existing file when "
    "one covers the topic; never create a per-session/per-incident file or "
    "paste an error transcript — distill it to the rule.\n"
...
_COMBINED_REVIEW_PROMPT = (
    ...
    "Target shape of the skill library: CLASS-LEVEL skills with a SKILL.md "
    "of always-on rules and a small `references/` set of topical depth — not "
    "narrow one-session skills, and not an umbrella hoarding a references/ "
    "file per session.\n\n"
    + _LESSON_LAYER_BLOCK
    ...
    "`references/<topic>.md` for topical depth (decision tables, recipes, "
    "quirks, condensed domain notes) — extend an existing topical file before "
    "creating one, never a per-session file; `templates/<name>.<ext>` "
...
def build_memory_write_metadata(
    ...
    *,
    ...
    metadata: Dict[str, Any] = {
        ...
        "platform": agent.platform or korra_env("KORRA_SESSION_SOURCE", "cli"),
        "tool_name": "memory",
    }
    if task_id:
