...
CURATOR_REVIEW_PROMPT = (
    "You are running as Korra's background skill CURATOR. This is an "
    ...
    "The right target shape is CLASS-LEVEL skills whose SKILL.md carries "
    "always-on rules and whose `references/`, `templates/`, and `scripts/` "
    "hold a SMALL set of topical depth — not one-session-one-skill "
    "micro-entries, and not an umbrella that hoards one references/ file per "
    "absorbed sibling. Consolidation means DISTILLING: absorbed content "
    "becomes task-ordered procedure and reusable rules (imperative plus one "
    "clause of why); duplicate lessons become one rule; incident narration, "
    "PR/issue numbers, dates, and quoted chatter are dropped. Moving a file "
    "unchanged under references/ is filing, not consolidating.\n\n"
    ...
    "narrow-but-valuable depth needed only sometimes. Distill it into the "
    "umbrella's appropriate support directory:\n"
    "      • `references/<topic>.md` — named by TOPIC and merged into an "
