...
The version is sourced live from ``korra_cli.__version__`` so it auto-aligns
...
``korra_cli.__version__`` is the canonical source of truth.
"""
...
def _hermes_version() -> str:
    ...
    Falls back to ``"unknown"`` if ``korra_cli`` cannot be imported (should
