  ...
  function TaskDetail({ taskId, board, profiles, onClose, onMove, onEdit, onRefresh, onOpenTask }) {
    ...
    return h(Modal, { title: task ? task.title : "Поручение", description: task ? `${statusLabel(task.status)} · ${profile ? profileLabel(profile) : task.assignee || (task.status === "review" ? "Ждёт вашей проверки" : "Исполнитель не назначен")}` : "Загружаем карточку…", busy, onClose },
      ...
      task && h("div", { className: "k21-task-detail" },
        ...
        !blockReason && h("section", { className: "k21-note" }, h("h3", null, ["done", "review", "archived"].includes(task.status) ? "Результат" : "Последняя запись агента"), h(RichText, null, summary || "Здесь появится итог работы агента. Он останется в карточке после завершения.")),
        h(["done", "review", "archived"].includes(task.status) ? "details" : "section", null,
          h(["done", "review", "archived"].includes(task.status) ? "summary" : "h3", null, "Задание"),
