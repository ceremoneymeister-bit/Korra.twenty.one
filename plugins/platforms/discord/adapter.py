...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = env_float("KORRA_DISCORD_TEXT_BATCH_DELAY_SECONDS", 0.6)
        self._text_batch_split_delay_seconds = env_float("KORRA_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0)
        ...
        self._liveness_interval_seconds = self._finite_positive_config_float(
            ...
            15.0,
            env_key="KORRA_DISCORD_LIVENESS_INTERVAL_SECONDS",
        )
        self._liveness_failure_threshold = self._config_int(
            "websocket_liveness_failure_threshold",
            2,
            env_key="KORRA_DISCORD_LIVENESS_FAILURE_THRESHOLD",
        )
    ...
    def _register_slash_commands(self) -> None:
        ...
        # korra_cli/commands.py automatically appear as Discord slash
        ...
        dropped_over_cap = 0
        try:
            from korra_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from korra_cli.commands import _iter_plugin_command_entries
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from korra_cli.commands import discord_skill_commands_by_category
...
def interactive_setup() -> None:
    ...
    """
    from korra_cli.config import get_env_value, remove_env_value, save_env_value
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    _websocket_liveness_keys = (
        (
            ...
            "KORRA_DISCORD_LIVENESS_INTERVAL_SECONDS",
        ),
        (
            "websocket_liveness_failure_threshold",
            "liveness_failure_threshold",
            "KORRA_DISCORD_LIVENESS_FAILURE_THRESHOLD",
        ),
    ...
    for primary_key, legacy_key, env_key in _websocket_liveness_keys:
        ...
        if value is not None:
            ...
            if env_key and not korra_env(env_key):
                korra_env_set(os.environ, env_key, str(value))
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``korra_cli.gateway.get_env_value`` at call time (not via
