...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            from korra_cli.commands import slack_native_slashes
            import re as _re
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        if slash_name in {"hermes", ""}:
            ...
            from korra_cli.commands import slack_subcommand_map
...
# ``korra_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``korra_cli/setup.py::_setup_slack``.
    """
    ...
    def _write_slack_manifest_and_instruct() -> None:
        ...
        try:
            from korra_cli.slack_cli import _build_full_manifest
            from korra_constants import get_hermes_home
            import json as _json
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``korra_cli.gateway.get_env_value`` at call time (not via the
