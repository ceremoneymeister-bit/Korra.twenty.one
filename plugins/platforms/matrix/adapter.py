...
def _resolve_matrix_bang_command(name: str) -> str | None:
    ...
    *name* only by underscore→hyphen normalization, e.g. ``reload_skills`` →
    ...
    try:
        from korra_cli.commands import is_gateway_known_command
...
from korra_constants import get_hermes_dir as _get_hermes_dir, korra_env
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(
            korra_env("KORRA_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
        self._text_batch_split_delay_seconds = float(
            korra_env("KORRA_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
...
# + _PLATFORMS["matrix"] static dict in korra_cli/{setup,gateway}.py, and the
...
def interactive_setup() -> None:
    """Configure Matrix credentials. Replaces korra_cli/setup.py::_setup_matrix
...
def _is_connected(config) -> bool:
    ...
    configured. Read via korra_cli.gateway.get_env_value so setup-status
