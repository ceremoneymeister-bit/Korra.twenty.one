...
# dict in korra_cli/gateway.py, and the _send_whatsapp dispatch in
...
def interactive_setup() -> None:
    ...
    Replaces the central _setup_whatsapp in korra_cli/gateway.py and the
...
def _is_connected(config) -> bool:
    ...
    if config is not None and getattr(config, "enabled", False) and extra:
        ...
        return True
    # Read via korra_cli.gateway.get_env_value (not os.getenv) so setup-status
