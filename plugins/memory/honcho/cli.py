...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom korra_cli already uses for gateway refs).
    """
    try:
...
def _headless() -> tuple[bool, bool]:
    """(is_remote, can_open_browser) — degrades safely if korra_cli internals move."""
    try:
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        from korra_cli.memory_setup import cmd_setup_provider
