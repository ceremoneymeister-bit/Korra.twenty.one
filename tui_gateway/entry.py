...
# from shadowing Hermes's own top-level modules.  ``korra_bootstrap`` lives at
...
# shared owner in korra_cli.mcp_startup — the wait/in-flight/join helpers
...
# korra_cli.mcp_startup.start_background_mcp_discovery can actually fire —
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    CLI path via ``korra_cli.mcp_startup``); ``timeout`` overrides it.
    """
    thread = _mcp_discovery_thread
    if thread is not None and thread.is_alive():
        try:
            from korra_cli.mcp_startup import _resolve_discovery_timeout
    ...
    # → korra_cli.mcp_startup); wait on it so the first agent build still
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``korra_cli.mcp_startup.start_background_mcp_discovery``. The late-refresh
...
def join_mcp_discovery(timeout: float | None = None) -> bool:
    ...
    entry thread first, then the ``korra_cli.mcp_startup`` thread used by the
    ...
    try:
        from korra_cli.mcp_startup import join_mcp_discovery as _startup_join
...
def _has_configured_mcp_servers() -> bool:
    ...
    from korra_cli.mcp_startup import _has_configured_mcp_servers as configured
...
def ensure_mcp_discovery_started() -> None:
    ...
    ``korra_cli.mcp_startup`` captures the caller's context-local override
...
def main():
    ...
    try:
        from korra_cli.model_switch import prewarm_picker_cache_async
