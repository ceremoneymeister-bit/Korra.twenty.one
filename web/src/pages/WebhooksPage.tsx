...
export default function WebhooksPage() {
  ...
  const loadWebhooks = useCallback(() => {
    return api
      .getWebhooks()
      .then(setData)
      .catch(() => showToast(tr("Failed to load webhooks"), "error"))
    ...
    for (let i = 0; i < 20; i++) {
      ...
      try {
        ...
        if (st.exit_code !== 0 && st.exit_code !== null) {
          ...
          setRestartError(tr("Gateway restart failed with exit {code}.", { code: st.exit_code }));
          showToast(
  ...
  const handleRestart = useCallback(async () => {
    ...
    try {
      ...
      setRestartNeeded(false);
      setRestartError(null);
      setRestartMessage(tr("Gateway restarting…"));
  ...
  }, [loadWebhooks, showToast, watchRestartOutcome, tr]);
  ...
  const handleEnableWebhooks = useCallback(async () => {
    ...
    try {
      ...
      if (result.restart_started) {
        setRestartMessage(tr("Webhooks enabled; gateway restarting…"));
        showToast(tr("Webhooks enabled; gateway restarting…"), "success");
        setTimeout(() => void loadWebhooks(), 4000);
        void watchRestartOutcome();
      } else {
  ...
  const handleCreate = async () => {
    if (!name.trim()) {
      showToast(tr("Name required"), "error");
      return;
    }
    setCreating(true);
    try {
  ...
  const handleToggleEnabled = useCallback(
    async (subName: string, nextEnabled: boolean) => {
      ...
      try {
        ...
        showToast(
          tr(nextEnabled ? "Enabled: “{name}”" : "Disabled: “{name}”", { name: subName }),
          "success",
        );
        loadWebhooks();
      } catch (e) {
  ...
  }, [setEnd, enabled, enabling, loading, tr]);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <DeleteConfirmDialog
        ...
        title={tr("Delete webhook")}
        description={
          pendingName
            ? tr("“{name}” — this will permanently remove this webhook subscription.", { name: pendingName })
            : tr("This will permanently remove this webhook subscription.")
        }
            ...
            {created ? (
              <div className="p-5 grid gap-4">
                <p className="text-sm text-muted-foreground">
                  {tr("Subscription created. Copy the secret now — it is only shown once.")}
                </p>

                <div className="grid gap-2">
                  <Label>{tr("Webhook URL")}</Label>
                ...
                <div className="grid gap-2">
                  <Label>{tr("Secret (shown once)")}</Label>
              ...
              <div className="p-5 grid gap-4">
                <div className="grid gap-2">
                  <Label htmlFor="webhook-name">{tr("Name")}</Label>
                  <Input
                    id="webhook-name"
                    autoFocus
                    placeholder={tr("e.g. github-push")}
                    value={name}
                ...
                <div className="grid gap-2">
                  <Label htmlFor="webhook-description">{tr("Description")}</Label>
                  <Input
                    id="webhook-description"
                    placeholder={tr("What this webhook does (optional)")}
                    value={description}
                ...
                <div className="grid gap-2">
                  <Label htmlFor="webhook-events">{tr("Events")}</Label>
                  <Input
                    id="webhook-events"
                    placeholder={tr("comma-separated, leave empty for all")}
                    value={events}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="grid gap-2">
                    <Label htmlFor="webhook-deliver">{tr("Deliver to")}</Label>
                    <Select
                      id="webhook-deliver"
                      value={deliver}
                      onValueChange={(v) => setDeliver(v)}
                    >
                      <SelectOption value="log">{tr("Log")}</SelectOption>
                      ...
                      <SelectOption value="email">{tr("Email")}</SelectOption>
                  ...
                  <div className="grid gap-2">
                    <Label htmlFor="webhook-deliver-only">{tr("Deliver only")}</Label>
                    <label className="flex items-center gap-2 text-sm text-muted-foreground h-9">
                      ...
                      />
                      {tr("Skip the agent, deliver payload directly")}
                    </label>
                  </div>
                </div>

                <div className="grid gap-2">
                  <Label htmlFor="webhook-prompt">{tr("Prompt")}</Label>
                  <textarea
                    id="webhook-prompt"
                    className="flex min-h-[80px] w-full border border-border bg-background/40 px-3 py-2 text-sm font-courier shadow-sm placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-foreground/30 focus-visible:border-foreground/25"
                    placeholder={tr("Instructions for the agent when this webhook fires (optional)")}
                    value={prompt}
          ...
          <CardContent className="flex flex-col gap-4 py-6 text-sm sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-start gap-3">
              ...
              <div className="flex flex-col gap-1">
                <span className="font-medium">{tr("Webhook receiver disabled")}</span>
                <span className="text-muted-foreground">
                  {tr("Webhooks are their own gateway platform. Enable them here to accept incoming HTTP events; chat channels are only needed when a subscription delivers to Telegram, Discord, Slack, or another channel.")}
                </span>
              </div>
            </div>
            <Button
            ...
            >
              {enabling ? tr("Enabling…") : tr("Enable webhooks")}
            </Button>
          </CardContent>
        </Card>
      )}
        ...
        <Card className="border-warning/50">
          <CardContent className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-start gap-2 text-sm">
              ...
              <span>
                {restartError ??
                  tr("Webhooks are enabled, but the gateway still needs a restart before the receiver can come online.")}
              </span>
            </div>
            <Button
              size="sm"
      ...
      <div className="flex flex-col gap-3">
        ...
        >
          ...
          {tr("Subscriptions ({count})", { count: subscriptions.length })}
        </H2>

        <p className="text-xs text-muted-foreground -mt-1">
          {tr("Subscription changes hot-reload once the webhook receiver is running. Disabled subscriptions reject incoming events.")}
        </p>

        {subscriptions.length === 0 && (
          <Card>
            <CardContent className="py-8 text-center text-sm text-muted-foreground">
              {tr("No webhook subscriptions yet.")}
            </CardContent>
          </Card>
        )}
            ...
            <CardContent className="flex items-start gap-4 py-4">
              <div className={cn("flex-1 min-w-0", !sub.enabled && "opacity-60")}>
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  ...
                  {sub.deliver_only && (
                    <Badge tone="secondary">{tr("deliver only")}</Badge>
                  )}
                  {!sub.enabled && <Badge tone="warning">{tr("disabled")}</Badge>}
                </div>
                ...
                <div className="flex items-center gap-1 flex-wrap mb-2">
                  {sub.events.length === 0 ? (
                    <Badge tone="secondary">{tr("(all)")}</Badge>
                  ) : (
                    sub.events.map((evt) => (
              ...
              <div className="flex items-center gap-1 shrink-0">
                ...
                >
                  {sub.enabled ? tr("Disable") : tr("Enable")}
                </Button>
                <Button
                  ghost
                  destructive
