    ...
    >
      ...
      {open && hasBody && (
        <div className="border-t border-border/60 px-3 py-2 space-y-2 text-xs font-mono">
          {tool.context && <Section label={tr("context")}>{tool.context}</Section>}

          {tool.preview && tool.status === "running" && (
            <Section label={tr("streaming")}>
              {tool.preview}
          ...
          {tool.error && (
            <Section label={tr("error")} tone="error">
