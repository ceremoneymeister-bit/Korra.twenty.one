...
function EditorBody({
  ...
  const [content, setContent] = useState(
    isEdit ? "" : tr("skillEditor.createTemplate"),
  );
  ...
  const handleSave = async () => {
    setError(null);
    if (!isEdit && !name.trim()) {
      setError(tr("Skill name is required."));
      return;
    }
    if (!content.trim()) {
      setError(tr("SKILL.md content is required."));
      return;
    }
    setSaving(true);
    try {
  ...
  return (
    <>
      <DialogHeader>
        <DialogTitle>
          {isEdit
            ? tr("Edit skill: {name}", { name: editName })
            : tr("New skill")}
        </DialogTitle>
        <DialogDescription>
          {isEdit
            ? tr("Rewrite this skill's SKILL.md. Frontmatter (name, description) is validated on save.")
            : tr("Author a custom skill — YAML frontmatter plus markdown instructions. It becomes available to the agent and attachable to cron jobs.")}
        </DialogDescription>
      </DialogHeader>
        ...
        {!isEdit && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="grid gap-1.5">
              <Label htmlFor="skill-editor-name">{tr("Name")}</Label>
              <Input
            ...
            <div className="grid gap-1.5">
              <Label htmlFor="skill-editor-category">{tr("Category (optional)")}</Label>
              <Input
