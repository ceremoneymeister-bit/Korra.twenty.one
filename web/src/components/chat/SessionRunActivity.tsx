...
export function SessionRunActivity({ tabs }: { tabs: AgentTabConfig[] }) {
  ...
  const href = (profile: string, sessionId: string) => `/agents?agent=${encodeURIComponent(profile || "default")}&resume=${encodeURIComponent(sessionId)}`;
  ...
  return <>
    {busy.length > 0 && <div className="flex shrink-0 flex-wrap items-center gap-3 py-2 text-sm" aria-label="Сейчас в работе">
      ...
      {busy.map(run => <Link key={run.message_id} to={href(run.profile, run.session_id)} className="neo-tab rounded-full px-3 py-2" title={run.user_message.content}>
    ...
    {unread.length > 0 && createPortal(<div className="fixed bottom-5 right-5 z-50 max-w-sm rounded-2xl bg-[var(--neo-surface)] p-4 shadow-[var(--neo-depth-2)]" role="status">
      {unread.slice(0, 3).map(run => <Link className="block py-1 text-sm" key={run.message_id} to={href(run.profile, run.session_id)}>{name(run.profile)} ответил · Открыть чат</Link>)}
