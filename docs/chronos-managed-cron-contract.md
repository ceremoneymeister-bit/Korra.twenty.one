...
Chronos lets a hosted Korra gateway **scale to zero** while idle and still
...
ChronosCronScheduler.reconcile()        ── agent computes next_run_at
  │  POST {portal}/api/agent-cron/provision   (auth: agent's portal access token)
  ▼
...
|---|---|---|---|
| 1 | agent → NAS (`provision`/`cancel`/`list`) | the agent's existing **portal access token** (Bearer) — for a hosted agent this is the **bootstrap-session token** NAS planted in `auth.json` (client `hermes-cli-vps`), NOT an `agent:*` client token | NAS (its normal agent-token path) |
...
- **Auth:** `Authorization: Bearer <agent portal access token>`. NAS validates via
...
1. **Dashboard app** (`korra_cli/web_server.py`) — the agent's only public
...
If `callback_url` / `portal_url` is blank or the agent has no portal login,
