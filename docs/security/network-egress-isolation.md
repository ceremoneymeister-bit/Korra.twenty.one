...
When running Korra inside Docker, the default `network_mode: host` gives the
...
The Korra [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   KORRA_UID=$(id -u) KORRA_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=korra,korra-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://korra-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Korra trust model and vulnerability reporting
