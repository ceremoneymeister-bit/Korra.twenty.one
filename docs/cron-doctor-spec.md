...
a provider run fails, or delivery starts failing. `korra cron list` shows some of
...
Add `korra cron doctor` as a read-only diagnostic command that summarizes cron
...
- `korra cron doctor` returns `0` and prints a healthy message when active jobs
