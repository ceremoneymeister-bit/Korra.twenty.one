...
This document is the formal interface between the **Korra gateway** (Python,
`gateway/relay/`) and the **connector** (Node/TypeScript,
the separate `gateway-gateway` repository). The connector implementer's first action is to
read this file.
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `korra gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`korra gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `korra` CLI.
