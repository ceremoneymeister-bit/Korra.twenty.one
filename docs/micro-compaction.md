...
has to be thrown away or summarized. Korra has always done this in one batch:
...
Korra folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Korra's normal session flush is
...
With `micro_compact` unset or `false` Korra behaves exactly as it always has:
