...
The canonical transcript store is SQLite via `SessionDB` (from `korra_state`). The
...
This prevents unwanted auto-resets after `korra update`, `korra gateway restart`,
or `/restart`.
