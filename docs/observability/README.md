# Korra Observer Hooks

Korra observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Korra's planner, model providers, memory, tool registry,
...
Korra also has a first-party NeMo Relay shared-metrics path. It uses these
...
Hook callbacks are fail-open. Korra catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Korra only builds
...
The native NeMo Relay SDK integration maps Korra session, turn, LLM, and tool
