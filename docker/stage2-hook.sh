...
# entries of korra_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
# Korra: засев берёт наши короткие русские шаблоны, а не апстримовые.
# Апстримовый cli-config.yaml.example — это 107 КБ, где ПЕРЕЧИСЛЕНЫ почти все
# ключи со значениями апстрима. Скопированный в контур, он молча перекрывал
# дефолты Korra: свежая раскатка получала распознавание речи на английском,
# модель whisper "base" вместо вшитой в образ и модель ответа через OpenRouter
# — при том, что в дефолтах движка стоит ровно то, что нужно. Наши шаблоны
# короткие: чего в них нет, то берётся из дефолтов и обновляется вместе с
# образом. seed_one молча пропускает отсутствующий источник, поэтому вторая
# строка каждой пары — запасной путь для сборок без файла Korra.
...
seed_one ".env" "korra-env.example"
seed_one ".env" ".env.example"
seed_one "config.yaml" "korra-config.yaml.example"
