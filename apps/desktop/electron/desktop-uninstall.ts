...
/**
 * Map an uninstall mode to the `python -m korra_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `korra_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'korra_cli.uninstall', '--mode', mode]
}
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import korra_cli`
 ...
 * modes + `pythonPath`=agentRoot so `import korra_cli` resolves from source
