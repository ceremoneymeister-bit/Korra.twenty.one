...
// _apply_profile_override in korra_cli/main.py) and bypasses the sticky
// ~/.hermes/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors korra_cli.profiles._PROFILE_ID_RE so we never hand the backend a
...
// errors.log, gateway.log produced by korra_logging.setup_logging — one log
...
//
// Mirror the Python logs (korra_logging.py RotatingFileHandler, maxBytes x
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'korra_cli', 'subcommands', 'dashboard.py'), 'utf8')
...
function isHermesSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'korra_cli', 'main.py'))
}
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  // backend is `python.exe -m korra_cli.main serve`, which need not hold
...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'korra_cli.main', ...backendArgs],
...
function resolveHermesBackend(backendArgs) {
  ...
  // 5. Last-ditch: pip-installed korra_cli module via system Python.
  ...
  if (python) {
    ...
    // having korra_cli installed -- common on dev boxes that have
    ...
    if (canImportHermesCli(python)) {
      return {
        kind: 'python',
        label: `installed korra_cli module via ${python}`,
        command: python,
        args: ['-m', 'korra_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: korra_cli is not importable; falling through to bootstrap.`)
  }
...
async function ensureRuntime(backend) {
  ...
  if (!fileExists(venvPython)) {
    ...
    // plus an importable korra_cli before it hands back the active runtime.
...
// feature; the server half lives in korra_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// convention (korra_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
...
async function spawnPoolBackend(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  // step 3 in korra_cli/main.py), so the child re-homes to this profile.
...
// `python -m korra_cli.main`, so we resolve the SAME backend the app itself
...
// korra_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
...
function resolveHermesVersion() {
  try {
    ...
    const initPath = path.join(root, 'korra_cli', '__init__.py')
...
// (korra_cli/uninstall.py + korra_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'korra_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
...
async function runDesktopUninstall(mode) {
  ...
  // Python with PYTHONPATH=<agentRoot> so `import korra_cli` resolves from
