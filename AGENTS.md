# Korra - Development Guide

Instructions for AI coding assistants and developers working on the Korra codebase.
...
Read the balance right: Korra ships a **lot** — most merges are bug fixes to
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`korra tools`, `korra setup`, auto-install) rather than bolting on a raw
  env var.
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `korra tools`
...
- **Third-party products / other people's projects integrated into the core
  ...
  **standalone plugin repo** users install into `~/.hermes/plugins/` (or via a
  pip entry point), and promote them in the community channels. This is a
  coupling-and-maintenance decision, not
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `korra <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `korra webhook`, `korra cron`, `korra tools`.
...
one behind a plain URL + token, or a hosted backend. Only the first two are spawned
...
- **Ask which identity you actually mean.** `HERMES_DESKTOP=1` legitimately
  ...
  and the embedded terminal pane (`korra --tui` against that same backend) is
...
├── korra_state.py       # SessionDB — SQLite session store (FTS5 search)
├── korra_constants.py   # get_hermes_home(), display_hermes_home() — profile-aware paths
├── korra_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── korra_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
├── ui-tui/               # Ink (React) terminal UI — `korra --tui`
...
Browse with `korra logs [--follow] [--level ...] [--session ...]`.

## TypeScript Style

Applies to TypeScript across Korra: desktop, TUI, website, and future TS packages.
...
- **Skin engine** (`korra_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `HermesCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.hermes/skills/`, injects as **user message** (not system prompt) to preserve prompt caching

### Slash Command Registry (`korra_cli/commands.py`)
...
1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `korra_cli/commands.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `korra --tui` or `HERMES_TUI=1`.
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMAND_SPECS` (the built-ins and their Desktop surfaces) plus `NO_DESKTOP_SURFACE` block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known Korra built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit Korra core.
...
core Korra tool that should ship in the base system.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `korra_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `korra_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `korra tools`, `korra setup`, most CLI subcommands | `korra_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`korra_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
korra_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
...
- `default` — Classic Korra gold/kawaii (the current look)
...
Add to `_BUILTIN_SKINS` dict in `korra_cli/skin_engine.py`:
...
Korra has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.hermes/plugins/` and pip-installed entry points.

### General plugins (`korra_cli/plugins.py` + `plugins/<name>/`)
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `korra` at startup so
  `korra <pluginname> <subcmd>` works with no change to `main.py`
...
it at argparse setup time and wires it into `korra <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `korra --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `korra_cli/main.py`, etc.).
...
path, and integrate via `korra memory setup` / `post_setup()` without
...
don't own. Promote standalone plugins in the community channels.
...
`plugin-llm-async-example`) live in the upstream `hermes-example-plugins`
companion repo, not in this tree.

### Bot Mode (`apps/desktop/src/plugins/hermes-bots/`)

The desktop "Bots" experience ships bundled in-tree. Each bot is a Korra
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `korra skills install official/<category>/<skill>`. Adapter lives in
...
2. **Tools referenced in SKILL.md prose must be native Korra tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Korra" is the secondary collaborator. If the
   contributor's commit shows "Korra" as author (because they
   used Korra to draft the skill), replace it with their actual name
...
Enable/disable per platform via `korra tools` (the curses UI) or the
...
Korra returns a delegation id immediately and the result re-enters the
...
- **CLI:** `korra_cli/curator.py` wires `korra curator <verb>` where
...
schedule jobs via the `cronjob` tool; users via `korra cron <verb>`
...
collaborate on shared tasks. Users drive it via `korra kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `korra_cli/kanban.py` wires `korra kanban` with verbs
...
- **Plan** (`korra_cli/update_inventory.py`, `korra update --plan`):
...
- **Snapshot** (`korra_cli/backup.py`): pre-update quick snapshot for
...
`korra serve` (control plane, desktop-spawned child) dies with the app
...
Korra ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
Korra supports **profiles** — multiple fully isolated instances, each with its own
`HERMES_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `korra_cli/main.py` sets
`HERMES_HOME` before any module imports. All `get_hermes_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_hermes_home()` for all HERMES_HOME paths.** Import from `korra_constants`.
...
2. **Use `display_hermes_home()` for user-facing messages.** Import from `korra_constants`.
...
6. **Profile operations are HOME-anchored, not HERMES_HOME-anchored** — `_get_profiles_root()`
   ...
   This is intentional — it lets `korra -p coder profile list` see all profiles regardless
...
- Use the canonical matchers: `gateway.status.looks_like_gateway_command_line`
  (gateway run), `korra_cli.update_cmd._hermes_holder_subcommand`
  (top-level subcommand of any Korra argv). Never hand-roll token scans.
...
Use `get_hermes_home()` from `korra_constants` for code paths. Use `display_hermes_home()`
for user-facing print/log messages. Hardcoding `~/.hermes` breaks profiles — each profile
has its own `HERMES_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### All CLI menu-pickers MUST use curses.
Interactive menus must use `korra_cli/curses_ui.py`. See `korra_cli/tools_config.py` for an example.
...
Use the pattern from `tests/korra_cli/test_profiles.py`:
```python
@pytest.fixture
...
Korra supports Linux, macOS and native Windows, and plenty of its behaviour
...
`tests/korra_cli/test_venv_holder_windows_live.py` on a real
