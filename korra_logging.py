...
Component separation:
    ...
    gui.log receives dashboard-side records from ``korra_cli.web_server``,
    ``korra_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
from korra_constants import get_config_path, get_hermes_home
...
COMPONENT_PREFIXES = {
    ...
    "cron": ("cron",),
    "gui": (
        "korra_cli.web_server",
        "korra_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    ...
    *force* is ``True``.
    ...
    from korra_constants import mkdir_under_hermes_home
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def __init__(self, *args, **kwargs):
        from korra_cli.config import is_managed
...
def _read_logging_config():
    ...
    try:
        ...
        # reuses the parse korra_cli.main's early bridge already did (one
        # config.yaml parse per process instead of 3-4). Fall back to a
        # direct parse when korra_cli.config isn't importable (bare
        # korra_logging consumers).
        try:
            from korra_cli.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
