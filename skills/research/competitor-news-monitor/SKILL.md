...
version: 0.1.0
author: Ben Barclay (benbarclay), Korra
license: MIT
...
Use `blogwatcher` for feeds and `web_search`/`web_extract` for pages. Write the watch contract (watchlist, categories, materiality threshold, last cutoff) to a state file under `$HERMES_HOME/competitor-watches/<watch-slug>.json`, then create the job:
