...
metadata:
  hermes:
    ...
    related_skills: [claude-code, korra-agent]
---

# Codex CLI

Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Korra terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Korra itself, `model.provider: openai-codex` uses Korra-managed Codex
OAuth from `$HERMES_HOME/auth.json` after `korra auth add openai-codex`. For the
...
## Korra Gateway Caveat

When invoking the Codex CLI from a Korra gateway/service context (for example,
