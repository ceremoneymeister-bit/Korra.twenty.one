...
**three** code paths in Korra that cap a batch. If none of them fired, the
cap came from the **model itself** — not from Korra — and the user's
narration of "the runtime caps at N" is the model rationalising its own
choice.

## The three real caps in Korra
...
3. **Cost-warning** — same `_get_max_concurrent_children()`. When the
   ...
   log line** — it does not cap anything. Easy to mis-read as "Korra is
   refusing my value."
...
korra config get delegation.max_concurrent_children

# 2. Did Korra's truncator or rejector actually fire?
grep -E "Truncated.*delegate_task|Too many tasks" $HERMES_HOME/logs/agent.log | tail
# If neither line appears, neither cap path executed.

# 3. Confirm the resolver returns what config says (in the venv, from the repo root)
...
appears, **the cap is the model**, not Korra.
