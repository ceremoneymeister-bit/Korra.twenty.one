...
├── korra_state.py       # SQLite session store
├── agent/                # Prompt builder, context compression, memory, model routing, credential pooling, skill dispatch
├── korra_cli/           # CLI subcommands, config, setup, commands
...
1. Add `CommandDef` to `COMMAND_REGISTRY` in `korra_cli/commands.py`
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/korra_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_hermes_home()` from `korra_constants` for all paths (profile-safe)
