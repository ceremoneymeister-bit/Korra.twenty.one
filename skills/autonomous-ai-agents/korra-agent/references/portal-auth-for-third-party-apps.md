...
## Layer 1 — Is this thing a Korra plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Korra process**. Its LLM calls go through Korra's already-configured provider. | Already uses Portal if user's Korra is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Korra. That LLM call is already authenticated
via Korra's provider config. The plugin's own server auth (e.g.
...
What Korra has that *feels* like OAuth — `korra login --provider nous`
opening a browser, user signs in, token lands in `$HERMES_HOME/auth.json` — is a
**Korra-specific browser flow**. Under the hood it produces a credential
Korra uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Korra's existing Portal credential out of `$HERMES_HOME/auth.json`.
...
rides on the credential Korra already holds.

Where this could live in Korra:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Korra itself.
...
1. First decide: Korra plugin (runs inside Korra) or separate app? If plugin,
   it already uses Portal via Korra's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Korra agent itself") is fully supported. Korra treats agent
...
pitch in the Tool Gateway announcement is specifically about Korra's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Korra is configured to use Portal as its provider. It is
