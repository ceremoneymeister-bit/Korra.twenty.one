# Themes / Skins — Author a Korra Color Theme

Author a Korra **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`korra_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`$HERMES_HOME/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Korra itself.
...
- Write access to the Korra data home — `$HERMES_HOME`, which resolves to the
  active profile's dir when a profile is in use. Skins live in `$HERMES_HOME/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `korra config set`).
...
3. `write_file` it to `$HERMES_HOME/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `korra config set display.skin default` (or they can `/skin default`).
...
```
korra skin set <key> <hex>      # e.g. korra skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `korra skin set background "#08201f"` changes
only the background; `korra skin use <name>` / `korra skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode the home path** when a profile is active — resolve the real
  home from `$HERMES_HOME` first, falling back to the platform default only when
  it is unset.
...
- **Never hand-edit `config.yaml` to activate.** Use `korra config set
...
- **You apply it, not the user.** `korra config set display.skin <name>` is
...
- `read_file` the written `$HERMES_HOME/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `korra config get display.skin` and confirm it reports `<name>`.
