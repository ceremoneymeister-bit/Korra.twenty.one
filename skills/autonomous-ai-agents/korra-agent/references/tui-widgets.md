...
Author widget apps for the Korra TUI (`korra --tui`): glanceable ambient
...
- The TUI must be in use (`korra --tui`). Widgets do not render in the
...
- A thrown `register()` is logged and skipped; check
  `$HERMES_HOME/logs/tui_gateway_crash.log` if a widget never appears.
