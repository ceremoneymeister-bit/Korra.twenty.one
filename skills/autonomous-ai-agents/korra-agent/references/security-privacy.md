...
Common "why is Korra doing X to my output / tool calls / commands?" toggles — and the exact commands to change them. Most of these need a fresh session (`/reset` in chat, or start a new `korra` invocation) because they're read once at startup.
...
```bash
korra config set security.redact_secrets true       # keep enabled globally
```
...
```bash
korra config set security.redact_secrets false
```
...
```bash
korra config set privacy.redact_pii true    # enable
korra config set privacy.redact_pii false   # disable (default)
```

### Command approval prompts

By default (`approvals.mode: smart`), Korra asks an auxiliary LLM to assess shell commands flagged as destructive (`rm -rf`, `git reset --hard`, etc.). The modes are:
...
```bash
korra config set approvals.mode smart       # recommended middle ground
korra config set approvals.mode off         # bypass everything (not recommended)
```
...
### "Reset permissions" / "make Korra ask again"
...
1. Shell-command allowlist: `korra config set command_allowlist '[]'`
2. Shell-hook consent (only if present): `rm -f $HERMES_HOME/shell-hooks-allowlist.json`

Then sanity-check `korra config get approvals.mode` (should not be `off`)
...
To keep the model away from network or media tools entirely, open `korra tools` and toggle per-platform. Takes effect on next session (`/reset`). See `references/configuration.md` for the toolset list.
