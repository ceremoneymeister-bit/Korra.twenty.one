...
### Tool not available
1. `korra tools` — check if toolset is enabled for your platform
2. Some tools need env vars (check `.env`)
3. `/reset` after enabling tools

### Model/provider issues
1. `korra doctor` — check config and dependencies
2. `korra auth` — re-authenticate OAuth providers (or `korra auth add <provider>`)
3. Check `.env` has the right API key
4. **Copilot 403**: `gh auth login` tokens do NOT work for Copilot API. You must use the Copilot-specific OAuth device code flow via `korra model` → GitHub Copilot.
...
### Skills not showing
1. `korra skills list` — verify installed
2. `korra skills config` — check platform enablement
3. Load explicitly: `korra -s name` (or the skill's own `/<name>` slash command)
...
```bash
korra config set auxiliary.vision.provider <your_provider>
korra config set auxiliary.vision.model <model_name>
```
