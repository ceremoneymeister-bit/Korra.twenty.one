...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``korra_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``korra_bootstrap``, never ``korra_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
# Apply on import — entry points just need ``import korra_bootstrap``
# (or ``from korra_bootstrap import apply_windows_utf8_bootstrap``) at
