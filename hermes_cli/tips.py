...
TIPS = [
    ...
    "Ctrl+Z suspends Korra to the background — run fg in your shell to resume.",
    ...
    "hermes mcp serve runs Korra itself as an MCP server for other agents.",
    ...
    "hermes backup creates a zip backup of your entire Korra home directory.",
    ...
    "hermes gateway install sets up Korra as a system service (systemd/launchd).",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Korra tools programmatically — results stay out of context.",
    ...
    "When exiting, Korra prints a resume command with session ID and stats.",
    ...
    # --- Gateway & Messaging ---
    "Korra runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    ...
    "BlueBubbles brings iMessage to Korra via a local macOS server.",
    ...
    "Korra loads project context from .hermes.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "hermes acp runs Korra as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Korra your own.",
    ...
    "Before a gateway session reset, Korra auto-flushes important facts to memory in the background.",
    ...
    "Profiles scope Korra state via HERMES_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    ...
    '/goal <text> sets a standing Ralph-loop objective — Korra auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Korra config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Korra is working.',
    ...
    'hermes dashboard embeds the full Korra TUI in your browser via xterm.js and a WebSocket PTY.',
    ...
    'hermes import restores a full Korra backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Korra block commands when the tirith scanner itself errors out.',
