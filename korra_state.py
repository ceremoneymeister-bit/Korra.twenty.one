...
# transitive dependency via korra_state_common). run_agent keeps its own
# predating copy — korra_state cannot import run_agent (circular) — guarded
...
from korra_cli.sqlite_runtime import (
    is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable,
)
from typing import Any, Callable, Dict, Iterator, List, Optional, Set, Tuple, TypeVar, cast

from korra_state_common import (  # noqa: F401  (re-exported for back-compat)
    _BRANCH_CHILD_SQL,
...
from korra_state_portability import SessionPortabilityMixin
from korra_state_schema import SessionSchemaMixin
...
def _real_platform_state_root() -> Optional[Path]:
    ...
    Deliberately avoids ``Path.home()`` / ``korra_constants``: tests
    routinely monkeypatch ``Path.home`` to a tempdir, and ``korra_state``
...
# korra_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def _apply_wal_size_limit(conn: sqlite3.Connection) -> None:
    ...
    ``korra_cli/kanban_db.py`` already bounds its WAL growth with
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``korra_cli.kanban_db.connect`` so
...
def _log_journal_mode_upgrade_once(db_label: str, previous_mode: str) -> None:
    ...
    ``korra_cli/managed_uv._default_live_venv``, which treats a database
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see korra_cli/kanban_db.py) would
...
def apply_database_pragmas(
    ...
    * ``synchronous`` — durability level: ``OFF``/``NORMAL``/``FULL``/``EXTRA``
    ...
    """
    try:
        # Local import avoids a circular import with korra_cli.config.
...
def _db_fingerprint(db_path: Path) -> "Optional[str]":
    ...
    RESERVED lock (see ``korra_cli.sqlite_safe_read`` rule 1). This function
    ...
    try:
        ...
        try:
            ...
            # Scaffold/embed installs ship korra_state without korra_cli. No
...
def _backup_db_file(db_path: Path) -> "Tuple[Optional[Path], Optional[str]]":
    ...
    connection's POSIX advisory locks (see ``korra_cli.sqlite_safe_read``).
...
def preflight_db_writability(
    ...
    *,
    ...
    to check). Shared by :class:`SessionDB` and ``korra_cli.kanban_db``.
    """
    raw = str(db_path)
    ...
    try:
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (korra_state.py:645-723). The supported degraded-runtime
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship korra_state without korra_cli). A
    ...
    try:
        ...
        logger.debug(
            "korra_cli.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch korra_state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``korra_cli.backup.is_zeroed_sqlite_file`` when available; this
    local copy keeps SessionDB openable without importing the CLI package
    in constrained embed paths.
    """
    try:
        from korra_cli.backup import is_zeroed_sqlite_file
...
_HERMES_CMDLINE_MARKERS = ("korra_cli.main", "korra_cli/main", "hermes serve",
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            # ``korra_state._set_last_init_error(None)`` explicitly.
    ...
    def _reap_inactive_orphan_desktop_holders(
        ...
        if not sys.platform.startswith("linux") or psutil is None:
            return []
        try:
            from korra_cli.dashboard_procs import _is_ephemeral_port_zero_backend
        except Exception:
            return []
    ...
    # of truth: korra_state_common._RECOVERABLE_END_REASONS, interpolated
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        if repair_alternation and messages:
            # Lazy import: korra_state already depends on agent.* (see
