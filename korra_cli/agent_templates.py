...
_CATALOGUE = {
    ...
    "korra.secretary": {
        "directory": "secretary",
        "owned_paths": {"SOUL.md", "distribution.yaml", "skills/secretarial-work"},
        "name": "Секретарь",
        "description": "Заметки, письма, план дня и встречи в Google Calendar и iCloud.",
        "requirements": [
            "Для начала нужна подключённая чат-модель. Можно сразу принести заметки, письмо или документ.",
            "Встречи читает и записывает через встроенные подключения Google Calendar и iCloud в «Ключах и доступах». Для почты и уведомлений нужны соответствующие подключения.",
            "При создании установятся роль и навыки. Письма, события и расписания агент создаёт по вашему поручению.",
        ],
    },
    "korra.psychologist": {
        "directory": "psychologist",
        "owned_paths": {"SOUL.md", "distribution.yaml", "skills/supportive-reflection"},
        "name": "Психолог",
        "description": "Поддерживающий ИИ для разбора переживаний и самопознания.",
        "requirements": [
            "Для начала нужна подключённая чат-модель. Можно сразу рассказать о ситуации и выбрать разговор или небольшое упражнение.",
            "Помощник для повседневной поддержки взрослых. Не заменяет психолога, врача или экстренную помощь.",
            "Запоминает полезный контекст и замечает повторяющиеся ситуации, проверяя свои предположения с вами. Память можно исправить или удалить; история чатов хранится отдельно.",
        ],
    },
    "korra.chinese-teacher": {
        "directory": "chinese-teacher",
        "owned_paths": {"SOUL.md", "distribution.yaml", "skills/chinese-practice"},
        "name": "Учитель китайского",
        "description": "Китайский для общения: короткие занятия, практика и повторение.",
        "requirements": [
            "Для начала нужна подключённая чат-модель. Цель и уровень уточняются в ходе первого упражнения.",
            "По умолчанию — путунхуа и упрощённые иероглифы; вариант языка можно изменить. Запоминает прогресс, повторяющиеся ошибки и полезные подсказки.",
            "Текстовый режим работает без аудиоподключений. Проверка произношения возможна только при доступном анализе исходного аудио.",
        ],
    },
    "korra.english-teacher": {
        "directory": "english-teacher",
        "owned_paths": {"SOUL.md", "distribution.yaml", "skills/english-practice"},
        "name": "Учитель английского",
        "description": "Английский для поездок, работы и повседневного общения.",
        "requirements": [
            "Для начала нужна подключённая чат-модель. Можно сразу назвать задачу или начать короткий диалог.",
            "Упражнения подстраиваются под ваши ответы. Запоминает цели, типичные ошибки и работающие способы объяснения для следующих занятий.",
            "Оценка уровня служит ориентиром для занятий. Текстовая расшифровка голоса не позволяет оценить произношение.",
        ],
    },
}
