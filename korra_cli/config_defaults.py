...
verbatim from korra_cli/config.py. Must not import from korra_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # all normalized by korra_cli.config.resolve_turn_limit.
        "max_turns": None,
    ...
    "display": {
        ...
        # model — see korra_cli/focus_view.py.
        "focus_view": False,
        ...
        # Сторож: tests/korra_cli/test_korra_canonical_defaults.py
        "language": "ru",
    ...
    "dashboard": {
        ...
        # protocol ping (see korra_cli/web_server.py — an event-loop stall
    ...
    "stt": {
        ...
        "local": {
            ...
            # Скачивать веса в рантайме запрещено (правило владельца
            # 02.09.2026). Без вшитого каталога локальный провайдер честно
            # отказывается и говорит, что делать, вместо того чтобы тянуть
            # полтора гигабайта на первом же голосовом — в опубликованном
            # образе ещё и в каталог только на чтение, то есть с гарантированной
            # ошибкой после долгого ожидания. true снимает запрет осознанно.
    ...
    "security": {
        ...
        # ``korra_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
    ...
    "kanban": {
        ...
        # Who plans on the board from an ordinary chat: "main" (the main
        # agent, the owner's orchestrator), "all" profiles, or "off".
        # ``toolsets: [kanban]`` in a profile's config also opts it in.
        "chat_tools": "main",
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Korra mints short-lived tokens from. Falls back to "
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Korra auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Korra, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Korra API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Korra API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Korra API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
