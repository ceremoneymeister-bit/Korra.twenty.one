...
def remove_wrapper_script():
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains korra_cli reference)
                content = wrapper.read_text(encoding="utf-8")
                if 'korra_cli' in content or 'hermes-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    try:
        from korra_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from korra_cli.gateway import get_launchd_plist_path
...
def remove_hermes_env_vars_windows() -> list[str]:
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            # Оба имени пары: контур мог быть установлен до переименования.
            for name in ("KORRA_HOME", "HERMES_HOME",
                         "KORRA_GIT_BASH_PATH", "HERMES_GIT_BASH_PATH"):
                try:
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    ...
    *windows* is an injectable platform verdict for tests (same pattern as
    ...
    try:
        ...
        from korra_cli._install_repair import _WINDOWS_BIN_LAUNCHERS
...
def _uninstall_profile(profile) -> None:
    ...
    #    Use `python -m korra_cli.main` so we don't depend on a `hermes`
    #    wrapper that may be half-removed mid-uninstall.
    hermes_invocation = [_sys.executable, "-m", "korra_cli.main", "--profile", name]
...
def run_gui_uninstall(args):
    ...
    """
    from korra_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    *,
    ...
    if _is_windows():
        ...
        if removed_path_entries:
            ...
            log_info("No Korra-owned PATH entries in User environment")
        ...
        if removed_env:
            ...
            log_info("No Korra-set User env vars to remove")
    ...
    log_info("Removing Korra-managed node/npm/npx symlinks...")
    ...
    if removed_node_links:
        ...
        log_info("No Korra-managed node/npm/npx symlinks found")
    ...
    try:
        from korra_cli.gui_uninstall import uninstall_gui
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/ceremoneymeister-bit/Korra.twenty.one/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/ceremoneymeister-bit/Korra.twenty.one/main/scripts/install.sh | bash", Colors.DIM))
        print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m korra_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import korra_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``korra_constants`` + ``korra_cli.colors``
    (and lazily ``korra_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m korra_cli.uninstall")
    parser.add_argument(
        "--mode",
