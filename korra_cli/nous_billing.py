...
billing screens drive. Companion to ``korra_cli/nous_account.py`` (which owns
...
# korra_cli.auth.NOUS_BILLING_MANAGE_SCOPE (kept here too so this module has no
...
def resolve_portal_base_url(state: Optional[dict[str, Any]] = None) -> str:
    ...
    """
    env = korra_env("KORRA_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL")
...
def _resolve_token_and_base(*, use_cache: bool = True) -> tuple[str, str]:
    ...
    try:
        from korra_cli.auth import AuthError, resolve_nous_access_token
    except ImportError:
