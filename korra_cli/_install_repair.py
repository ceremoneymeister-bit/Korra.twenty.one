...
- ``korra_cli._early_recovery.recover_if_needed`` — stdlib-only, runs BEFORE
  ``korra_cli.main``'s third-party imports, so it can complete a pending
  update while no native extension is mapped yet (#83569).
- ``korra_cli.main._recover_core_update_marker_locked`` — the historical
...
the corrupted-venv state it exists to repair. ``korra_cli.main`` imports
``managed_uv``, ``korra_constants``, and friends only in its late path; the
...
:func:`korra_cli._early_recovery._find_uv_binary` lookup and falls back to
...
from korra_cli import _early_recovery as _er
...
def _is_termux_env(env: dict | None = None) -> bool:
    """Stdlib Termux probe (korra_cli.main's version lives behind imports)."""
...
def _venv_scripts_dir(root: Path) -> Path | None:
    ...
    # korra_constants is stdlib-only, so the canonical layout helpers are safe
...
def _venv_is_relocatable(venv_dir: Path) -> bool:
    ...
    uv writes the flag; ``korra_cli.managed_uv`` builds its replacement
...
def ensure_windows_bin_launchers(
    ...
    *,
    ...
    here is :func:`korra_constants.get_default_hermes_root`.
    ...
    *windows* and *user_path_entries* are injectable for tests, same pattern
    as ``korra_constants.venv_bin_dir``.
    """
    if windows is None:
        windows = _is_windows()
    if not windows:
    ...
    # every korra_cli.main process start (right after the profile
...
def _read_user_path_raw() -> tuple[list[str], int]:
    ...
    (same discipline as ``korra_cli.uninstall``). Only called on Windows.
    """
    import winreg
...
def migrate_windows_bin_path(
    ...
    *read_user_path*/*write_user_path* are injectable for tests.
    ...
    from korra_constants import get_default_hermes_root, venv_bin_dir
