"""Dashboard process-hygiene helpers — extracted from ``korra_cli/main.py``.
...
helpers that STAY in ``korra_cli.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``korra_cli.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``korra_cli.main`` resolve unchanged.
"""
...
def _m():
    """Lazy ``korra_cli.main`` reference (call-time; keeps patches working)."""
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    try:
        if sys.platform == "win32":
            ...
            # with `korra_cli.gateway._scan_gateway_pids` and avoids the
    ...
    # neither "hermes serve" nor "korra_cli.main serve". Every serve/
    ...
    # truncated view.
    try:
        from korra_cli.process_identity import ledger_entries
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    ...
    if "serve" not in cmd:
        return False
    if "hermes" not in cmd and "korra_cli" not in cmd:
        return False
