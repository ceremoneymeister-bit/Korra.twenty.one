...
Usage:
    python -m korra_cli.main web          # Start on http://127.0.0.1:9119
    python -m korra_cli.main web --port 8080
"""
...
from korra_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
from korra_cli.install_identity import get_install_id as _shared_get_install_id
...
import yaml
from korra_constants import korra_env, korra_env_present, korra_env_set, korra_env_setdefault, korra_env_expand
...
WEB_DIST = Path(korra_env("KORRA_WEB_DIST")) if korra_env_present("KORRA_WEB_DIST") else Path(__file__).parent / "web_dist"
...
def _warm_gateway_module() -> None:
    ...
    ``korra_cli.gateway``. But the first WS connection and its initial
    ...
    for mod in (
        ...
        "korra_cli.auth",
        "korra_cli.copilot_auth",
        "korra_cli.runtime_provider",
...
async def _lifespan(app: "FastAPI"):
    ...
    # Import korra_cli.gateway eagerly *before* the lifespan yield so the
    ...
    if korra_env("KORRA_DESKTOP") == "1":
        ...
        try:
            from korra_cli.gateway import _reap_unsupervised_gateway_orphans
...
from korra_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
def _resolve_session_token() -> str:
    return korra_env("KORRA_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
...
#
# This list is defined in ``korra_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from korra_cli.dashboard_auth.public_paths import (
...
def _desktop_loopback_auth_exempt(
    ...
    if korra_env("KORRA_DESKTOP") != "1":
        return False
    return bool(
        korra_env("KORRA_DASHBOARD_SESSION_TOKEN")
...
async def _dashboard_auth_gate(request: Request, call_next):
    from korra_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from korra_cli.dashboard_auth.token_auth import token_auth_middleware
...
from korra_cli.web_models import (  # noqa: F401
    ConfigUpdate,
    EnvVarUpdate,
    EnvVarDelete,
    EnvVarReveal,
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    from korra_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from korra_cli.model_normalize import normalize_model_for_provider
    from korra_cli.providers import resolve_custom_provider, resolve_user_provider
    ...
    if (
        ...
        try:
            ...
            cur_provider = ""
        from korra_cli.models import _AGGREGATOR_PROVIDERS
...
def _count_status_active_sessions() -> int:
    ...
    """
    from korra_state import _default_db_path
...
_MANAGED_FILES_ROOT_ENV = "KORRA_DASHBOARD_FILES_ROOT"
...
def _default_hermes_root_is_opt_data() -> bool:
    raw = korra_env("KORRA_HOME", "").strip()
    if not raw:
        return False
    try:
...
def _dashboard_local_update_managed_externally() -> bool:
    ...
    if _default_hermes_root_is_opt_data():
        return True
    try:
        from korra_constants import is_container
...
async def _durable_browser_chat_response(
    *,
    ...
    from korra_cli.chat_delivery import (
        DeliveryConflict,
        request_fingerprint,
...
# gate + path hardening as /api/fs). Logic lives in ``korra_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from korra_cli import web_git as _web_git  # noqa: E402
...
from korra_cli.web_routers import git as _git_routes  # noqa: E402

app.include_router(_git_routes.router)
from korra_cli.web_routers.git import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    git_status_route,
    git_worktrees_route,
    git_branches_route,
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``profile_platforms`` — ``{profile: platforms}`` runtime platform maps
    ...
    """
    try:
        from korra_cli.profiles import _check_gateway_running, profiles_to_serve
...
_PROFILE_PLATFORM_STATUS_KEY_RE = re.compile(
    # Profile segment mirrors korra_cli.profiles._PROFILE_ID_RE.  Platform
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        # Windows install the first import of korra_cli.gateway blocks the
        ...
        try:
            from korra_cli.auth import get_nous_session_validity
        ...
        try:
            from korra_state import SessionDB as _SDB
            from korra_constants import get_hermes_home as _ghh
...
def _get_portal_status_sync():
    ...
    auth: Dict[str, Any] = {}
    try:
        from korra_cli.auth import get_nous_auth_status_local
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from korra_cli.debug import build_debug_share
...
def _dashboard_spawn_executable() -> str:
    ...
    runpy.run_module('korra_cli.main', ...)"``) — so ``sys.executable`` is
...
def _spawn_hermes_action(
    ...
    *,
    ...
    Uses the running interpreter's ``korra_cli.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "korra_cli.main", *subcommand]
    ...
    action_env = {**os.environ, **korra_env_expand({"KORRA_NONINTERACTIVE": "1"})}
    ...
    action_id = korra_env("KORRA_ACTION_ID", env=env_overrides or {})
    if action_id:
...
async def update_hermes():
    ...
    try:
        proc = _spawn_hermes_action(
            ["update"],
            "hermes-update",
            env_overrides=korra_env_expand({"KORRA_ACTION_ID": action_id}),
        )
    except Exception as exc:
...
async def check_hermes_update(force: bool = False):
    ...
    try:
        from korra_cli.banner import check_for_updates
...
def _latest_update_receipt_summary() -> Optional[Dict[str, Any]]:
    ...
    """
    try:
        from korra_cli.update_receipt import read_latest_receipt
...
from korra_cli.web_routers import sessions as _sessions_routes  # noqa: E402

app.include_router(_sessions_routes.list_router)
from korra_cli.web_routers.sessions import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_sessions,
)
...
# in korra_cli/config.py — listed here for deterministic ordering in the UI.
...
async def get_model_options(
    ...
    try:
        ...
        from korra_cli.inventory import build_model_options_payload, load_picker_context
...
def get_recommended_default_model(provider: str = ""):
    ...
    try:
        from korra_cli.inventory import build_models_payload, load_picker_context
        from korra_cli.models import pick_silent_default_model
...
def get_moa_models(profile: Optional[str] = None):
    ...
    try:
        from korra_cli.moa_config import normalize_moa_config
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    try:
        from korra_cli.moa_config import normalize_moa_config, validate_moa_payload
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    try:
        ...
        if model and not body.confirm_expensive_model:
            try:
                from korra_cli.model_selection_guards import combined_selection_warning
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (korra_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from korra_cli.nous_subscription import apply_nous_managed_defaults
        ...
        try:
            from korra_cli.profiles import seed_provider_credentials_from_root
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from korra_cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    """
    try:
        from korra_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from korra_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
...
# korra_cli/setup_hidden_env.py. Shared with the `hermes setup gateway`
# wizard so the surfaces ask for the same things.
from korra_cli.setup_hidden_env import (  # noqa: E402
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from korra_cli import auth as hauth
        if provider_id == "nous":
...
async def disconnect_oauth_provider(
    ...
    def _run():
        with _profile_scope(profile):
            ...
            try:
                from korra_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
async def _start_device_code_flow(
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            korra_env("KORRA_PORTAL_BASE_URL")
    ...
    if provider_id == "xai-oauth":
        from korra_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from korra_cli.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from korra_cli.auth import _save_codex_tokens
...
def _open_session_db_at_path(db_path: Path, *, read_only: bool):
    ...
    from korra_state import SessionDB, is_malformed_schema_error
...
async def get_logs(
    ...
    from korra_cli.logs import _read_tail, LOG_FILES
    ...
    try:
        from korra_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
from korra_cli.web_routers import cron as _cron_routes  # noqa: E402

app.include_router(_cron_routes.router)
from korra_cli.web_routers.cron import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_cron_jobs,
    get_cron_job,
    list_cron_job_runs,
    create_cron_job,
...
def _gateway_fire_endpoint(profile: str, home: Path) -> str:
    ...
    try:
        ...
        # yaml.safe_load of config.yaml (tests/korra_cli/
...
#
# Wraps the same config data layer the CLI uses (korra_cli.mcp_config), so
...
from korra_cli.web_routers import mcp as _mcp_routes  # noqa: E402

app.include_router(_mcp_routes.router)
from korra_cli.web_routers.mcp import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_mcp_servers,
    add_mcp_server,
    replace_mcp_servers,
    remove_mcp_server,
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from korra_cli.dashboard_auth.prefix import prefix_from_request, resolve_public_url
...
#
# Wraps the same JSON store the CLI uses (korra_cli.webhook); the webhook
...
async def list_webhooks():
    import korra_cli.webhook as wh
...
async def list_credential_pool():
    ...
    from korra_cli.auth import read_credential_pool
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    from korra_cli.auth import suppress_credential_source
...
from korra_cli.web_routers import skills as _skills_routes  # noqa: E402

app.include_router(_skills_routes.hub_router)
from korra_cli.web_routers.skills import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    install_skill_hub,
    uninstall_skill_hub,
    update_skills_hub,
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    from korra_cli.mcp_config import _save_bearer_auth_token
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    """
    from korra_constants import set_hermes_home_override, reset_hermes_home_override
    from korra_cli.skills_config import get_disabled_skills, save_disabled_skills
...
# «Подключённые сервисы» и лента «Календаря» на дашборде: сводка подключений по агентам
...
app.include_router(_updates_routes.router)
from korra_cli.web_routers.tools import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_toolsets,
    toggle_toolset,
    get_toolset_config,
    get_toolset_models,
...
if sys.platform.startswith("win"):
    try:
        from korra_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from korra_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
from korra_cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from korra_cli.dashboard_auth.audit import AuditEvent, audit_log
        from korra_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    matching what ``korra_cli.main._launch_tui`` does for the CLI path.
    ...
    """
    from korra_cli.main import PROJECT_ROOT, _apply_tui_python_env, _make_tui_argv
    ...
    if profile_dir is not None:
        korra_env_set(env, "KORRA_HOME", str(profile_dir))
    try:
        from korra_cli.config import (
    ...
    korra_env_setdefault(env, "KORRA_TUI_DISABLE_MOUSE", "1")
    korra_env_setdefault(env, "KORRA_TUI_INLINE", "1")
    ...
    korra_env_set(env, "KORRA_TUI_DASHBOARD", "1")
    ...
    if resume:
        ...
        korra_env_set(env, "KORRA_TUI_RESUME", resume)

    if sidecar_url:
        korra_env_set(env, "KORRA_TUI_SIDECAR_URL", sidecar_url)
    ...
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            korra_env_set(env, "KORRA_TUI_GATEWAY_URL", gateway_ws_url)
...
def _resolve_client_ws_host() -> Optional[str]:
    ...
    """
    explicit = korra_env("KORRA_DASHBOARD_WS_HOST", "").strip()
    if explicit:
        return explicit
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from korra_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from korra_cli.console_engine import HermesConsoleEngine
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if raw_resume and env:
        registry_resume = korra_env("KORRA_TUI_RESUME", env=env) or raw_resume
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`korra_cli.dashboard_auth.prefix.normalise_prefix`
...
def _read_dashboard_bubble_chat_flag() -> bool:
    ...
    val = korra_env("KORRA_DASHBOARD_CHAT", "")
    if val:
        return val == "1" or val.lower() == "true"
    env_path = Path(korra_env("KORRA_HOME", "/opt/data")) / ".env"
    if env_path.exists():
        try:
...
def mount_spa(application: FastAPI):
    ...
    _headless = korra_env("KORRA_SERVE_HEADLESS") == "1"
    if _headless:
        _msg = (
    ...
    def _serve_index(prefix: str = ""):
        ...
        _ui_mode_env = korra_env("KORRA_UI_MODE", "").strip().lower()
        ...
        owner_timezone_js = json.dumps(
            korra_env("KORRA_OWNER_TIMEZONE", "").strip()
            or korra_env("KORRA_TIMEZONE", "").strip()
        )
        schedule_timezone_js = json.dumps(
            korra_env("KORRA_TIMEZONE", "").strip() or "UTC"
        )
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as korra_cli.plugins):
    ...
    from korra_cli.plugins import get_bundled_plugins_dir
    ...
    # ``korra_cli.plugins`` resolves plugin install locations. The
    ...
    # ``korra_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("KORRA_ENABLE_PROJECT_PLUGINS"):
...
async def get_dashboard_plugins():
    ...
    def _run():
        ...
        try:
            from korra_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
            enabled_set = _get_enabled_set()
            disabled_set = _get_disabled_set()
        except Exception:
            enabled_set = set()
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from korra_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from korra_cli.plugins_cmd import dashboard_remove_user_plugin
...
from korra_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    """
    target = korra_env("KORRA_DESKTOP_READY_FILE")
    if not target:
        return
...
def _start_parent_death_watchdog() -> None:
    ...
    tracking.
    """
    raw_pid = korra_env("KORRA_PARENT_PID")
    start_marker = korra_env("KORRA_PARENT_START_MARKER")
    nonce = korra_env("KORRA_PARENT_NONCE")
    ...
    try:
        poll = max(0.5, float(korra_env("KORRA_SERVE_WATCHDOG_POLL_S", "2.0")))
    except (TypeError, ValueError):
        poll = 2.0
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            if host in _LOOPBACK_HOST_VALUES:
                _public_url_for_msg = ""
                try:
                    from korra_cli.dashboard_auth.prefix import (
            ...
            try:
                ...
                from korra_cli.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            if korra_env("KORRA_DESKTOP") == "1":
                try:
                    from korra_cli.dashboard_procs import (
            ...
            try:
                from korra_cli.process_identity import reap_orphaned_mcp_helpers
