...
from korra_constants import korra_env, korra_env_set, korra_env_expand, korra_env_pop
...
def _current_profile_cli_args() -> list[str]:
    ...
    from korra_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "korra_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def get_task_name() -> str:
    ...
    """
    _assert_windows()
    # Local import to avoid circular module initialization during korra_cli boot.
    from korra_cli.gateway import _profile_suffix
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - invokes ``python -m korra_cli.main [--profile X] gateway run``
    ...
    # if someone imports korra_constants-based logic during startup.
    ...
    prog_args = [python_exe_path, "-m", "korra_cli.main"]
    if profile_arg:
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "korra_cli.main"]
    if profile_arg:
...
def windowless_gateway_restart_spec(
    ...
    preserves every argument after the interpreter (``-m korra_cli.main
    ...
    from korra_cli.config import get_hermes_home
    from korra_cli.gateway import PROJECT_ROOT
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``python.exe -m korra_cli.main gateway run`` directly — NOT
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    from korra_cli.gateway import find_gateway_pids, _profile_arg
