...
def _can_import_hermes_cli(interpreter: Path) -> bool:
    """Whether *interpreter* can import ``korra_cli.main`` unaided.
...
def _running_interpreter_fallback() -> str:
    ...
    The RUNNING interpreter by definition has ``korra_cli`` importable
...
def resolve_exec_command(project_root: Optional[Path] = None) -> str:
    ...
    """
    from korra_cli.relaunch import resolve_hermes_bin
    ...
    if not _can_import_hermes_cli(Path(interpreter)):
        # The candidate interpreter cannot actually import korra_cli.main
...
def _resolve_hermes_bin_for_desktop_entry(
    ...
    Wraps :func:`korra_cli.relaunch.resolve_hermes_bin` with one
    ...
    if resolve_fn is None:
        from korra_cli.relaunch import resolve_hermes_bin as resolve_fn
    ...
    def _inside_checkout(candidate: str) -> bool:
        ...
        for root in {checkout_root, resolved_root}:
            if root is not None and (path == root or root in path.parents):
                return True
        # The `python -m korra_cli.main` relaunch context surfaces the
...
def _project_root() -> Path:
    """This file lives at ``<checkout>/korra_cli/linux_desktop_entry.py``.
