"""Hermes update pipeline — extracted from ``korra_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``korra_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``korra_cli.main`` reference — so existing call sites and test monkeypatches
that target ``korra_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``korra_cli.main``.
...
Imports are one-way: ``korra_cli.main`` imports this module, never the reverse
...
from korra_constants import get_default_hermes_root, venv_python_path, korra_env, korra_env_set, korra_env_pop

logger = logging.getLogger(__name__)


def _m():
    """Lazy ``korra_cli.main`` reference.

    Lets callers keep patching ``korra_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``korra_cli.main`` -> ``korra_cli.update_cmd`` stays one-way
    at import time.
    """
    from korra_cli import main
...
def _purge_stale_hermes_modules() -> None:
    ...
    ``from korra_cli.gateway import ...`` — executing NEW source inside an
    ...
    field failure: freshly-pulled ``korra_cli.gateway`` does
    ``from korra_cli.cli_output import line_input``, but ``cli_output`` was
...
def _reload_config_modules() -> None:
    ...
    This function force-reloads ``korra_cli.config_defaults``,
    ``korra_cli.config``, and ``korra_cli.config_migrations`` from disk
    so subsequent imports read the UPDATED code.

    It also reloads ``korra_cli._subprocess_compat`` and
    ``korra_cli.dashboard_procs`` so that post-update dashboard cleanup
    ...
    importlib.invalidate_caches()
    for mod_name in (
        "korra_cli.config_defaults",
        "korra_cli.config",
        "korra_cli.config_migrations",
        "korra_cli._subprocess_compat",
...
def _run_config_check_fresh() -> tuple:
    ...
    from korra_cli.config import check_config_version
...
def _run_migrate_config_fresh(*, interactive: bool = False, quiet: bool = False) -> dict:
    ...
    """
    _reload_config_modules()
    from korra_cli.config import migrate_config
...
def _migrate_sibling_profile_configs() -> list[tuple[str, int, int]]:
    ...
    try:
        ...
        from korra_cli.profiles import _get_profiles_root, _PROFILE_ID_RE
...
def _check_and_apply_config_migration(
    *,
    ...
    # it and warn loudly.
    try:
        from korra_cli.backup import restore_cron_jobs_if_emptied
    ...
    try:
        from korra_cli.backup import restore_cron_jobs_all_profiles
...
_UPDATE_CRITICAL_FILES = (
    "korra_cli/main.py",
    "korra_cli/config.py",
    "korra_cli/__init__.py",
    "korra_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _critical_module_import_failures(
    ...
    a later one.
    """
    from korra_constants import FIRST_PARTY_MODULE_ROOTS
    ...
    probe = (
        ...
        # The root set is injected from korra_constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        # Korra: свои модули теперь `korra_*`; старый префикс оставлен, чтобы
        # зонд узнавал сломанное дерево и на переходе (см. is_first_party_module).
        "        if missing in %r or missing.startswith('korra_')"
        " or missing.startswith('hermes_') or %r:\n"
...
def _reload_process_scan_modules() -> None:
    ...
    ``from korra_cli._subprocess_compat import bounded_probe_run``. If the
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``korra_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``korra_constants.py``
...
def _print_update_completion(message: str) -> None:
    ...
    action_id = korra_env("KORRA_ACTION_ID", "")
...
def _restore_state_db_from_snapshot(state_path: Path, snap_state: Path) -> bool:
    ...
    """
    from korra_cli.backup import _foreign_db_holder_pids, verify_sqlite_integrity
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    ...
    if uv_bin:
        ...
        from korra_cli.managed_uv import managed_python_env
    ...
    try:
        from korra_cli.backup import _quick_snapshot_root, verify_sqlite_integrity
    ...
    try:
        from korra_cli.update_receipt import finalize_update_receipt
...
def _classify_concurrent_instance(pid: int) -> str:
    ...
    from korra_cli._scan_venv_blockers import _is_pausable_gateway  # noqa: PLC0415
...
def _capture_active_tool_dependencies() -> list[str]:
    ...
    try:
        from korra_cli import tools_config
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    try:
        from korra_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from korra_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    """
    from korra_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from korra_constants import is_wsl
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    from korra_cli.gitlock import clear_stale_git_locks, clear_stale_tmp_packs
...
def _ensure_acp_launcher() -> None:
    ...
    ``korra_cli._install_repair.ensure_windows_bin_launchers`` re-stages
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    try:
        ...
        try:
            from korra_cli.backup import create_pre_update_snapshots_all_profiles
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    Windows: the Desktop app's backend (``python.exe -m korra_cli.main
    ...
    for proc in proc_iter:
        ...
        # the `-m korra_cli.main gateway run` argv, so autostarted gateways
...
# ``cryptography`` is the canonical case: ``korra_cli.main`` used to import
...
def _hermes_holder_subcommand(cmdline: str) -> str | None:
    ...
    ``korra_cli.main`` / ``hermes(.exe)`` entry token, then returns the
    ...
    for i, token in enumerate(tokens):
        ...
        if low.endswith("korra_cli.main") and i > 0 and tokens[i - 1] == "-m":
            entry_idx = i
            break
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from korra_cli._scan_venv_blockers import _is_pausable_gateway
...
def _refuse_gateway_ancestor_tree_kill(
    ...
    try:
        from korra_cli.gateway import _is_pid_ancestor_of_current_process
...
def _ledger_manual_serve_holders(
    ...
    """
    try:
        from korra_cli.process_identity import ledger_entries, spawner_is_dead
    except Exception:
        return []
...
def _orphaned_desktop_backend_pids(
    ...
    - its cmdline is a Hermes backend (``korra_cli.main`` + ``serve`` /
      ``dashboard``), and
    ...
    def _is_backend(argv_low: str) -> bool:
        return "korra_cli.main" in argv_low and (
...
def _ledger_reapable_backend_pids(
    ...
    (``korra_cli.process_identity``). A holder qualifies when ALL of:
...
def _handoff_reapable_backend_pids(
    ...
    - Only Hermes backends (``korra_cli.main`` + ``serve``/``dashboard``)
...
def _stop_process_trees(
    ...
    from korra_cli._subprocess_compat import pid_is_hermes, windows_hide_flags
...
def _looks_like_desktop_control_plane(cmdline: str) -> bool:
    ...
    """
    if "korra_cli.main" not in (cmdline or "").lower():
        return False
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m korra_cli.main
...
def _warn_incomplete_gateway_fleet_restart(failed_units: list) -> None:
    ...
    from korra_cli.gateway import is_macos
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    when ``korra_cli.gateway`` itself no longer imports, which is one of the
...
def _gateway_recovery_partition(
    ...
    only safe source here: re-importing ``korra_cli.gateway`` in the failing
...
def _recover_gateway_restart_after_abort(
    ...
    command = [
        sys.executable,
        "-m",
        "korra_cli.update_restart_recovery",
        "--stdin",
    ]
    env = os.environ.copy()
    korra_env_set(env, "KORRA_UPDATE_RESTART_RECOVERY", "1")
    for marker in ("_HERMES_GATEWAY", "KORRA_GATEWAY", "KORRA_GATEWAY_MODE"):
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    (e.g. importing ``korra_cli.gateway`` from the freshly pulled checkout)
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        from korra_cli.update_receipt import begin_update_receipt
    ...
    try:
        ...
        # orphan merge-conflict markers in korra_cli/config.py bricked
        ...
        # the old bytecode (e.g. get_hermes_home added to korra_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # symbols in korra_constants or lazy_deps. The dependency install
        ...
        try:
            from korra_cli.macos_tcc_anchor import ensure_tcc_anchor
        ...
        try:
            from korra_cli.profiles import backfill_profile_envs
        ...
        try:
            from korra_cli._install_repair import migrate_windows_bin_path
        ...
        try:
            ...
            try:
                from korra_cli.gateway import _get_restart_exit_wait_budget
            ...
            try:
                from korra_cli.update_receipt import record_gateway_restart
        ...
        try:
            from korra_cli.update_receipt import (
        ...
        try:
            if _pre_update_plan is not None and _pre_update_plan.runtimes:
                ...
                if report_unaccounted_runtimes(_runtime_outcomes):
                    gateway_fleet_restart_incomplete = True
                try:
                    import korra_cli.update_receipt as _ur
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state
      (typically the freshly-pulled ``korra_cli.gateway`` no longer imports,
