...
from korra_cli.archive_safe import (
    archive_root_dirs,
    make_targz,
...
_PROFILE_DIRS = [
    ...
    # See korra_constants.get_subprocess_home().
    "home",
]
...
# (#58394). Add new artifacts here when introduced in ``korra_constants``.
...
def profile_matches_home(name: str, home: "Path | None" = None) -> bool:
    ...
    (:func:`korra_constants.get_hermes_home`).  Used by single-profile
...
def seed_provider_credentials_from_root(
    ...
    try:
        ...
        if home == root:
            return []
        from agent.secret_scope import load_env_file
        from korra_cli.config import get_compatible_custom_providers, save_env_value
        from korra_cli.providers import resolve_provider_full
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    ...
    hermes_markers = ("korra_cli.main", "hermes-gateway", "tui_gateway")
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    try:
        ...
        from korra_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    try:
        ...
        from korra_cli.gateway import get_service_name, get_launchd_plist_path
