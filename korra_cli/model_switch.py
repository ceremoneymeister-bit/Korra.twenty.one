...
- ``korra_cli.providers``        -- canonical provider identity + overlays
- ``korra_cli.model_normalize``  -- per-provider name formatting
...
_HERMES_MODEL_WARNING = (
    ...
    "for use with Korra. They lack the tool-calling capabilities "
...
def _direct_alias_source_identity() -> Optional[tuple]:
    ...
    """
    try:
        from korra_constants import get_config_path
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from korra_cli.model_switch import
...
async def resolve_display_context_length_async(
    ...
    ``korra_cli.route_identity.should_clear_context_pin_async``, which
...
def switch_model(
    ...
    if explicit_provider:
        ...
        from korra_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from korra_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
        ...
        if not new_model:
            if pdef.base_url:
                from korra_cli.runtime_provider import _auto_detect_local_model
    ...
    # =================================================================
        ...
        try:
            from korra_cli.config import load_config
            from korra_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from korra_cli.providers import resolve_user_provider as _ruser
    ...
    # korra_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    # kimi) the same way.
    from korra_cli.models import opencode_provider_family as _oc_family_fn
    if _oc_family_fn(target_provider) is not None and isinstance(base_url, str):
        from korra_cli.models import normalize_opencode_base_url
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from korra_cli.config import normalize_extra_headers
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    """
    from korra_cli.models import cached_provider_model_ids
    ...
    def _fetch_one(slug: str) -> None:
        try:
            ...
            if models:
                from korra_cli.models import update_provider_cache_entry
...
def _collect_authed_provider_slugs(
    ...
    from korra_cli.auth import PROVIDER_REGISTRY, _load_auth_store
    from korra_cli.providers import HERMES_OVERLAYS, ALIASES as _PROVIDER_ALIAS_TABLE
    from korra_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS, CANONICAL_PROVIDERS
    ...
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if pconfig and pconfig.auth_type != "api_key":
            continue
        from korra_cli.auth import is_runtime_provider_routable
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from korra_cli/models.py (OPENROUTER_MODELS,
    ...
    from korra_cli.config import coerce_provider_id, stringify_provider_map
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from korra_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if not has_creds:
            try:
                from korra_cli.auth import _load_auth_store
                store = _load_auth_store()
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(hermes_id)
                )
    ...
    from korra_cli.auth import PROVIDER_REGISTRY as _auth_registry
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                from korra_cli.auth import get_provider_auth_state as _nous_state
    ...
    # with `hermes model`).
    try:
        from korra_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for grp in ep_groups.values():
            ...
            if _probe_live:
                ...
                try:
                    from korra_cli.models import cached_fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # run_agent.py, korra_cli/config.py) already consume that dict.
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from korra_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`korra_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from korra_cli.models import fetch_openrouter_models
