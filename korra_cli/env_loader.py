...
from korra_constants import korra_env, korra_env_set, korra_env_pop, korra_env_present
...
# several hot modules (cli.py, korra_cli/main.py, run_agent.py,
...
def _known_hermes_env_keys() -> set[str]:
    ...
    """
    from korra_cli.config import _EXTRA_ENV_KEYS
    from korra_cli.config_defaults import OPTIONAL_ENV_VARS
...
def _clear_known_keys_missing_from_dotenv(path: Path) -> None:
    ...
    ``tests/korra_cli/test_dump_env_visibility.py``), and a startup scrub
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    ``korra_cli.config._sanitize_env_lines`` normalizes line endings while
    treating content after the first ``=`` as opaque for boundary discovery.
    """
    if not path.exists():
        return
    try:
        from korra_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_hermes_dotenv(
    *,
    ...
    from korra_cli import _early_recovery
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    Delegates to ``korra_cli.config.apply_terminal_config_to_env`` — the
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    time from several hot modules (cli.py, korra_cli/main.py,
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # shared cache here lets main.py's early bridge and korra_logging reuse
