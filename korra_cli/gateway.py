...
# when korra_constants is cached from a pre-update version during `hermes update`.
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "korra_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m korra_cli.main gateway
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    if sys.platform == "win32":
        try:
            from korra_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # base interpreter needs to import korra_cli.  Empty on POSIX, where
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from korra_cli.service_manager import detect_service_manager, get_service_manager
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    in-process under a ``gateway restart`` argv (korra_cli/gateway.py restart
    ...
    if is_windows():
        try:
            ...
            from korra_cli.gateway_windows import get_task_name
...
from korra_constants import is_container, is_termux, is_wsl
...
def _windows_gateway_breakaway_state() -> bool | None:
    ...
    if not is_windows():
        return None
    from korra_cli._subprocess_compat import _WINDOWS_GATEWAY_BREAKAWAY_ENV
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            ...
            # korra_constants cached from before this symbol existed. See
            # _reload_hermes_constants() in korra_cli/managed_uv.py.
            from korra_cli.managed_uv import _reload_hermes_constants
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m korra_cli.main``, so module
...
Group={group_name}
ExecStart={python_path} -m korra_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}ExecStart={python_path} -m korra_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="KORRA_HOME={hermes_home}"
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m korra_cli.main [--profile X] gateway run --replace` argv.
    ...
    """
    cmd = [get_python_path(), "-m", "korra_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    working.
    """
    from korra_cli._subprocess_compat import windows_detach_popen_kwargs
...
def _setup_standard_platform(platform: dict):
    ...
    from korra_cli.setup_hidden_env import is_setup_hidden_env as _is_setup_hidden_env
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``korra_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from korra_cli import setup as _s
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`korra_cli.service_manager` are caught and surfaced as
