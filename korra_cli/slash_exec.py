...
``CommandDef.execute`` (korra_cli/commands.py) names a key in
...
for a fixed context (enforced by tests/korra_cli/test_commands_execute.py).

Import discipline: this module imports nothing heavy at module level and
``korra_cli.commands`` does NOT import this module (the ``execute`` field is
...
def _exec_help(ctx: CommandContext) -> CommandReply:
    ...
    from korra_cli.commands import gateway_help_lines
