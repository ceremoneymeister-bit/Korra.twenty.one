...
# coverage category (korra_cli.nous_account.TOOL_COVERAGE_CATEGORIES). Lets the
...
def _local_stt_backend_available() -> bool:
    ...
    """
    if get_env_value("KORRA_LOCAL_STT_COMMAND"):
        return True
    try:
    ...
    *,
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("KORRA_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("KORRA_LOCAL_STT_COMMAND"))
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from korra_cli.setup import prompt_checklist
    except Exception:
        return set()
