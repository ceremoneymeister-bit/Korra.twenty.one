...
from korra_constants import korra_env, korra_env_present, korra_env_aliases, korra_env_pop, korra_env_set
...
from korra_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``korra_cli/`` — i.e. the git checkout for source
...
def _running_in_container() -> bool:
    """Thin wrapper around ``korra_constants.is_container`` (import-safe)."""
    try:
...
# korra_cli/main.py; lives here so the wording stays consistent and we
...
# Re-export from korra_constants — canonical definition lives there.
from korra_constants import get_hermes_home, get_process_hermes_home  # noqa: F811,E402
...
def ensure_hermes_home():
    ...
    from korra_constants import assert_named_profile_home_live
...
from korra_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``korra_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (korra_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (korra_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (korra_cli/setup.py)
    "known_plugin_toolsets", # written/read by korra_cli/tools_config.py toolset-save flow
    "known_builtin_toolsets",  # ditto — which builtin toolsets a platform's checklist has offered
    "tool_gateway_declined_tools",  # per-tool Tool Gateway offer declines (korra_cli/nous_subscription.py, #92647)
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if floor_refused:
        ...
        # The per-version steps live in korra_cli.config_migrations as a
    ...
    try:
        ...
        from korra_cli.toolset_validation import validate_platform_toolsets
        from korra_cli.toolset_scope import toolset_allowed_for_platform
...
# Back-compat alias — canonical set lives in korra_cli.personality.
from korra_cli.personality import NEUTRAL_PERSONALITY_NAMES as _NEUTRAL_PERSONALITY_NAMES  # noqa: F401


def _prompt_text(value: Any) -> str:
    """Normalize config prompt values from YAML before handing them to AIAgent.

    Delegates to :mod:`korra_cli.personality` — the single owner of
...
def render_personality_prompt(value: Any) -> str:
    ...
    from korra_cli.personality import render_personality_prompt as _render
...
def resolve_ephemeral_system_prompt_from_config(cfg: Optional[Dict[str, Any]]) -> str:
    ...
    Delegates to :mod:`korra_cli.personality` (single owner).
    """
    from korra_cli.personality import resolve_ephemeral_system_prompt
...
_SCHEMA_DEFINED_DICT_KEYS = frozenset({
    ...
    # (korra_cli/plugins_cmd.py, korra_cli/plugin_index.py). Absent from
