...
from korra_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
def normalize_reasoning_effort(effort: Optional[str]) -> Optional[str]:
    ...
    Accepts any level in ``korra_constants.VALID_REASONING_EFFORTS`` plus
    ...
    """
    from korra_constants import VALID_REASONING_EFFORTS
...
class Task:
    ...
    # ``korra_constants.VALID_REASONING_EFFORTS``, or ``"none"`` for thinking
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional link to a first-class Project (korra_cli/projects_db). When set,
...
def _sqlite_connect(path: Path) -> sqlite3.Connection:
    ...
    advisory locks on the database (see ``korra_cli.sqlite_safe_read``).
...
def _backup_corrupt_db(path: Path) -> Optional[Path]:
    ...
    # see korra_cli.sqlite_safe_read), so it must only run once the board has
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
    ...
    with _cross_process_init_lock(path):
        ...
        from korra_state import preflight_db_writability
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See korra_state._WAL_INCOMPAT_MARKERS for detection logic.
                from korra_state import apply_wal_with_fallback
                apply_wal_with_fallback(conn, db_label=f"kanban.db ({path.name})")
...
def _check_file_length_invariant(conn: sqlite3.Connection) -> None:
    ...
    """
    from korra_cli.sqlite_safe_read import file_length_matches_header
...
def create_task(
    ...
    *,
    ...
    if project_id:
        from korra_cli import projects_db as _pdb
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    if not rows:
        return False
    try:
        from korra_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
    ...
    *,
    ...
    def _any_spawnable_review() -> bool:
        if not review_rows:
            return False
        try:
            from korra_cli.profiles import profile_exists as _rpe
        except Exception:
    ...
    if _default_assignee:
        try:
            from korra_cli.profiles import profile_exists as _pe
    ...
    for row in review_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        try:
            from korra_cli.profiles import profile_exists
        except Exception:
            profile_exists = None  # type: ignore[assignment]
...
def _module_hermes_argv() -> list[str]:
    ...
    # ``korra_cli.main`` is the console-script target declared in
...
def _resolve_hermes_argv() -> list[str]:
    ...
    3. ``sys.executable -m korra_cli.main`` — fallback for setups where
    ...
    local (not imported from gateway) because ``korra_cli`` sits below
...
def _default_spawn(
    ...
    *,
    ...
    # _apply_profile_override() runs *before* korra_constants is imported.
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Install Korra or activate its venv before running the kanban dispatcher."
        )
...
def list_profiles_on_disk() -> list[str]:
    ...
    ``korra_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from korra_constants import get_default_hermes_root
