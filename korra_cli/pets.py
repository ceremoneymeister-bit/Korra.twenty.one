...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_list = subs.add_parser("list", help='Открыть галерею petdex')
    p_list.add_argument("query", nargs="?", default="", help='Фильтр по части кода или имени')
    p_list.add_argument("--installed", action="store_true", help='Показать только установленных питомцев')
    p_list.add_argument("--limit", type=int, default=40, help='Максимум строк; 0 — все')
    p_list.set_defaults(func=_cmd_list)

    p_install = subs.add_parser("install", help='Установить питомца из галереи')
    p_install.add_argument("slug", help='Код питомца, например boba')
    p_install.add_argument("--force", action="store_true", help='Загрузить заново, даже если уже установлен')
    p_install.add_argument("--select", action="store_true", help='Сделать текущим питомцем')
    p_install.set_defaults(func=_cmd_install)

    p_select = subs.add_parser("select", help='Выбрать текущего питомца; сохраняет display.pet.*')
    p_select.add_argument("slug", nargs="?", default="", help='Код питомца; без аргумента — меню выбора')
    p_select.set_defaults(func=_cmd_select)

    p_show = subs.add_parser("show", help='Показать анимацию текущего питомца в терминале')
    p_show.add_argument("slug", nargs="?", default="", help='Код питомца; по умолчанию текущий')
    p_show.add_argument("--state", default="", help='Одно состояние: idle, run, review, failed, wave или jump')
    p_show.add_argument("--cycle", action="store_true", help='По очереди показать все состояния')
    p_show.add_argument("--once", action="store_true", help='Проиграть один раз без повторения')
    p_show.add_argument("--mode", default=None, help='Режим изображения: kitty, iterm, sixel, unicode или auto')
    p_show.add_argument("--scale", type=float, default=0, help='Масштаб; 0 — из настроек')
    p_show.set_defaults(func=_cmd_show)

    subs.add_parser("off", help='Скрыть питомца').set_defaults(func=_cmd_off)

    p_scale = subs.add_parser("scale", help='Изменить размер питомца во всех интерфейсах через display.pet.scale')
    p_scale.add_argument("factor", help='Масштаб, например 0.5; ограничен диапазоном 0.1–3.0')
    p_scale.set_defaults(func=_cmd_scale)

    p_remove = subs.add_parser("remove", help='Удалить установленного питомца')
    p_remove.add_argument("slug", help='Код питомца')
    p_remove.set_defaults(func=_cmd_remove)

    subs.add_parser("doctor", help='Проверить настройку питомца и графические возможности терминала').set_defaults(
        func=_cmd_doctor
    )
