...
from korra_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE
from korra_cli.plugin_capabilities import (  # noqa: F401 — re-exported
    CAPABILITY_REGISTRY,
    VALID_CAPABILITY_IDS,
    plugin_capability_granted,
)
from korra_cli.plugin_capabilities import (
...
VALID_HOOKS: Set[str] = {
    ...
    "pre_transcription",
    # Kanban task lifecycle hooks. Fired by korra_cli.kanban_db when a task
    ...
    #   result: korra_cli.kanban_db.DispatchResult (spawned, reclaimed,
...
class PluginManifest:
    ...
    # ``korra_cli.plugin_capabilities.CAPABILITY_REGISTRY``. Declaration is
...
class PluginContext:
    ...
    def platform_actions(self):
        ...
        if self._platform_actions is None:
            from korra_cli.platform_actions import PlatformActions
    ...
    def profile_name(self) -> str:
        ...
        :func:`korra_cli.profiles.get_active_profile_name`, so it works in
    ...
    def register_dashboard_auth_provider(self, provider) -> Optional[PluginRegistration]:
        ...
        ``provider`` must be an instance of
        :class:`korra_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from korra_cli.dashboard_auth import DashboardAuthProvider
        from korra_cli.dashboard_auth.registry import (
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        # Lazy import to avoid circular: korra_cli.main imports plugins indirectly
        from korra_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
...
class PluginManager:
    ...
    def _refresh_secret_sources_after_discovery(self) -> None:
        ...
        try:
            ...
            from korra_cli.env_loader import load_hermes_dotenv, reset_secret_source_cache
        except Exception:
            return
        try:
    ...
    def register_approval_transport(
        ...
        *,
        ...
        from korra_cli.approval_transport import RegisteredApprovalTransport
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        for manifest in winners.values():
            ...
            if lookup_key not in enabled and manifest.name not in enabled:
                continue
            try:
                from korra_cli.agent_plugins import _discover_mcp
        ...
        *,
        ...
        for child in sorted(path.iterdir()):
            ...
            if portable_file.exists() or portable_file.is_symlink():
                try:
                    from korra_cli.agent_plugins import read_agent_plugin_manifest
    ...
    def _load_portable_plugin(
        ...
        lookup_key = manifest.key or manifest.name
        try:
            from korra_cli.agent_plugins import load_agent_plugin
...
# korra_constants.set_hermes_home_override). A process-wide single-slot
...
def _get_pre_tool_call_directive_details(
    ...
    from korra_cli.lifecycle import invoke_hook as invoke_lifecycle_hook
