...
def _detect_supervisor_for_pid(
    ...
    if pid in service_pids:
        try:
            from korra_cli.gateway import is_macos, supports_systemd_services
...
def collect_runtime_inventory() -> UpdatePlan:
    ...
    try:
        from korra_cli.gateway import _get_service_pids
    ...
    try:
        from korra_cli.gateway import find_windows_gateway_services
    ...
    try:
        from korra_cli.gateway import find_profile_gateway_processes
...
def print_update_plan(plan: UpdatePlan) -> None:
    ...
    if not plan.runtimes:
        print("  Running Korra services: none detected — code swap only.")
        return
