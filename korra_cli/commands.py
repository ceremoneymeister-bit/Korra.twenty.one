...
from korra_constants import INDICATOR_STYLES
...
def _personalities_from_cli_config() -> Dict[str, Any]:
    ...
    from korra_cli.personality import available_personalities

    try:
        from korra_cli.config import get_config_path
...
class CommandDef:
    ...
    # Names a key in ``korra_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``korra_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    # Session
    CommandDef("start", 'Обработать запуск платформы без ответного сообщения', "Session",
               gateway_only=True, busy_policy="dispatch", busy_handler="start"),
    CommandDef("new", 'Начать новую беседу с чистой историей', "Session",
               aliases=("reset",), args_hint="[name]",
               busy_policy="interrupt_then_dispatch", busy_handler="new"),
    CommandDef("topic", 'Настроить или проверить темы в личном чате Telegram', "Session",
               gateway_only=True, args_hint="[off|help|session-id]"),
    CommandDef("clear", 'Очистить экран и начать новую беседу', "Session",
               cli_only=True, desktop="terminal"),
    CommandDef("redraw", 'Перерисовать интерфейс, если экран терминала сбился', "Session",
               cli_only=True, desktop="terminal"),
    CommandDef("history", 'Показать историю беседы', "Session",
               cli_only=True, desktop="terminal"),
    CommandDef("save", 'Сохранить беседу в файл; /save без аргументов — справка', "Session",
               args_hint="<json|md|html> [filename] [redact]"),
    CommandDef("retry", 'Повторно отправить последнее сообщение агенту', "Session"),
    CommandDef("prompt", 'Написать запрос в $EDITOR в формате Markdown и отправить', "Session",
               cli_only=True, args_hint="[initial text]", aliases=("compose",)),
    CommandDef("undo", 'Вернуться на N ваших сообщений назад (по умолчанию 1)', "Session",
               args_hint="[N]"),
    CommandDef("title", 'Задать название текущей беседы', "Session",
               args_hint="[name]"),
    CommandDef("handoff", 'Перенести беседу в мессенджер: Telegram, Discord и другие', "Session",
               args_hint="<platform>", cli_only=True, argument_mode="options"),
    CommandDef("branch", 'Создать ответвление беседы для другого варианта решения', "Session",
               aliases=("fork",), args_hint="[name]"),
    CommandDef("worktree", 'Показать, создать или удалить отдельные рабочие копии Git', "Session",
               cli_only=True, args_hint="[new [name]|list|prune [--dry-run]]",
               subcommands=("new", "list", "prune")),
    CommandDef("compress", 'Сжать контекст; here [N] сохраняет последние N ходов, --preview показывает предварительный результат', "Session",
               aliases=("compact",), args_hint="[here [N] | focus topic | --preview|--dry-run]"),
    CommandDef("rollback", 'Показать или восстановить точки сохранения файлов; ваши ручные изменения сохраняются, --all заменяет и их', "Session",
               args_hint="[number] [--all]"),
    CommandDef("snapshot", 'Создать или восстановить снимок настроек и данных Корры', "Session",
               cli_only=True, aliases=("snap",), args_hint="[create|restore <id>|prune]",
               desktop="terminal"),
    CommandDef("export", 'Сохранить профиль с настройками, навыками и темой в архив', "Configuration",
               cli_only=True, args_hint="[profile] [-o output.tar.gz]"),
    CommandDef("import", 'Создать профиль из архива', "Configuration",
    ...
    CommandDef("pause", 'Приостановить всю новую работу; /pause off — продолжить', "Session",
               gateway_only=True, args_hint="[reason | off]",
               busy_policy="dispatch"),
    CommandDef("approve", 'Разрешить ожидающую подтверждения опасную команду', "Session",
               gateway_only=True, args_hint="[session|always]", busy_policy="dispatch",
               desktop="messaging"),
    CommandDef("deny", 'Отклонить опасную команду, при желании указав причину', "Session",
               gateway_only=True, args_hint="[all] [reason]", busy_policy="dispatch",
               desktop="messaging"),
    CommandDef("bg", 'Выполнить запрос в отдельной фоновой беседе', "Session",
               args_hint="<prompt>", busy_policy="dispatch"),
    CommandDef("btw", 'Задать вопрос по текущей беседе, не прерывая работу', "Session",
               args_hint="<question>", busy_policy="dispatch"),
    CommandDef("agents", 'Показать активных агентов и выполняемые задачи', "Session",
               aliases=("tasks",), busy_policy="dispatch"),
    CommandDef("journey", 'Открыть историю обучения',
               "Session", aliases=("learning", "memory-graph"), cli_only=True,
               args_hint="[list|delete <id>|edit <id>]",
               subcommands=("list", "delete", "edit")),
    CommandDef("queue", 'Добавить запрос в очередь на следующий ход, не прерывая работу', "Session",
               aliases=("q",), args_hint="<prompt>",
               busy_policy="dispatch", busy_handler="queue"),
    CommandDef("steer", 'Передать уточнение после следующего вызова инструмента, не прерывая работу', "Session",
               args_hint="<prompt>", busy_policy="dispatch", busy_handler="steer"),
    CommandDef("goal", 'Задать цель, над которой Корра будет работать до результата', "Session",
    ...
    CommandDef("heartbeat", 'Задать повторяющийся запрос для периодов бездействия беседы', "Session",
               aliases=("hb",), args_hint="[every <interval> <prompt> | status | pause | resume | clear]",
               subcommands=("status", "pause", "resume", "clear"),
               busy_policy="dispatch"),
    CommandDef("refine", 'Разобрать беседу и сохранить выводы в память и навыки', "Session",
               args_hint="[focus instructions]"),
    CommandDef("review", 'Поручить независимому агенту проверку обсуждаемого кода, документов или изменений', "Session",
               args_hint="[review instructions]"),
    CommandDef("loop", 'Повторять запрос через заданный интервал в этой беседе', "Session",
               aliases=("proactive",),
    ...
    CommandDef("plan", 'Подготовить план в Markdown в папке планов профиля без выполнения действий', "Session",
               args_hint="[task]"),
    CommandDef("moa", 'Выполнить запрос несколькими моделями по настройке MoA и вернуть вашу модель', "Session",
               args_hint="<prompt>", busy_policy="reject", busy_handler="moa"),
    CommandDef("subgoal", 'Добавить или изменить критерии выполнения текущей цели', "Session",
               args_hint="[text | remove N | clear]", busy_policy="dispatch"),
    CommandDef("status", 'Показать сведения о беседе, модели, токенах и контексте', "Session",
               busy_policy="dispatch"),
    CommandDef("egress", 'Показать состояние прокси исходящих соединений Docker', "Session",
               args_hint="[status]", subcommands=("status",),
               busy_policy="dispatch", busy_handler="egress",
               execute="egress"),
    CommandDef("context", 'Показать заполнение контекста, его состав, статистику сжатия и скорость обработки', "Session",
               aliases=("ctx",), args_hint="[all]", subcommands=("all",),
               busy_policy="dispatch"),
    CommandDef("whoami", 'Показать ваши права на команды: администратор или пользователь', "Info"),
    CommandDef("profile", 'Показать имя и папку текущего профиля', "Info",
               busy_policy="dispatch", execute="profile"),
    CommandDef("sethome", 'Сделать этот чат основным каналом уведомлений', "Session",
               gateway_only=True, aliases=("set-home",), desktop="terminal"),
    CommandDef("resume", 'Продолжить беседу по названию', "Session",
               args_hint="[name]", argument_mode="mixed"),

    # Configuration
    CommandDef("sessions", 'Найти и продолжить прошлую беседу', "Session"),

    # Configuration
    CommandDef("config", 'Показать текущие настройки', "Configuration",
               cli_only=True, desktop="terminal"),
    CommandDef("model", 'Сменить модель в этой беседе; --global — сохранить для новых бесед', "Configuration",
    ...
    CommandDef("personality", 'Выбрать готовый характер общения', "Configuration",
               args_hint="[name]", argument_mode="options"),
    CommandDef("statusbar", 'Показать или скрыть строку состояния контекста и модели', "Configuration",
               cli_only=True, aliases=("sb",), desktop="terminal"),
    CommandDef("battery", 'Показать или скрыть цветной индикатор заряда батареи',
               "Configuration", cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("timestamps", 'Показать или скрыть время [ЧЧ:ММ] у сообщений и в /history', "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status"), aliases=("ts",)),
    CommandDef("diff", 'Показать изменения Git в рабочей папке', "Info",
               args_hint="[staged|all|session] [--stat] [path...]",
               subcommands=("staged", "all", "session")),
    CommandDef("verbose", 'Переключить подробность действий инструментов: off → new → all → verbose',
               "Configuration", cli_only=True,
               gateway_config_gate="display.tool_progress_command",
               busy_policy="dispatch", desktop="terminal"),
    CommandDef("focus", 'Переключить режим фокуса: только ваш запрос и итоговый ответ',
               "Configuration", cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("footer", 'Показать или скрыть сведения о выполнении под ответами в мессенджерах',
               "Configuration", args_hint="[on|off|status]",
               subcommands=("on", "off", "status"), busy_policy="dispatch",
               desktop="terminal"),
    CommandDef("yolo", 'Переключить YOLO: пропускать подтверждения опасных команд',
               "Configuration", busy_policy="dispatch"),
    CommandDef("approvals", 'Показать или задать постоянный режим подтверждения опасных команд',
               "Configuration", args_hint="[manual|smart|off]",
               subcommands=("manual", "smart", "off")),
    CommandDef("reasoning", 'Настроить глубину и показ рассуждений', "Configuration",
               ...
               desktop="advanced"),
    CommandDef("fast", 'Переключить ускоренный режим OpenAI или Anthropic: normal/fast', "Configuration",
               args_hint="[normal|fast|status] [--global]",
               subcommands=("normal", "fast", "status", "on", "off", "--global"),
               desktop="advanced"),
    CommandDef("skin", 'Показать или изменить тему оформления', "Configuration",
               cli_only=True, args_hint="[name]", argument_mode="options"),
    CommandDef("indicator", 'Выбрать вид индикатора работы в терминале', "Configuration",
    ...
    CommandDef("wake", 'Включить или отключить пробуждение по голосовой фразе', "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", 'Выбрать, что делать с сообщениями, пока Корра занята', "Configuration",
    ...
    CommandDef("toolsets", 'Показать доступные наборы инструментов', "Tools & Skills",
               cli_only=True, desktop="terminal"),
    CommandDef("skills", 'Найти, установить, посмотреть или настроить навыки',
               "Tools & Skills", cli_only=True,
               gateway_config_gate="skills.write_approval",
               subcommands=("search", "browse", "inspect", "install", "audit",
                            "pending", "approve", "reject", "diff", "approval"),
               desktop="settings"),
    CommandDef("memory", 'Проверить записи в память или настроить их подтверждение',
               "Tools & Skills",
               args_hint="[pending|approve|reject|approval] [id|on|off]",
               subcommands=("pending", "approve", "reject", "approval")),
    CommandDef("bundles", 'Показать комплекты навыков, вызываемые командой /<name>',
               "Tools & Skills", execute="bundles"),
    CommandDef("pet", 'Выбрать или скрыть питомца: /pet, /pet list, /pet <slug>', "Tools & Skills",
               cli_only=True, args_hint="[toggle|list|scale <n>|<slug>]", subcommands=("toggle", "list", "scale", "off")),
    CommandDef("hatch", 'Создать питомца по описанию',
               "Tools & Skills", cli_only=True, aliases=("generate-pet",), args_hint="[description]"),
    CommandDef("learn", 'Создать полезный навык по вашим материалам: папкам, ссылкам, беседе или заметкам',
               "Tools & Skills", args_hint="<what to learn from>"),
    CommandDef("init", 'Создать или обновить инструкции проекта AGENTS.md по содержимому репозитория',
               "Tools & Skills", args_hint="[notes]"),
    CommandDef("cron", 'Управление задачами по расписанию', "Tools & Skills",
               cli_only=True, args_hint="[subcommand]",
               subcommands=("list", "add", "create", "edit", "pause", "resume", "run", "remove"),
               desktop="terminal"),
    CommandDef("suggestions", 'Просмотреть и принять или отклонить предложенные автоматизации',
               "Tools & Skills", aliases=("suggest",), args_hint="[accept|dismiss N | catalog]",
               subcommands=("accept", "dismiss", "catalog", "clear")),
    CommandDef("blueprint", 'Настроить автоматизацию по шаблону',
               "Tools & Skills", aliases=("bp",), args_hint="[name] [slot=value ...]"),
    CommandDef("curator", 'Обслуживание навыков в фоне: состояние, запуск, закрепление и архив',
               "Tools & Skills", args_hint="[subcommand]",
               subcommands=("status", "run", "pause", "resume", "pin", "unpin", "restore", "list-archived"),
               desktop="advanced"),
    CommandDef("kanban", 'Доска совместной работы профилей: задачи, связи и комментарии',
               "Tools & Skills", args_hint="[subcommand]",
    ...
    CommandDef("reload", 'Перезагрузить переменные .env в текущую беседу', "Tools & Skills",
               cli_only=True, desktop="terminal"),
    CommandDef("reload-mcp", 'Перезагрузить серверы MCP из настроек', "Tools & Skills",
               aliases=("reload_mcp",), desktop="advanced"),
    CommandDef("reload-skills", 'Обновить список установленных и удалённых навыков профиля',
               "Tools & Skills", aliases=("reload_skills",), desktop="advanced"),
    CommandDef("browser", 'Подключить ваш браузер на Chromium через CDP или переключиться в Browser Use', "Tools & Skills",
               cli_only=True, args_hint="[connect|disconnect|status|use]",
               subcommands=("connect", "disconnect", "status", "use")),
    CommandDef("plugins", 'Показать установленные плагины и их состояние',
               "Tools & Skills", cli_only=True, desktop="terminal"),

    # Info
    CommandDef("commands", 'Просмотреть все команды и навыки по страницам', "Info",
               gateway_only=True, args_hint="[page]", busy_policy="dispatch",
               execute="gateway_commands"),
    CommandDef("help", 'Показать команды; /help skills — навыки, /help <текст> — поиск', "Info", busy_policy="dispatch",
               execute="gateway_help", args_hint="[skills|<filter>]"),
    CommandDef("palette", 'Открыть поиск команд; также Ctrl+P', "Info",
    ...
    CommandDef("usage", 'Показать расход токенов и лимиты; reset — применить доступный сброс лимита Codex', "Info",
               args_hint="[reset [--force]]"),
    CommandDef("subscription", 'Посмотреть тариф Nous и изменить его в браузере', "Info",
               cli_only=True, aliases=("upgrade",)),
    CommandDef("topup", 'Показать баланс Nous и открыть управление оплатой', "Info"),
    CommandDef("insights", 'Показать статистику использования', "Info",
               args_hint="[days]", desktop="advanced"),
    CommandDef("platforms", 'Показать состояние шлюза и мессенджеров', "Info",
               cli_only=True, aliases=("gateway",), desktop="terminal"),
    CommandDef("platform", 'Приостановить, возобновить или проверить проблемное подключение мессенджера', "Info",
               gateway_only=True, args_hint="<pause|resume|list> [name]"),
    CommandDef("copy", 'Скопировать последний ответ в буфер обмена', "Info",
               cli_only=True, args_hint="[number]", desktop="terminal"),
    CommandDef("paste", 'Прикрепить изображение из буфера обмена', "Info",
               cli_only=True, desktop="terminal"),
    CommandDef("image", 'Прикрепить изображение с компьютера к следующему запросу', "Info",
               cli_only=True, args_hint="<path>", desktop="terminal"),
    CommandDef("update", 'Обновить Корру до последней версии', "Info",
               busy_policy="dispatch", desktop="terminal"),
    CommandDef("version", 'Показать версию Корры', "Info", aliases=("v",),
               busy_policy="dispatch", execute="version"),
    CommandDef("debug", 'Загрузить диагностический отчёт с журналами и получить ссылки для поддержки', "Info",
               args_hint="[nous|local]"),

    # Exit
    CommandDef("quit", 'Выйти; --delete также удалит историю беседы', "Exit",
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`korra_cli.plugins.PluginContext.register_command`. They behave
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from korra_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
    ...
    def _personality_completions(sub_text: str, sub_lower: str):
        """Yield completions for /personality via korra_cli.personality."""
        try:
            # Single owner: built-ins + user overrides from agent.personalities.
            from cli import load_cli_config
            from korra_cli.personality import (
