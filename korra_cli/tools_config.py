...
from korra_cli.nous_subscription import (
    MANAGED_FEATURE_COVERAGE_CATEGORY,
    NousSubscriptionFeatures,
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from korra_cli.nous_account import format_nous_portal_entitlement_message
from korra_cli.toolset_scope import (
...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    :func:`korra_cli._subprocess_compat.windows_hide_flags`) suppresses it
...
from korra_cli.cli_output import (  # noqa: E402 — late import block
...
CONFIGURABLE_TOOLSETS = [
    ("web", "🔍 Поиск и сбор данных из интернета", "Интернет-поиск и извлечение содержимого страниц"),
    ("browser", "🌐 Автоматизация браузера", "Навигация, нажатия, ввод и прокрутка страниц"),
    ("terminal", "💻 Терминал и процессы", "Команды терминала и управление процессами"),
    ("file", "📁 Работа с файлами", "Чтение, запись, правки и поиск по файлам"),
    ("code_execution", "⚡ Выполнение кода", "Запуск кода в изолированной среде"),
    ...
    ("todo", "📋 Планирование задач", "Ведение списка задач"),
    ("memory", "💾 Память", "Долговременная память между сессиями"),
    ("context_engine", "🧩 Движок контекста", "Инструменты активного движка контекста"),
    ("session_search", "🔎 Поиск по диалогам", "Поиск по прошлым разговорам"),
    ...
    ("computer_use", "🖱️  Управление компьютером", "Фоновое управление рабочим столом macOS, Windows и Linux"),
]
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from korra_cli.auth import _read_xai_oauth_tokens
...
def _get_plugin_toolset_keys() -> set:
    ...
    try:
        from korra_cli.plugins import get_plugin_toolset_keys_nowait
...
from korra_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
def _cua_driver_cmd() -> str:
    ...
    return korra_env("KORRA_CUA_DRIVER_CMD", "").strip() or "cua-driver"
...
def _agent_browser_installed() -> bool:
    ...
    from korra_cli.nous_subscription import _local_browser_runnable
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from korra_cli.nous_subscription import _has_agent_browser
...
# Kept in sync with the dashboard selects (korra_cli/web_server.py
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    """
    try:
        from korra_cli.inventory import (
