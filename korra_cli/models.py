...
from korra_cli.urllib_security import open_credentialed_url, url_origin
...
def _custom_provider_ssl_context(base_url: str):
    ...
    if not base_url:
        return None
    try:
        from korra_cli.config import get_custom_provider_tls_settings
...
def _codex_curated_models() -> list[str]:
    ...
    """
    from korra_cli.codex_models import DEFAULT_CODEX_MODELS, _finalize_codex_models
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # endpoint expects (see korra_cli/model_setup_flows.py).
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from korra_cli.model_catalog import get_default_model_from_cache
...
# ``get_recommended_default_model`` in korra_cli/web_server.py and
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from korra_cli.model_catalog import get_curated_openrouter_models
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from korra_cli.model_catalog import get_curated_nous_models
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from korra_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from korra_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``korra_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from korra_cli.model_search import model_alias_canonical
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from korra_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # ``hermes model`` shows.
            from korra_cli.providers import is_official_openai_host
...
def opencode_zen_free_headers() -> dict:
    ...
    """
    try:
        from korra_cli import __version__ as _v
    except Exception:
        _v = "0"
    return {
        "Authorization": "",
    ...
    *,
        ...
        if ollama_models is None:
            return {
                ...
                "message": (
                    ...
                    "Korra will save the model name, but local Ollama model discovery could not verify it."
                ),
            }
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Korra will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Korra cannot verify the model name."
