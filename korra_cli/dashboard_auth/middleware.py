...
_GATE_PUBLIC_PREFIXES: tuple[str, ...] = (
    ...
    "/favicon.ico",
    "/ds-assets/",
    # Знаки бренда со страницы входа (словесный знак и «21»). Публичны по той
    # же причине, что и шрифты: вход рисуется до сессии, а логотип продукта
...
def _auto_sso_response(request: Request) -> Response | None:
    ...
    from korra_cli.dashboard_auth.cookies import detect_https
    set_sso_attempt_cookie(
...
async def gated_auth_middleware(
    ...
    if session is None:
        ...
        from korra_cli.dashboard_auth.cookies import clear_session_cookies
        from korra_cli.dashboard_auth.prefix import prefix_from_request
