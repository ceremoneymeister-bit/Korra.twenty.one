...
def resolve_hermes_bin() -> Optional[str]:
    ...
    Priority:
      ...
      3. ``None`` → caller should fall back to ``python -m korra_cli.main``.
    ...
    ``hermes.exe`` on PATH or the ``sys.executable -m korra_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
...
def relaunch(
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    ``python -m korra_cli.main``) or a ``.cmd`` batch file, and both
