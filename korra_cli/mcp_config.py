...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from korra_cli.cli_output import prompt as _shared_prompt
...
def cmd_mcp_add(args):
    ...
    # korra_cli/main.py for why the dest is renamed.
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Ошибка: `korra mcp configure` требует интерактивного терминала.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
