...
def _build_full_manifest(
    ...
    """
    from korra_cli.commands import slack_app_manifest
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with Korra in threads and DMs.",
        }
        ...
        features["agent_view"] = {
            "agent_description": "Chat with Korra in Slack Messages.",
        }
        bot_scopes.append("assistant:write")
    ...
    display_information = {
        "name": bot_name[:35],
        "description": (bot_description or "Your Korra agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``korra_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$HERMES_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Korra")
    ...
    """
    name = getattr(args, "name", None) or "Korra"
    description = getattr(args, "description", None) or "Your Korra agent on Slack"
    ...
    if write_target is not None:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Korra app\n"
