...
from korra_cli.sqlite_runtime import SQLiteRuntimeInfo, probe_sqlite_runtime
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print(
            "    Sessions stay protected meanwhile: Korra keeps databases "
...
class _UvResult(str):
    ...
    the call site from the *old*, already-imported ``korra_cli.main`` against
...
def _reload_hermes_constants():
    """Re-execute ``korra_constants`` from disk and return the fresh module.

    ``hermes update`` imports ``korra_constants`` from the OLD checkout,
    ...
    return importlib.reload(korra_constants)
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("korra_cli.main")
    ...
    if holders:
        ...
        return True, f"other Korra processes still hold the venv (PID {pids})"
    return False, ""
