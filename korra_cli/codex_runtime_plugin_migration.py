...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Korra config.")
...
def _translate_one_server(
    ...
    for key in hermes_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            ...
            skipped.append(f"{key} (unknown Korra key)")
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Korra)")
...
def _build_hermes_tools_mcp_entry() -> dict:
    ...
    hermes_home = korra_env("KORRA_HOME") or ""
    if hermes_home and _looks_like_test_tempdir(hermes_home):
        hermes_home = ""
    if hermes_home:
        korra_env_set(env, "KORRA_HOME", hermes_home)
    ...
    korra_env_set(env, "KORRA_QUIET", "1")
    korra_env_set(env, "KORRA_REDACT_SECRETS", korra_env("KORRA_REDACT_SECRETS", "true", env=env))

    out: dict[str, Any] = {
        "command": sys.executable,
        "args": ["-m", "agent.transports.korra_tools_mcp_server"],
    }
    if env:
        out["env"] = env
    ...
    *,
    ...
    if not isinstance(hermes_servers, dict):
        report.errors.append(
            "mcp_servers in Korra config is not a dict; cannot migrate."
        )
        return report
    ...
    # The server itself is agent/transports/korra_tools_mcp_server.py
