...
def _split_line(line: str) -> list[str]:
    try:
        ...
        from korra_cli._subprocess_compat import split_command_line
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("korra_cli.main")
    builder = getattr(module, builder_name)
...
class HermesConsoleEngine:
    ...
    def _register_broad_cli_surface(self) -> None:
        ...
        extracted = {
            ...
            "prompt-size": (
                "korra_cli.subcommands.prompt_size",
            ...
            "insights": (
                "korra_cli.subcommands.insights",
            ...
            "security": (
                "korra_cli.subcommands.security",
            ...
            "import": (
                "korra_cli.subcommands.import_cmd",
        ...
        _register_command_family(
            ...
            root="portal",
            paths=portal_paths,
            summaries=_adder_summaries("korra_cli.portal_cli", "add_parser"),
            handler_factory=lambda fixed: _adder_handler(
                "portal",
                fixed,
                "korra_cli.portal_cli",
                "add_parser",
            ),
        )
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("korra_cli.projects_cmd", "build_parser"),
            mutating=[
                ("create",),
                ("add-folder",),
                ("remove-folder",),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("korra_cli.kanban", "build_parser"),
            mutating=[
                ("init",),
                ("boards", "create"),
                ("boards", "rm"),
...
def _version(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from korra_cli._startup_fast import print_fast_version_info
...
def _config_show(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from korra_cli.config import show_config
...
def _sessions_repair(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        from korra_state import DEFAULT_DB_PATH, _db_opens_cleanly, repair_state_db_schema
...
def _profile_status(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    return _dispatch_extracted_subcommand(
        ...
        fixed=(),
        args=(),
        module_name="korra_cli.subcommands.profile",
...
def _cron_list(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    ns = parser.parse_args(args)
    from korra_cli.cron import cron_list
...
def _cron_status(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from korra_cli.cron import cron_status
