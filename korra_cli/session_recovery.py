...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``korra_cli.sqlite_safe_read`` exists to
    ...
    keeps this path consistent with ``korra_state._backup_db_file``.
    """
    from korra_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
    ...
    try:
        ...
        if before != after:
            raise SessionRecoverySafetyError(
                ...
                "Stop every Korra process using this profile and retry. "
                ...
                "Korra process even after the gateway is stopped. Run the "
...
def _recover_via_lost_and_found(
    *,
    ...
    from korra_cli.session_lost_and_found import (
