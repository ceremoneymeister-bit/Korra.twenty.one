...
``korra_cli.config.migrate_config``. Each step is a function
...
``korra_cli.config`` imports :func:`run_migrations` lazily (inside
``migrate_config``), and every step function here resolves its helpers
(``read_raw_config``, ``_persist_migration``, ``get_env_value``, …) lazily
through the live ``korra_cli.config`` module object at call time via
:func:`_cfg`. There is deliberately NO module-level import of
``korra_cli.config`` here, so no circular import can form — and, just as
importantly, tests that monkeypatch helpers on ``korra_cli.config`` (e.g.
``patch("korra_cli.config.read_raw_config", ...)``) keep working, because
...
def _cfg():
    """Return the live ``korra_cli.config`` module (lazy, cycle-free)."""
...
def _migrate_to_38(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from korra_cli.relay_plugin_cutover import legacy_relay_plugin_keys
