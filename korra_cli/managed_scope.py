...
This is DISTINCT from ``korra_cli.config.is_managed()`` / ``HERMES_MANAGED``,
...
def get_managed_dir() -> Optional[Path]:
    ...
    """
    override = korra_env("KORRA_MANAGED_DIR", "").strip()
    if override:
        p = Path(override)
...
def apply_managed_overlay(config: dict) -> dict:
    ...
    (rather than going through korra_cli.config.load_config) to honor managed
    scope. Mirrors korra_cli.config._load_config_impl's managed merge exactly:
    ...
    try:
        ...
        from korra_cli.config import _deep_merge, _expand_env_vars, _normalize_root_model_keys
