...
This module is imported by ``korra_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no korra_cli.config,
...
``korra_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def _resolved_home() -> str:
    hermes_home = korra_env("KORRA_HOME", "").strip()
    if hermes_home:
        return hermes_home
...
def print_fast_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        from korra_cli.banner import format_banner_version_label
    ...
    try:
        ...
        from korra_cli.config import detect_install_method
    ...
    try:
        from korra_cli.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    ...
    if is_termux and korra_env("KORRA_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
