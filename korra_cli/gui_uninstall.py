...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    if sys.platform == "darwin":
        ...
        # ``uninstall_gui``.
        from korra_cli.linux_desktop_entry import desktop_entry_path
...
def agent_is_installed(hermes_home: Path) -> bool:
    ...
    if (agent_root / "korra_cli").is_dir():
        return True
...
def uninstall_gui(
    ...
    Never touches ``hermes-agent/korra_cli`` (agent source), ``venv/``, or any
    ...
    if sys.platform.startswith("linux"):
        ...
        try:
            from korra_cli.linux_desktop_entry import (
                desktop_entry_path,
