"""Dependency-light venv recovery that runs BEFORE korra_cli.main's imports.

The ``hermes`` console entry point is ``korra_cli.main:main``.  Importing
``korra_cli.main`` pulls in third-party packages at module level (``dotenv``
via ``korra_cli.env_loader``, ``yaml`` via ``korra_cli.config``, ...).  In
...
a corrupted venv.  ``korra_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``korra_cli.main`` to become
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``korra_cli.main``
...
def recover_if_needed(
    ...
    """Repair wiped core packages so ``korra_cli.main`` can import at all.
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    ``korra_cli.main``'s third-party imports, maps nothing yet, so the
    ...
    """
    try:
        from korra_cli import _install_repair as ir
