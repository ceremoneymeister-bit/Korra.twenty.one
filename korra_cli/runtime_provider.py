...
from korra_cli import auth as auth_mod
...
from korra_cli import config as _config_mod
from korra_cli.providers import custom_provider_aliases, custom_provider_slug
from korra_constants import OPENROUTER_BASE_URL, korra_env
from korra_cli.providers import is_official_openai_host


def load_config():
    """Late-bound delegate to :func:`korra_cli.config.load_config`.

    Deliberately NOT a module-level ``from korra_cli.config import
    load_config``: this module is often imported lazily (inside functions),
    so its first import can happen while a test has
    ``korra_cli.config.load_config`` patched — a from-import would then
    ...
    patching ``korra_cli.config.load_config`` OR
    ``korra_cli.runtime_provider.load_config`` behaves as expected.
    """
...
def _config_base_url_trustworthy_for_bare_custom(cfg_base_url: str, cfg_provider: str) -> bool:
    ...
    try:
        from korra_cli.auth import resolve_provider as _resolve_provider
...
def _detect_api_mode_for_url(base_url: str) -> Optional[str]:
    ...
    # host_mandated_api_mode clause in korra_cli/providers.py so the
...
def _copilot_runtime_api_mode(
    ...
    *,
    ...
    try:
        from korra_cli.models import copilot_model_api_mode
...
def _parse_api_mode(raw: Any) -> Optional[str]:
    ...
    if isinstance(raw, str):
        from korra_cli.config import _canonical_api_mode
    ...
    *,
    ...
    if provider == "openai-codex":
        ...
        if effective_model and api_mode != "anthropic_messages":
            try:
                from korra_cli.models import azure_foundry_model_api_mode
        ...
        if opencode_provider_family(provider) is not None:
            ...
            # Refs #16878.
            from korra_cli.models import opencode_model_api_mode
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = _getenv("KORRA_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if requested_norm != "custom" and not requested_norm.startswith("custom:"):
        try:
            ...
            # the built-in. See tests/korra_cli/test_runtime_provider_resolution.py
    ...
    *,
    ...
    if not candidate:
        try:
            ...
            candidate = ""
    if not candidate:
        candidate = korra_env("KORRA_INFERENCE_PROVIDER", "").strip()
    ...
    if provider == "nous":
        ...
        api_key = explicit_api_key or (
            ...
            if _agent_key_is_usable(
                state,
                max(60, env_int("KORRA_NOUS_MIN_KEY_TTL_SECONDS", 1800)),
            )
            else ""
        )
        ...
        if not api_key:
            creds = resolve_nous_runtime_credentials(
                timeout_seconds=float(_getenv("KORRA_NOUS_TIMEOUT_SECONDS", "15")),
            )
            api_key = creds.get("api_key", "")
...
def resolve_runtime_provider(
    *,
    ...
    from korra_cli.config import is_provider_enabled, load_config
    ...
    if pool and pool.has_credentials():
        ...
        if provider == "nous" and entry is not None:
            min_ttl = max(60, env_int("KORRA_NOUS_MIN_KEY_TTL_SECONDS", 1800))
            nous_state = {
    ...
    if provider == "minimax-oauth":
        ...
        if pconfig and pconfig.auth_type == "oauth_minimax":
            from korra_cli.auth import resolve_minimax_oauth_runtime_credentials
