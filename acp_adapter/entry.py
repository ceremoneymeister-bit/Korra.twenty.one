...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="hermes-acp",
        description="Run Korra as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Korra version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Korra provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
...
def _run_check() -> None:
    ...
    print("Korra ACP check OK")


def _run_setup() -> None:
    from korra_cli.main import main as hermes_main
...
def main(argv: list[str] | None = None) -> None:
    ...
    if korra_env("KORRA_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
