...
def build_auth_methods() -> list[Any]:
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Korra using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Korra provider",
            description=(
                "Open Korra's interactive model/provider setup in a terminal. "
                "Use this when Korra has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
