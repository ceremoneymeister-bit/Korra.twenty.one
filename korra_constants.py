...
def get_hermes_home() -> Path:
    ...
    template in ``korra_cli/gateway.py`` and the kanban dispatcher in
    ``korra_cli/kanban_db.py``).  See https://github.com/NousResearch/hermes-agent/issues/18594.
    """
...
# Install tree root (this file lives at <install_root>/korra_constants.py).
...
def get_config_path() -> Path:
    ...
    in 7+ files (skill_utils.py, korra_logging.py, korra_time.py, etc.).
    """
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    across four ``korra_cli`` modules using three different Windows
    ...
    A few sites outside ``korra_cli`` (``tools/code_execution_tool.py``,
    ...
    *windows* lets a caller pass its own platform verdict. Several callers
    ...
    Windows paths on Linux CI (``korra_cli.main._is_windows`` and friends);
...
# `korra_cli.update_cmd`'s post-update probe consumes this same set so the
...
def is_first_party_module(name: str | None) -> bool:
    ...
    if not root:
        return False
    # Korra: свои модули верхнего уровня теперь `korra_*`. Префикс `hermes_`
    # оставлен намеренно — ровно ради случая, для которого эта функция и
    # написана: наполовину обновлённое дерево, где старые `hermes_*` ещё лежат
    # рядом с новыми. Сторонний пакет от этого «своим» не становится: проверка
    # по-прежнему требует подчёркивания, поэтому `hermesx` не совпадает.
    return (
        root in FIRST_PARTY_MODULE_ROOTS
        or root.startswith("korra_")
        or root.startswith("hermes_")
    )
