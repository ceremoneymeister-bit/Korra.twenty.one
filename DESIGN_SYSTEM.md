...
Дополнение после production-прохода 2 сентября: базовый русский интерфейс
переведён на self-hosted Onest 2.001 (кириллица, variable WOFF2, SIL OFL 1.1).
Каталог выбора шрифта больше не обращается к Google Fonts: в нём оставлены
системные стеки, локальный Onest и уже локальный JetBrains Mono. Это согласует
типографику с production CSP `style-src/font-src 'self'` и исключает незаметный
fallback для кириллицы.
