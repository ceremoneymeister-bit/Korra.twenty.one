...
def _discovery_cache_path() -> Optional[Path]:
    ...
    try:
        ...
        # import time (korra_constants itself is stdlib-only, so no cycle).
...
class ToolRegistry:
    ...
    def deregister(self, name: str, *, scope: Optional[str] = None) -> None:
        ...
        Explicit scope selects a profile overlay for MCP lifecycle callbacks
        (upstream ee0e234a2c). Plugins cannot select another profile's scope.
        ...
        with self._lock:
            ...
            if caller_owner is not None and scope is not None and scope != caller_scope:
                raise PermissionError("Plugin cannot deregister tools outside its own profile scope")
            if scope is None:
                scope = caller_scope
            target = (
