...
_LAZY_TARGET_ENV = "KORRA_LAZY_INSTALL_TARGET"
...
def _allow_lazy_installs() -> bool:
    ...
    if korra_env("KORRA_DISABLE_LAZY_INSTALLS") == "1":
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    Mirrors the strategy in ``korra_cli.tools_config._pip_install`` but
...
def ensure(feature: str, *, prompt: bool = True) -> None:
    ...
    if _lazy_install_target() is None:
        try:
            from korra_cli.config import get_managed_system
        ...
        if managed_by:
            raise FeatureUnavailable(
                ...
                f"packages come from {managed_by}, so Korra cannot install "
                f"them at runtime. Add the dependencies for {feature!r} via "
                f"{managed_by} (or run a pip/uv install of Korra instead)."
            )
...
def install_specs(specs: list[str] | tuple[str, ...], *, timeout: int = 300) -> InstallSpecsResult:
    ...
    * **Gated** — honors ``security.allow_lazy_installs`` and refuses to run
    ...
    if not _allow_lazy_installs():
        target = _lazy_install_target()
        if korra_env("KORRA_DISABLE_LAZY_INSTALLS") == "1" and target is None:
            reason = (
