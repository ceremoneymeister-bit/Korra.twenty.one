...
# ``agent.auxiliary_client`` pulls credential_pool → korra_cli.auth → httpx
...
from korra_constants import get_hermes_dir, korra_env
...
def _resolve_download_timeout() -> float:
    env_val = korra_env("KORRA_VISION_DOWNLOAD_TIMEOUT", "").strip()
    if env_val:
        try:
...
def _resolve_vision_cpu_workers() -> int:
    ...
    """
    env_val = korra_env("KORRA_VISION_MAX_CONCURRENCY", "").strip()
    if env_val:
        try:
            parsed = int(env_val)
            if parsed >= 1:
