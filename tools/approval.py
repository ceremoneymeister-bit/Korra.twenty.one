...
from korra_constants import korra_env, korra_env_present, korra_env_set, korra_env_pop
...
def _save_blocked_payload(command: str) -> Optional[str]:
    ...
    try:
        ...
        path.write_text(
            "#!/bin/bash\n"
            "# Auto-saved by Korra: this command exceeded the inline command\n"
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bappdata[\\/](?:local|roaming)[\\/]hermes[^\n]*\.env\b', "access to Korra secrets (Windows path)"),
    # ─────────────────────────────────────────────────────────────────────
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_HERMES_CONFIG_PATH}|{_HERMES_ENV_PATH})', "in-place edit of Korra config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_HERMES_CONFIG_PATH}|{_HERMES_ENV_PATH})', "in-place edit of Korra config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_HERMES_CONFIG_PATH}|{_HERMES_ENV_PATH})', "in-place edit of Korra config/env (perl/ruby)"),
...
def get_plugin_manager():
    ...
    from korra_cli.plugins import discover_plugins, get_plugin_manager as _get_manager
...
def _present_with_selected_transport(
    *,
    ...
    try:
        from agent.redact import redact_sensitive_text
        from korra_cli.approval_transport import ApprovalRequest, invoke_approval_transport
        ...
        if _is_single_query_approval_context():
            if _get_single_query_approval_mode() == "deny":
                ...
                try:
                    ...
                    try:
                        from korra_cli.config import load_config_readonly as _load_cfg
