...
It lives beside ``tools/skills_sync.py`` (NOT under ``korra_cli/``) so the
...
def _check_version(caps: Dict[str, Any]) -> None:
    ...
    if major != WIRE_VERSION:
        raise SyncError(
            f"this server speaks sync version {ver!r}, but this Korra speaks "
            f"{WIRE_VERSION} — update Korra to sync with it"
        )
