...
from korra_constants import display_hermes_home, korra_env

logger = logging.getLogger(__name__)
def get_env_value(name, default=None):
    """Read env values through the live config module.

    Tests may monkeypatch and later restore ``korra_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from korra_cli.config import get_env_value as _get_env_value
    except ImportError:
...
# Base URL now resolved via korra_cli.models.deepinfra_base_url (shared).
...
def _load_tts_config() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.debug("korra_cli.config not available, using default TTS config")
        return {}
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    the shared ``korra_cli.models`` helpers so every DeepInfra surface
    ...
    from korra_cli.models import deepinfra_base_url, deepinfra_model_ids
...
def _text_to_speech_single(
    ...
    *,
    ...
    platform = get_session_env("KORRA_SESSION_PLATFORM", "").lower()
