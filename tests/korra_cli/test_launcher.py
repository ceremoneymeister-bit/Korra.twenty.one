...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./hermes` should use `korra_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "hermes"
    called = []

    fake_main_module = types.ModuleType("korra_cli.main")

    def fake_main():
        called.append("korra_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "korra_cli.main", fake_main_module)
