...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    from korra_cli.auth_commands import auth_add_command
...
def test_auth_add_migrates_display_name_derived_legacy_pool_key(
    ...
    with patch("korra_cli.models.clear_provider_models_cache") as clear_cache:
        auth_add_command(_Args())
...
def test_auth_list_includes_non_registry_configured_provider(
    ...
    from korra_cli.auth_commands import auth_list_command
...
def test_interactive_auth_add_accepts_non_registry_configured_provider(
    ...
    from korra_cli import auth_commands
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "korra_cli.auth._nous_device_code_login",
        lambda **kwargs: {
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("korra_cli.auth._codex_device_code_login", lambda: next(logins))
...
def test_codex_auth_status_reports_pool_only_credential(tmp_path, monkeypatch):
    ...
    from korra_cli.auth import get_codex_auth_status
...
def test_codex_runtime_pool_only_rate_limit_is_not_missing_auth(tmp_path, monkeypatch):
    ...
    from korra_cli.auth import AuthError, CODEX_RATE_LIMITED_CODE, resolve_codex_runtime_credentials
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "korra_cli.auth._xai_oauth_device_code_login",
        lambda **kwargs: {
            "tokens": {
...
def test_auth_remove_reindexes_priorities(tmp_path, monkeypatch):
    ...
    from korra_cli.auth_commands import auth_remove_command
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    ...
    assert "Вы вышли из OpenAI Codex." in out
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from korra_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
def test_seed_from_singletons_respects_hermes_pkce_suppression(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(credentials, "read_hermes_oauth_credentials", lambda: {
        "accessToken": "tok", "refreshToken": "r", "expiresAt": 9999999999000,
    })
    monkeypatch.setattr(credentials, "read_claude_code_credentials", lambda: None)
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    from types import SimpleNamespace
    from korra_cli.auth import is_source_suppressed
    from korra_cli.auth_commands import auth_remove_command

    with patch(
        "korra_cli.copilot_auth.resolve_copilot_token",
        return_value=("ghp_fake", "gh"),
    ), patch(
        "korra_cli.copilot_auth.get_copilot_api_token",
