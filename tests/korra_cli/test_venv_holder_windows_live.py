...
def _detect() -> list[tuple[int, str, str]]:
    from korra_cli.update_cmd import _detect_venv_python_processes
...
class TestDetection:
    def test_detects_hermes_argv_process(self):
        """Baseline: a live process running `-m korra_cli.main serve` with
        cwd under the install root is detected as a venv holder."""
        proc = _spawn(["-m", "korra_cli.main", "serve"])
        try:
            matches = _detect()
            ...
            assert "korra_cli.main" in cmdline
        finally:
            _kill(proc)
    ...
    def test_long_runtime_path_gateway_detected_with_full_argv(self):
        ...
        proc = _spawn([padding, "-m", "korra_cli.main", "gateway", "run"])
        try:
            matches = _detect()
...
class TestClassification:
    def test_pausable_exemption_sees_long_path_gateway(self):
        ...
        from korra_cli.update_cmd import _leftover_pausable_gateway_pids
...
class TestHolderMessage:
    ...
    def test_dashboard_not_labeled_desktop_backend(self):
        from korra_cli.update_cmd import _format_venv_python_holders_message

        proc = _spawn(["-m", "korra_cli.main", "dashboard"])
        try:
            matches = [m for m in _detect() if m[0] == proc.pid]
    ...
    def test_substring_subcommand_not_mislabeled(self):
        ...
        proc = _spawn(["-m", "korra_cli.main", "kanban", "--preserve-cache"])
        try:
            matches = [m for m in _detect() if m[0] == proc.pid]
...
class TestAncestorExclusion:
    ...
    def test_gateway_parent_visible_to_child_scan(self, tmp_path):
        ...
        # `... -m korra_cli.main gateway run`, not a multi-line -c blob).
...
class TestConcurrentGateClassification:
    ...
    def test_live_gateway_process_classified_gateway(self):
        """A real process whose argv carries `-m korra_cli.main gateway run`
        classifies as ``gateway`` via real psutil against the live table."""
        from korra_cli.update_cmd import _classify_concurrent_instance

        proc = _spawn(["-m", "korra_cli.main", "gateway", "run"])
        try:
    ...
    def test_live_non_gateway_processes_keep_the_abort(self):
        ...
        repl = _spawn(["-m", "korra_cli.main"])
        mgmt = _spawn(["-m", "korra_cli.main", "gateway", "status"])
        try:
    ...
    def test_live_filter_drops_only_the_gateway(self):
        ...
        from korra_cli.update_cmd import (
            _filter_non_gateway_concurrent_instances,
        )

        gw = _spawn(["-m", "korra_cli.main", "gateway", "run"])
        backend = _spawn(["-m", "korra_cli.main", "serve", "--port", "8127"])
        dead = _spawn([])
