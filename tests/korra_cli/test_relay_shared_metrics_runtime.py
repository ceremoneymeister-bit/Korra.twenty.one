...
from korra_cli import lifecycle, plugins
from korra_cli.observability import relay_runtime, relay_shared_metrics
...
def test_real_binding_drives_lifecycle_aggregation_export_and_snapshot(
    ...
    from korra_cli.observability.shared_metrics import SharedMetricsStore

    root = tmp_path / "hermes-home" / "telemetry" / "shared_metrics"
    store = SharedMetricsStore(root / "metrics.sqlite3", root / "outbox")
    tomorrow = datetime.now(timezone.utc) + timedelta(days=1)
    monkeypatch.setattr(
        "korra_cli.observability.shared_metrics._utc_now",
        lambda: tomorrow,
    )
...
def test_managed_config_cannot_override_shared_metrics_consent(
    ...
    from korra_cli import config, managed_scope
