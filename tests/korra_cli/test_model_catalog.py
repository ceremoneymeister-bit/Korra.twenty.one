"""Tests for korra_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from korra_cli.model_catalog import _validate_manifest
...
class TestDefaultModelFromCache:
    ...
    def test_shipped_manifest_labels_glm52_default(self, isolated_home):
        ...
        import korra_cli.model_catalog as model_catalog
        from korra_cli.models import PREFERRED_SILENT_DEFAULT_MODEL
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in korra_cli.models."""
    ...
    def test_picker_nous_row_uses_curated_list(self, tmp_path, monkeypatch):
        ...
        import importlib
        from korra_cli import model_catalog
        from korra_cli.models import get_curated_nous_model_ids
        importlib.reload(model_catalog)
        try:
            from korra_cli.model_switch import list_picker_providers
            ...
            with patch.object(
                model_catalog, "_fetch_manifest", return_value=_valid_manifest()
            ), patch("korra_cli.models.check_nous_free_tier", return_value=False), patch(
                "korra_cli.models.union_with_portal_free_recommendations",
...
class TestManifestMatchesInRepoLists:
    ...
    def test_in_repo_lists_match_manifest(self):
        ...
        manifest_path = repo_root / "korra_cli" / "data" / "model-catalog.json"
        ...
        assert self._strip_volatile(actual) == self._strip_volatile(expected), (
            "korra_cli/data/model-catalog.json is out of sync with "
            "_PROVIDER_MODELS['nous'] / OPENROUTER_MODELS. "
            "Run: python scripts/build_model_catalog.py && "
            "git add korra_cli/data/model-catalog.json"
        )
