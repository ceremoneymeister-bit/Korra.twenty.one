...
class TestChatVerboseArg:
    ...
    def test_cmd_chat_forwards_none_when_verbose_is_absent(self, monkeypatch):
        ...
        fake_banner = types.ModuleType("korra_cli.banner")
        ...
        monkeypatch.setitem(sys.modules, "korra_cli.banner", fake_banner)
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    # One driver subprocess parses ALL argvs: korra_cli.main is a very heavy
    # import (previously 11 separate `python -m korra_cli.main` spawns with a
...
class TestChatSubparserInheritedValueFlags:
    ...
    Uses the real `korra_cli._parser.build_top_level_parser()` rather than
