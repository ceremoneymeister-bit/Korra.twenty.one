...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        ...
        from korra_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    ...
    from korra_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "korra_state", mod("korra_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "korra_cli.config",
        mod("korra_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "korra_cli.models",
        mod("korra_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    monkeypatch.setitem(
        sys.modules,
        "korra_cli.tools_config",
        mod("korra_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
