"""Tests for the Kanban DB layer (korra_cli.kanban_db)."""
...
def test_stale_claim_reclaim_event_records_diagnostic_payload(
    ...
    timing context)."""
    import json
    import korra_cli.kanban_db as _kb
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see korra_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    import korra_state as _hermes_state
    monkeypatch.setattr(
    ...
    korra_state._wal_fallback_warned_paths.clear()
    ...
    with _patch("korra_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("ERROR", logger="korra_state"):
            conn = kb.connect()
...
def test_connect_works_when_wal_is_silently_refused(tmp_path, monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        korra_state, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    ...
    with _patch(
        "korra_cli.kanban_db.sqlite3.connect",
...
def test_resolve_hermes_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m korra_cli.main`.
    ...
    import shutil
    import sys
    import korra_cli.kanban_db as kb

    monkeypatch.delenv("HERMES_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_hermes_argv()
    assert argv == [sys.executable, "-m", "korra_cli.main"]
...
def test_resolve_hermes_argv_module_actually_runs():
    ...
    sufficient — if `korra_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m korra_cli.main --version`
    ...
    assert "Korra" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="korra_cli.kanban_db"):
        with kb.connect() as conn:
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    """
    import struct
    from korra_cli.kanban_db import connect
