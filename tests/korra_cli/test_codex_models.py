...
from korra_cli.codex_models import (
...
def test_curated_codex_fallback_excludes_chatgpt_rejected_pro_slugs(monkeypatch):
    ...
    monkeypatch.setattr(
        "korra_cli.codex_models._fetch_models_from_api",
...
def test_picker_never_synthesizes_900k_for_pro_or_unknown_slugs():
    ...
    from korra_cli.codex_models import _finalize_codex_models
...
def test_setup_wizard_codex_import_resolves():
    ...
    # This mirrors the exact import used in korra_cli/setup.py line 873.
    # A prior bug had 'get_codex_models' (wrong) instead of 'get_codex_model_ids'.
    from korra_cli.codex_models import get_codex_model_ids as setup_import
...
def test_model_command_prompts_to_reuse_or_reauthenticate_codex_session(monkeypatch, capsys):
    from korra_cli.main import _model_flow_openai_codex
    ...
    monkeypatch.setattr(
        "korra_cli.auth.get_codex_auth_status",
        lambda: {"logged_in": True, "source": "hermes-auth-store"},
    )
    monkeypatch.setattr(
        "korra_cli.auth.resolve_codex_runtime_credentials",
    ...
    monkeypatch.setattr("korra_cli.auth._login_openai_codex", _fake_login)
    monkeypatch.setattr(
