...
class TestReadChain:
    def test_returns_empty_list_when_unset(self):
        from korra_cli.fallback_cmd import _read_chain
        assert _read_chain({}) == []
...
class TestExtractFallback:
    def test_extracts_from_default_field(self):
        from korra_cli.fallback_cmd import _extract_fallback_from_model_cfg
...
class TestAddCommand:
    def test_add_appends_new_entry(self, isolated_home, capsys):
        ...
        with patch("korra_cli.main.select_provider_and_model", side_effect=fake_picker), \
                patch("korra_cli.main._require_tty"):
            from korra_cli.fallback_cmd import cmd_fallback_add
...
class TestRemoveCommand:
    ...
    def test_remove_selected_entry(self, isolated_home, capsys):
        ...
        with patch("korra_cli.setup._curses_prompt_choice", return_value=1):
            from korra_cli.fallback_cmd import cmd_fallback_remove
...
class TestClearCommand:
    ...
    def test_clear_with_confirmation(self, isolated_home, capsys, monkeypatch):
        ...
        from korra_cli.fallback_cmd import cmd_fallback_clear
...
class TestArgparseWiring:
    ...
    def test_fallback_help_lists_subcommands(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "korra_cli.main", "fallback", "--help"],
            capture_output=True,
            text=True,
            timeout=30,
        )
