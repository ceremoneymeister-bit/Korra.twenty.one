...
from korra_cli import main as cli_main
...
def test_gui_installs_packages_and_launches_desktop_app(tmp_path, monkeypatch):
    ...
    with patch("korra_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("korra_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("korra_cli.main._desktop_build_needed", return_value=True), \
         patch("korra_cli.main._write_desktop_build_stamp"), \
         patch("korra_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("korra_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("korra_cli.main._register_linux_desktop_entry"), \
         patch("korra_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]) as mock_run, \
...
def test_gui_install_env_prepends_managed_node_on_bare_path(tmp_path, monkeypatch):
    ...
    from korra_constants import iter_hermes_node_dirs
    ...
    with patch("korra_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("korra_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("korra_cli.main._desktop_build_needed", return_value=True), \
         patch("korra_cli.main._write_desktop_build_stamp"), \
         patch("korra_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("korra_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("korra_cli.main.subprocess.run", return_value=launch_ok), \
...
def test_gui_does_not_retry_after_packaged_executable_exists(tmp_path, monkeypatch, capsys):
    ...
    with patch("korra_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("korra_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("korra_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("korra_cli.main._purge_electron_build_cache", return_value=[Path("/c/electron.zip")]) as mock_purge, \
         patch("korra_cli.main._redownload_electron_dist", return_value=True) as mock_dl, \
         patch("korra_cli.main.subprocess.run", return_value=pack_fail) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_setup_tcc_identity_creates_cert_imports_trusts_and_configures(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("korra_cli.config.set_config_value", lambda key, value: None)
...
def test_cmd_gui_setup_tcc_identity_exits_before_build(tmp_path, monkeypatch):
    ...
    with patch("korra_cli.main._desktop_macos_setup_tcc_identity", return_value=True) as mock_setup, \
         patch("korra_cli.main._run_npm_install_deterministic") as mock_install, \
         pytest.raises(SystemExit) as exc:
...
def test_gui_registers_linux_desktop_entry_before_launch(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("korra_cli.linux_desktop_entry.is_supported", lambda: True)
    monkeypatch.setattr(
        "korra_cli.linux_desktop_entry.install_desktop_entry",
        lambda project_root: registered.append(project_root) or (tmp_path / "hermes.desktop"),
    )

    launch_ok = subprocess.CompletedProcess([str(packaged_exe)], 0)

    with patch("korra_cli.main._desktop_build_needed", return_value=False), \
         patch("korra_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("korra_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("korra_cli.main.subprocess.run", return_value=launch_ok), \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
...
def test_gui_launches_even_when_desktop_entry_install_fails(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("korra_cli.linux_desktop_entry.is_supported", lambda: True)
    monkeypatch.setattr("korra_cli.linux_desktop_entry.install_desktop_entry", boom)
    ...
    with patch("korra_cli.main._desktop_build_needed", return_value=False), \
         patch("korra_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("korra_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("korra_cli.main.subprocess.run", return_value=launch_ok) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_skips_desktop_entry_off_linux(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("korra_cli.linux_desktop_entry.is_supported", lambda: False)

    def fail(_project_root):
        raise AssertionError("must not install a desktop entry off Linux")

    monkeypatch.setattr("korra_cli.linux_desktop_entry.install_desktop_entry", fail)
...
def test_gui_bridges_ozone_hint_to_launch_env(tmp_path, monkeypatch):
    ...
    with patch("korra_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("korra_cli.main._run_npm_install_deterministic", return_value=ok), \
         patch("korra_cli.main._desktop_build_needed", return_value=True), \
         patch("korra_cli.main._write_desktop_build_stamp"), \
         patch("korra_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("korra_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("korra_cli.config.load_config", return_value=cfg), \
         patch("korra_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("korra_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
    ...
    with patch("korra_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("korra_cli.config.load_config", return_value=cfg), \
         patch("korra_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("korra_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run2, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
...
def test_detect_linux_password_store_via_dbus_secret_service(monkeypatch):
    ...
    with patch("korra_cli.main.subprocess.run", return_value=ping_ok) as mock_run:
        assert cli_main._detect_linux_password_store() == "gnome-libsecret"
...
def test_detect_linux_password_store_none_when_no_keychain(monkeypatch):
    ...
    with patch("korra_cli.main.subprocess.run", return_value=ping_fail):
        assert cli_main._detect_linux_password_store() is None
    with patch("korra_cli.main.subprocess.run", side_effect=FileNotFoundError):
        assert cli_main._detect_linux_password_store() is None
...
def test_gui_linux_packaged_launch_bridges_detected_password_store(tmp_path, monkeypatch):
    ...
    with patch("korra_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("korra_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("korra_cli.main._detect_linux_password_store", return_value="gnome-libsecret"), \
         patch("korra_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
...
def test_gui_linux_source_launch_bridges_detected_password_store(tmp_path, monkeypatch):
    ...
    with patch("korra_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("korra_cli.config.load_config", return_value={}), \
         patch("korra_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("korra_cli.main._detect_linux_password_store", return_value="kwallet6"), \
         patch("korra_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
         pytest.raises(SystemExit):
...
def test_gui_config_password_store_skips_detection(tmp_path, monkeypatch):
    ...
    with patch("korra_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("korra_cli.config.load_config", return_value=cfg), \
         patch("korra_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("korra_cli.main._detect_linux_password_store") as mock_detect, \
         patch("korra_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
