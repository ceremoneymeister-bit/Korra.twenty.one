...
(e.g. orphan merge-conflict markers in ``korra_cli/config.py``), the CLI
...
markers in ``korra_cli/config.py``; users who ran ``hermes update`` in
...
def test_validate_critical_files_syntax_tolerates_missing_files(tmp_path):
    ...
    # Populate everything except korra_constants.py
    for relpath in hermes_main._UPDATE_CRITICAL_FILES:
        if relpath == "korra_constants.py":
            continue
