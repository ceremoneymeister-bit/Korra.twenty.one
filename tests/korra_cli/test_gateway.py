...
def test_s6_runtime_snapshot_reports_supervised_service(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("korra_constants.is_container", lambda: True)
    monkeypatch.setattr("korra_cli.service_manager.detect_service_manager", lambda: "s6")
    monkeypatch.setattr("korra_cli.service_manager.get_service_manager", lambda: FakeS6Manager())
...
class TestWaitForGatewayExit:
    ...
    def test_kill_gateway_processes_force_uses_helper(self, monkeypatch):
        ...
        monkeypatch.setattr(
            gateway, "_capture_gateway_argv", lambda pid: ["python", "-m", "korra_cli.main", "gateway", "run"]
        )
        monkeypatch.setattr(
            gateway,
            "terminate_pid",
    ...
    def test_kill_gateway_processes_force_refuses_recycled_pid(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda pid: None if pid == 11 else ["python", "-m", "korra_cli.main", "gateway", "run"],
        )
        monkeypatch.setattr(
            gateway,
            "terminate_pid",
...
def test_module_has_logger():
    ...
    assert gateway.logger.name == "korra_cli.gateway"
...
class TestWindowsScheduledTaskSupervisorGuard:
    ...
    def test_running_task_skips_reap(self, monkeypatch):
        ...
        import korra_cli.gateway_windows as gateway_windows
