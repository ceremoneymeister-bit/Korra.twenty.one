"""Tests for Qwen OAuth provider authentication (korra_cli/auth.py).
...
def qwen_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "korra_cli.auth._qwen_cli_auth_path", lambda: creds_path
    )
    return tmp_path
...
def test_get_qwen_auth_status_refreshes_expired_token(qwen_env):
    ...
    with patch(
        "korra_cli.auth._refresh_qwen_cli_tokens", return_value=refreshed
    ) as mock_refresh:
        status = get_qwen_auth_status()
...
def test_model_flow_qwen_oauth_stale_token_shows_reauth_guidance(qwen_env, monkeypatch, capsys):
    from korra_cli.main import _model_flow_qwen_oauth
    ...
    monkeypatch.setattr(
        "korra_cli.auth._refresh_qwen_cli_tokens",
