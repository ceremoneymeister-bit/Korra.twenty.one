...
from korra_cli import linux_desktop_entry as lde
...
def test_install_writes_entry_with_absolute_exec_and_icon(
    ...
    monkeypatch.setattr(
        "korra_cli.relaunch.resolve_hermes_bin", lambda: str(hermes_bin)
    )
...
def test_installed_entry_is_executable(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    monkeypatch.setattr(
        "korra_cli.relaunch.resolve_hermes_bin", lambda: "/usr/bin/hermes"
    )
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
...
def test_exec_falls_back_to_interpreter_module(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    monkeypatch.setattr("korra_cli.relaunch.resolve_hermes_bin", lambda: None)
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])

    entry = lde.install_desktop_entry(root)
    exec_line = _parse(entry.read_text(encoding="utf-8"))["Exec"]

    assert exec_line.endswith("-m korra_cli.main desktop")
...
def test_exec_never_persists_a_bare_interpreter_command(
    ...
    """The `python -m korra_cli.main` relaunch context must not write
...
def test_exec_keeps_resolver_fallback_when_no_wrapper_on_path(
    ...
    `sys.executable -m korra_cli.main desktop` fallback. Persisting the
    ...
    monkeypatch.setattr("korra_cli.relaunch.resolve_hermes_bin", fake_resolve)
...
def test_exec_arg_quoting_handles_spaces(tmp_path, xdg_home, monkeypatch):
    ...
    monkeypatch.setattr("korra_cli.relaunch.resolve_hermes_bin", lambda: str(spaced))
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
