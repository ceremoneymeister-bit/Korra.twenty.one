...
from korra_cli.main import cmd_update, PROJECT_ROOT
...
def _patch_managed_uv(request):
    ...
    with patch("korra_cli.managed_uv.resolve_uv", side_effect=_fake_resolve_uv), \
         patch("korra_cli.managed_uv.ensure_uv", side_effect=_fake_ensure_uv), \
         patch("korra_cli.managed_uv.update_managed_uv", side_effect=_fake_update_managed_uv):
        yield
...
def _patch_gateway_discovery():
    ...
    """
    with patch("korra_cli.main._purge_stale_hermes_modules", return_value=None), \
         patch("korra_cli.gateway.find_gateway_pids", return_value=[]), \
         patch("korra_cli.gateway.supports_systemd_services", return_value=False), \
         patch("korra_cli.gateway.find_profile_gateway_processes", return_value=[]):
        yield
...
class TestCmdUpdateNpmLockfileCache:
    ...
    def test_record_npm_lockfile_hash(self, tmp_path, monkeypatch):
        from korra_cli import main as hm
    ...
    def test_update_uses_one_shared_npm_cache_across_profiles(
        ...
        monkeypatch.setattr(korra_constants.Path, "home", lambda: tmp_path)
        monkeypatch.setattr(
            korra_constants, "find_node_executable", lambda _name: "/usr/bin/npm"
        )
...
class TestCmdUpdateTermuxUvBootstrap:
    ...
    def test_termux_reuses_existing_path_uv_without_pip(self, mock_run, monkeypatch):
        ...
        monkeypatch.setattr("korra_cli.managed_uv.resolve_uv", lambda: None)
...
class TestCmdUpdateBranchFallback:
    ...
    def test_fork_upstream_sync_that_moves_head_runs_post_update_steps(
        ...
        with patch.object(
            ...
            "korra_cli.update_receipt.collect_fleet_versions",
            return_value=[],
        ), patch(
            ...
            "korra_cli.gateway.find_gateway_pids",
            return_value=[],
        ), patch(
            "korra_cli.gateway.find_profile_gateway_processes",
            return_value=[],
        ), patch(
            "korra_cli.gateway._get_service_pids",
            return_value=set(),
        ), patch.object(
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "korra_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "korra_cli.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch(
            "korra_cli.update_cmd._reload_config_modules"
        ), patch(
            "korra_cli.update_cmd._run_config_check_fresh", return_value=(1, 2)
        ), patch(
            "korra_cli.update_cmd._run_migrate_config_fresh",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ) as migrate_config, patch("korra_cli.main.sys") as mock_sys:
...
class TestCmdUpdateMigrationPrompt:
    ...
    def test_version_bump_only_applies_silently_without_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "korra_cli.config.get_missing_env_vars", return_value=[]
        ), patch(
            "korra_cli.config.get_missing_config_fields", return_value=[]
        ), patch(
            "korra_cli.update_cmd._reload_config_modules"
        ), patch(
            "korra_cli.update_cmd._run_config_check_fresh", return_value=(5, 24)
        ), patch(
            "korra_cli.update_cmd._run_migrate_config_fresh",
    ...
    def test_version_bump_only_surfaces_migration_resets(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "korra_cli.update_cmd._run_config_check_fresh", return_value=(33, 34)
        ), patch(
            "korra_cli.update_cmd._run_migrate_config_fresh",
            return_value={
                "env_added": [],
    ...
    def test_new_options_are_listed_by_name_before_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "korra_cli.config.get_missing_env_vars", return_value=env_items
        ), patch(
            "korra_cli.config.get_missing_config_fields", return_value=cfg_items
        ), patch(
            "korra_cli.update_cmd._reload_config_modules"
        ), patch(
            "korra_cli.update_cmd._run_config_check_fresh", return_value=(1, 24)
        ), patch(
            "korra_cli.update_cmd._run_migrate_config_fresh",
            return_value={"env_added": [], "config_added": [], "warnings": []},
        ), patch("korra_cli.main.sys") as mock_sys:
...
class TestConfigVersionCheckUsesFreshModules:
    ...
    returned the OLD cached ``korra_cli.config`` module — so
    ...
    def test_run_config_check_fresh_reloads_modules(self):
        ...
        sys.modules held the OLD korra_cli.config with the OLD
        DEFAULT_CONFIG["_config_version"] after git pull.
        """
        from unittest.mock import patch

        import korra_cli.update_cmd as update_cmd
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        with (
            patch("korra_cli.profiles.list_profiles", return_value=all_profiles),
            patch("korra_cli.profiles.seed_profile_skills", side_effect=fake_seed),
    ...
    def test_single_profile_default_is_synced(
        ...
        with (
            patch("korra_cli.profiles.list_profiles", return_value=[default_p]),
            patch("korra_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)
...
class TestCmdUpdateCheckBranchFlag:
    ...
    @patch("korra_cli.config.detect_install_method", return_value="git")
    @patch("subprocess.run")
...
class TestCmdUpdateZipBranchRefusal:
    ...
    def test_zip_fallback_refuses_non_main_branch(self, capsys):
        from korra_cli.main import _update_via_zip
...
class TestNodeRuntimeNpmResolution:
    ...
    def test_wsl_update_skips_windows_npm_build_paths(self, mock_args, monkeypatch):
        ...
        monkeypatch.setattr(korra_constants, "is_wsl", lambda: True)
        monkeypatch.setattr(
            korra_constants,
    ...
    def test_update_rebuilds_desktop_that_disappears_mid_update(self):
        ...
        desktop_build.assert_called_once_with(
            [hm.sys.executable, "-m", "korra_cli.main", "desktop", "--build-only"],
            cwd=PROJECT_ROOT,
            env=ANY,
        )
    ...
    def test_git_failure_zip_fallback_rebuilds_missing_desktop(self, tmp_path, monkeypatch):
        ...
        with (
            ...
            patch("korra_cli.managed_uv.ensure_uv", return_value="uv"),
            patch("korra_cli.managed_uv.update_managed_uv"),
            patch(
            ...
            patch("korra_cli.model_catalog.seed_cache_from_checkout", return_value=False),
        ):
...
class TestGitTrampolineSelfHeal:
    ...
    def test_healthy_git_command_unchanged(self):
        ...
        with (
            ...
            patch(
                "korra_cli.update_cmd.subprocess.run",
                side_effect=self._fake_run_healthy,
            ),
            patch("korra_cli.update_cmd._locate_real_git") as locate,
        ):
    ...
    def test_trampoline_swaps_to_real_git(self, capsys):
        ...
        with (
            ...
            patch(
                "korra_cli.update_cmd._locate_real_git", return_value=real
            ),
        ):
            result = update_cmd._ensure_non_trampoline_git(git_cmd)
    ...
    def test_trampoline_no_real_git_keeps_command(self, capsys):
        ...
        with (
            ...
            patch("korra_cli.update_cmd._locate_real_git", return_value=None),
        ):
            result = update_cmd._ensure_non_trampoline_git(git_cmd)
        assert result == git_cmd
        out = capsys.readouterr().out
    ...
    def test_off_windows_noop(self):
        ...
        with (
            ...
            patch("korra_cli.update_cmd.subprocess.run") as run,
        ):
            result = update_cmd._ensure_non_trampoline_git(git_cmd)
        assert result == git_cmd
