...
class TestIsWsl:
    ...
    def test_detects_wsl2(self):
        ...
        with patch("builtins.open", mock_open(read_data=fake_content)):
            assert korra_constants.is_wsl() is True


    def test_no_proc_version(self):
        with patch("builtins.open", side_effect=FileNotFoundError):
            assert korra_constants.is_wsl() is False
...
class TestGatewayCommandWSLMessages:
    ...
    def test_install_wsl_no_systemd(self, monkeypatch, capsys):
        ...
        assert 'Обнаружен WSL' in out
        assert 'systemd не работает' in out
        assert "korra gateway run" in out
        assert "tmux" in out
    ...
    def test_status_wsl_running_manual(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert 'Примечание для WSL' in out
        assert 'tmux или screen' in out
