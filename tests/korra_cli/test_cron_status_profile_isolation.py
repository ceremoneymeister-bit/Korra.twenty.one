...
class TestCronStatusHeartbeatGuard:
    ...
    def test_no_heartbeat_triggers_yellow_warning(self, monkeypatch, capsys):
        ...
        with (
            patch("korra_cli.gateway.find_gateway_pids", return_value={789}),
    ...
    def test_fresh_heartbeat_shows_green_checkmark(self, monkeypatch, capsys):
        ...
        with (
            patch("korra_cli.gateway.find_gateway_pids", return_value={999}),
...
class TestGetServicePidsProfileScope:
    ...
    def test_default_scope_filters_current_profile_systemd_unit(self, monkeypatch):
        from korra_cli import gateway as gateway_mod
        ...
        with (
            ...
            patch("korra_cli.gateway.get_service_name", return_value="hermes-gateway-jarvis"),
    ...
    def test_all_profiles_true_enumerates_fleet(self, monkeypatch):
        ...
        with (
            patch("korra_cli.gateway.is_macos", return_value=False),
            patch("korra_cli.gateway.supports_systemd_services", return_value=True),
            patch("korra_cli.gateway.get_service_name", return_value="hermes-gateway"),
            patch("subprocess.run", side_effect=_run_side_effect),
        ):
...
class TestCronStatusMissingHeartbeat:
    ...
    def test_missing_heartbeat_warns_when_gateway_old(self, tmp_cron_dir, capsys, monkeypatch):
        ...
        import korra_cli.cron as cron_cli
        ...
        out = io.StringIO()
        with (
            patch("korra_cli.cron._active_cron_provider_name", return_value="builtin"),
            patch("korra_cli.gateway.find_gateway_pids", return_value=[4242]),
