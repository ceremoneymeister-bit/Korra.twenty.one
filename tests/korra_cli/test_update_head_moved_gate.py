...
def _patch_update_deps(monkeypatch, tmp_path, run_side_effect):
    """Patch the korra_cli.main helpers ``_cmd_update_impl`` touches.

    ``_m()`` in update_cmd.py lazily returns korra_cli.main, so patching
    attributes on that module is the canonical test surface (matches
    tests/korra_cli/test_cmd_update.py).
    """
    ...
    import korra_cli.gateway as hermes_gateway
