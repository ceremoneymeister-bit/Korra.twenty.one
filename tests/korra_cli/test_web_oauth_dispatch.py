"""Regression tests for the OAuth dispatcher in korra_cli.web_server.
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        "korra_cli.auth._minimax_request_user_code",
        return_value=fake_user_code_resp,
    ), patch(
        "korra_cli.auth._minimax_pkce_pair",
        return_value=("verifier-stub", "challenge-stub", "stub-state"),
    ), patch(
        "korra_cli.web_server._minimax_poller",
        return_value=None,
    ):
        resp = client.post(
...
def test_status_falls_through_to_generic_dispatcher_for_catalog_only_provider():
    ...
    logged-out; now they dispatch to korra_cli.auth.get_auth_status (the
    ...
    with patch("korra_cli.auth.get_auth_status", return_value=fake_status):
