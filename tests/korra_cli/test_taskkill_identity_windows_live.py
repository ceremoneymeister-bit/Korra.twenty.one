...
- ``gateway.status.terminate_pid(force=True)`` requires a matching
  ...
  mismatched identity.
- ``korra_cli._subprocess_compat.pid_is_hermes`` fails closed on foreign
  processes and identity mismatches.
- ``korra_cli.update_cmd._refuse_gateway_ancestor_tree_kill`` refuses to
...
class TestPidIsHermesLive:
    def test_foreign_real_process_is_refused(self):
        ...
        from korra_cli._subprocess_compat import pid_is_hermes
...
class TestAncestorRefusalLive:
    def test_real_parent_chain_is_refused(self, capsys):
        ...
        from korra_cli.update_cmd import _refuse_gateway_ancestor_tree_kill
