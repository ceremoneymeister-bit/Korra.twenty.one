...
def test_free_prints_catalog_and_deep_links_with_plan(cli, monkeypatch, capsys):
    ...
    assert "Выберите тариф" in out
    assert "Plus · $20/мес. · баланс: $22/мес." in out
    assert "Ultra · $200/мес. · баланс: $220/мес." in out
    ...
    assert "подключения Plus" in out
...
def test_upgrade_confirm_names_the_subscription_card(cli, monkeypatch, capsys):
    ...
    assert "Visa ····4242 — с карты вашей подписки будет списана указанная сумма." in out
