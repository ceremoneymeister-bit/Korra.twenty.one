...
from korra_cli import nous_subscription as ns
...
def _capture_checklist(monkeypatch, *, selected_idx):
    ...
    import korra_cli.setup as setup_mod

    monkeypatch.setattr(setup_mod, "prompt_checklist", _fake_checklist, raising=False)
    monkeypatch.setattr(
        "korra_cli.config.save_config", lambda cfg: None, raising=False
    )
    return captured
...
def test_prompt_enable_tool_gateway_persists_decline(monkeypatch):
    ...
    captured = _capture_checklist(monkeypatch, selected_idx=[])
    monkeypatch.setattr(
        "korra_cli.config.save_config", lambda cfg: saved.append(dict(cfg)), raising=False
    )
...
def _block_legacy_agent_browser_checks(monkeypatch):
    ...
    monkeypatch.setattr("korra_constants.agent_browser_runnable", lambda path: False)
...
def test_has_agent_browser_import_failure_falls_back_to_path_check(monkeypatch):
    ...
    monkeypatch.setattr(
        "korra_constants.agent_browser_runnable",
...
def test_has_agent_browser_import_failure_falls_back_to_hermes_managed_node_path(
    ...
    monkeypatch.setattr(
        "korra_constants.with_hermes_node_path", lambda: {"PATH": str(managed_dir)}
    )
    monkeypatch.setattr(
        "korra_constants.agent_browser_runnable",
