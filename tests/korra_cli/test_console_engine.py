...
from korra_cli.console_engine import HermesConsoleEngine, run_console_repl
...
def test_sessions_export_rejects_oversized_single_before_touching_output(
    ...
    monkeypatch.setattr(korra_state, "resolved_max_export_messages", lambda: 2)
    materialized = []
...
def test_capture_output_surfaces_string_exit_code_as_command_error():
    from korra_cli.console_engine import ConsoleCommandError, _capture_output
...
def test_config_set_on_unparseable_yaml_reports_error_not_crash(tmp_path, monkeypatch):
    ...
    assert result.status == "error"
    assert 'некорректный YAML' in (result.output or "") or "Не удалось прочитать" in (result.output or "")
