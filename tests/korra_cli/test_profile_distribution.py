"""Tests for korra_cli.profile_distribution — git-based profile installs.
...
# ---------------------------------------------------------------------------
# Isolated profile env (matches tests/korra_cli/test_profiles.py)
# ---------------------------------------------------------------------------
...
class TestVersionRequires:
    ...
    def test_check_matrix(self, spec, cur, ok):
        if ok:
            ...
            with pytest.raises(DistributionError, match="requires Korra"):
                check_hermes_requires(spec, cur)
...
class TestInstall:
    ...
    def test_update_respects_distribution_owned_allowlist(self, profile_env):
        ...
        from korra_cli.profile_distribution import read_manifest as _read
    ...
    def test_install_enforces_hermes_requires(self, profile_env, monkeypatch):
        # Pin current Korra version to something well below the requirement
        import korra_cli
        monkeypatch.setattr(korra_cli, "__version__", "0.1.0", raising=False)
...
class TestInstalledAtStamp:
    ...
    def test_update_refreshes_installed_at(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr(
            "korra_cli.profile_distribution.datetime", _FakeDT, raising=True
        )

        from korra_cli.profile_distribution import update_distribution
...
class TestProfileInfoDistribution:
    ...
    def test_malformed_manifest_does_not_break_list(self, profile_env):
        from korra_cli.profiles import create_profile, list_profiles, get_profile_dir
