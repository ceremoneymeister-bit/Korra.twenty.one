...
korra_cli.config.load_config, so the managed overlay has to be wired into each.
...
def homes(tmp_path, monkeypatch):
    ...
    import korra_cli.config as cfg
...
def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # korra_time checks an env override first; ensure it's unset so config wins.
    monkeypatch.delenv("HERMES_TIMEZONE", raising=False)
    monkeypatch.delenv("TZ", raising=False)
    _seed(home, managed, user="timezone: America/New_York\n", mgd="timezone: Asia/Tokyo\n")
    import korra_time

    assert korra_time._resolve_timezone_name() == "Asia/Tokyo"
