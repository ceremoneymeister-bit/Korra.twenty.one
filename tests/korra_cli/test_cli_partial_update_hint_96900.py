...
``korra_cli.config`` while the older ``config.py`` does not export them.
...
from korra_constants import emit_partial_update_hint, partial_update_hint
...
def _missing_config_name_error(name: str = "resolve_turn_limit") -> ImportError:
    exc = ImportError(
        f"cannot import name '{name}' from 'korra_cli.config'"
    )
    exc.name = "korra_cli.config"
    return exc
