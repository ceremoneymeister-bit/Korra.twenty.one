"""Tests for korra_cli/webhook.py — webhook subscription CLI."""
...
def _isolate(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "korra_cli.webhook._is_webhook_enabled", lambda: True
    )
...
def test_webhook_base_url_maps_wildcard_hosts_to_localhost(monkeypatch, host):
    monkeypatch.setattr(
        "korra_cli.webhook._get_webhook_config",
...
class TestWebhookEnabledGate:
    ...
    def test_real_check_disabled(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "korra_cli.webhook._is_webhook_enabled",
            lambda: bool({}.get("enabled")),
        )
        import korra_cli.webhook as wh_mod
