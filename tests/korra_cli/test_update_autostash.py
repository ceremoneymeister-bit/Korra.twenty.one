...
from korra_cli import config as hermes_config
...
def test_refresh_active_memory_provider_dependencies_reinstalls_active_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        "korra_cli.memory_setup._install_dependencies",
...
def test_reload_updated_runtime_modules_restores_new_hermes_constants_symbol(monkeypatch):
    ...
    monkeypatch.delattr(korra_constants, "apply_subprocess_home_env", raising=False)
    assert not hasattr(korra_constants, "apply_subprocess_home_env")

    hermes_main._reload_updated_runtime_modules()

    assert callable(korra_constants.apply_subprocess_home_env)
...
def _setup_keep_stash_test(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "korra_cli.gateway.find_gateway_pids", lambda **kw: [], raising=False
    )
...
def test_bootstrap_marker_not_autostashed_by_update(tmp_path):
    ...
    # Exact flags used by hermes update (korra_cli/main.py).
