...
checkout where korra_cli/gateway.py newly imports `line_input` from
korra_cli.cli_output, but the updater process had cli_output cached from
before that symbol existed. The function-level `from korra_cli.gateway
...
def test_purge_protects_executing_modules():
    ...
    cli_main._purge_stale_hermes_modules()
    assert sys.modules.get("korra_cli.update_cmd") is update_cmd
    assert sys.modules.get("korra_cli.main") is cli_main
    assert "korra_cli" in sys.modules
...
def test_purge_never_raises_on_weird_sys_modules():
    ...
    sys.modules["korra_cli._purge_test_none"] = None  # type: ignore[assignment]
    try:
        cli_main._purge_stale_hermes_modules()
    finally:
        sys.modules.pop("korra_cli._purge_test_none", None)
...
def test_stale_symbol_scenario_end_to_end():
    ...
    try:
        ...
        try:
            from korra_cli.cli_output import line_input  # noqa: F401
            raised = False
        except ImportError:
            raised = True
