...
class TestSubcommandCompletion:
    ...
    def test_tools_enable_skips_already_listed(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "korra_cli.tools_config._get_platform_tools",
            lambda *_a, **_k: set(),
        )
        monkeypatch.setattr("korra_cli.config.load_config", lambda: {})
        monkeypatch.setattr(
            "korra_cli.tools_config._get_plugin_toolset_keys",
            lambda: set(),
        )
...
class TestTelegramMenuCommands:
    ...
    def test_configured_priority_promotes_skill_into_last_menu_slot(
        ...
        with (
            patch("korra_cli.commands.telegram_bot_commands", return_value=fake_core),
            patch("agent.skill_commands.get_skill_commands", return_value=fake_cmds),
            patch("korra_cli.plugins.get_plugin_commands", return_value=fake_plugins),
            patch("tools.skills_tool.SKILLS_DIR", local_dir),
            patch("korra_cli.commands._telegram_command_menu_config", return_value=menu_cfg),
        ):
    ...
    def test_scalar_configured_priority_is_accepted_as_one_command(self):
        ...
        from unittest.mock import patch
        from korra_cli.commands import _telegram_effective_priority
        ...
        with patch("korra_cli.config.read_raw_config", return_value=raw_config):
...
from korra_cli.commands import discord_skill_commands_by_category  # noqa: E402
...
class TestPluginCommandEnumeration:
    ...
    def _patch_plugin_commands(self, monkeypatch, commands):
        """Monkeypatch korra_cli.plugins.get_plugin_commands() to a fixed dict."""
        from korra_cli import plugins as _plugins_mod
