...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("korra_cli.setup.ensure_hermes_home", {}),
        ("korra_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("korra_cli.config.is_managed", {"return_value": False}),
        ("korra_cli.setup.load_config", {"return_value": {}}),
        ("korra_cli.setup.save_config", {}),
        ("korra_cli.setup.get_env_value", {"return_value": None}),
        ("korra_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("korra_cli.setup._print_setup_summary", {}),
        ("korra_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("korra_cli.setup.load_config", {"return_value": {}}),
        ("korra_cli.setup.save_config", {}),
        ("korra_cli.auth.get_active_provider", {"return_value": None}),
        ("korra_cli.setup.get_env_value", {"return_value": None}),
        ("korra_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="korra_cli.setup.prompt_choice",
                quick="korra_cli.setup._run_quick_setup",
                model="korra_cli.setup.setup_model_provider",
                terminal="korra_cli.setup.setup_terminal_backend",
                agent="korra_cli.setup.setup_agent_settings",
                gateway="korra_cli.setup.setup_gateway",
                tools="korra_cli.setup.setup_tools",
            )
...
class TestQuickFlag:
    ...
    def test_quick_flag_runs_quick_setup_only(self, existing_install):
        ...
        with ExitStack() as stack:
            ...
            from korra_cli.setup import run_setup_wizard
            from korra_cli import setup as setup_mod
...
class TestFreshInstall:
    ...
    def test_reconfigure_on_fresh_install_falls_through(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("korra_cli.setup.prompt_choice", {"return_value": 0}),
                first="korra_cli.setup._run_first_time_quick_setup",
            )
            from korra_cli.setup import run_setup_wizard
            from korra_cli import setup as setup_mod
    ...
    def test_blank_slate_runs_inside_navigation_step(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("korra_cli.setup.prompt_choice", {"return_value": 2}),
                blank="korra_cli.setup._run_blank_slate_setup",
            )
            from korra_cli import setup as setup_mod
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        import sys
        from korra_cli.main import main

        captured = {}
        monkeypatch.setattr(
            "korra_cli.setup.run_setup_wizard",
