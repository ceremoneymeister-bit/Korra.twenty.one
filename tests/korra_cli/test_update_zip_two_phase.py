...
from korra_constants import venv_bin_dir, venv_python_path
...
def test_top_level_files_are_swapped_atomically(tmp_path):
    ...
    korra_constants.py, ...). Covering only directories would leave exactly
...
def test_venv_helpers_honour_an_explicit_platform_verdict():
    ...
    `korra_cli.main._is_windows`. A helper that reads `sys.platform`
