...
def _summary_output(capsys, provider_ready: bool):
    ...
    with patch("korra_cli.auth.resolve_provider", resolver), \
...
def test_summary_warns_when_no_provider(capsys):
    ...
    assert "Сервис модели не настроен" in out
    assert "korra model" in out
    assert "korra setup --portal" in out


def test_summary_quiet_when_provider_ready(capsys):
    out = _summary_output(capsys, provider_ready=True)
    assert "Сервис модели не настроен" not in out
