"""Behavior tests for korra_cli.inventory.
...
def test_anthropic_oauth_presence_accepts_pool_only_oauth_entry():
    ...
    """
    from korra_cli.inventory import _anthropic_oauth_credentials_present
    ...
    with (
        ...
        patch(
            "korra_cli.auth.read_credential_pool",
            return_value=[
...
def test_build_models_payload_forwards_refresh_flag():
    ...
    with patch("korra_cli.model_switch.list_authenticated_providers", side_effect=_capture):
...
def test_list_authenticated_providers_refresh_busts_cache():
    ...
    with patch("korra_cli.models.clear_provider_models_cache") as clear:
...
def _apply_featured_with_dates(rows, dates: dict[str, str]):
    ...
    from korra_cli import inventory
