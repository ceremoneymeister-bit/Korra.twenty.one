...
import korra_cli.gateway as gateway_cli
...
class TestGeneratedSystemdUnits:
    ...
    def test_launchd_plist_persists_configured_nofile_soft_limit(self, monkeypatch):
        ...
        import korra_cli.resource_limits as resource_limits
...
class TestSystemUnitHermesHome:
    ...
    def test_node_path_lookup_remains_fallback_without_managed_node(
        ...
        monkeypatch.setattr(
            "korra_constants.iter_hermes_node_dirs", lambda root=None: []
        )
        monkeypatch.setattr(
            "korra_constants.hermes_managed_node_tree_present",
...
class TestLegacyHermesUnitDetection:
    ...
    _OUR_UNIT_TEXT = (
        ...
        "ExecStart=/usr/bin/python -m korra_cli.main gateway run --replace\n"
    )
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          ...
          - python path/to/korra_cli/main.py gateway run
          - hermes gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m korra_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/hermes/korra_cli/main.py gateway run",
...
class TestMigrateLegacyCommand:
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "korra_cli.main", "gateway", "--help"],
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "korra_cli.main", "gateway", "status", "-l", "--help"],
