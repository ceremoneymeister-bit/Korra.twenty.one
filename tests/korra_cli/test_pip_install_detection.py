...
def test_code_scoped_stamp_wins_over_home_stamp(tmp_path):
    ...
    with patch("korra_cli.config.get_managed_system", return_value=None), \
         patch("korra_cli.config.get_hermes_home", return_value=home):
...
def test_stamp_install_method_writes_code_scoped(tmp_path):
    ...
    code.mkdir()
    home.mkdir()
    with patch("korra_cli.config.get_hermes_home", return_value=home):
        from korra_cli.config import stamp_install_method
...
def test_container_without_stamp_is_not_docker(tmp_path):
    ...
    (tmp_path / ".git").mkdir()
    with patch("korra_cli.config.get_managed_system", return_value=None), \
         patch("korra_cli.config.get_hermes_home", return_value=tmp_path), \
         patch("korra_constants.is_container", return_value=True):
        from korra_cli.config import detect_install_method
