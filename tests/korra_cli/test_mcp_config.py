"""
Tests for korra_cli.mcp_config — ``hermes mcp`` subcommands.
...
def _isolate_config(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "korra_cli.config.get_hermes_home", lambda: tmp_path
    )
    config_path = tmp_path / "config.yaml"
    env_path = tmp_path / ".env"
    monkeypatch.setattr(
        "korra_cli.config.get_config_path", lambda: config_path
    )
    monkeypatch.setattr(
        "korra_cli.config.get_env_path", lambda: env_path
    )
    return tmp_path
...
class TestMcpList:
    def test_list_empty_config(self, tmp_path, capsys):
        from korra_cli.mcp_config import cmd_mcp_list
...
class TestMcpRemove:
    ...
    def test_remove_cleans_oauth_tokens(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr(
            "korra_cli.mcp_config.get_hermes_home", lambda: tmp_path
        )
...
class TestMcpAdd:
    ...
    def test_add_http_server_all_tools(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr(
            "korra_cli.mcp_config._probe_single_server", mock_probe
        )
    ...
    def test_add_preset_fills_transport(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr(
            "korra_cli.mcp_config._MCP_PRESETS",
...
class TestMcpTest:
    ...
    def test_probe_uses_configured_connect_timeout(self, monkeypatch):
        ...
        import asyncio
        from korra_cli import mcp_config
...
class TestProbeEnvResolution:
    ...
    def test_resolve_interpolates_header(self, monkeypatch):
        from korra_cli.mcp_config import _resolve_mcp_server_config
    ...
    def test_probe_resolves_before_connect(self, monkeypatch):
        ...
        import korra_cli.mcp_config as mc
...
class TestStripBearerPrefix:
    ...
    def test_bare_token_unchanged(self):
        from korra_cli.mcp_config import _strip_bearer_prefix
...
class TestConfigHelpers:
    def test_save_and_load_mcp_server(self, tmp_path):
        from korra_cli.mcp_config import _save_mcp_server, _get_mcp_servers
    ...
    def test_env_key_for_server(self):
        from korra_cli.mcp_config import _env_key_for_server
...
class TestMcpReauth:
    def test_reauth_all_visits_only_oauth_servers_in_order(
        ...
        visited = []
        monkeypatch.setattr(
            "korra_cli.mcp_config._reauth_oauth_server",
            lambda name, cfg: visited.append(name) or True,
        )
        from korra_cli.mcp_config import cmd_mcp_reauth
