...
hand-rolled parsers — ``korra_cli.config.load_env``,
``korra_cli.main._has_any_provider_configured``, and
...
def test_config_load_env_strips_export_prefix(tmp_path):
    from korra_cli.config import invalidate_env_cache, load_env
    ...
    try:
        with patch("korra_cli.config.get_env_path", return_value=env_path):
            env = load_env()
    finally:
        invalidate_env_cache()
...
def test_has_any_provider_configured_with_export_prefix(tmp_path, monkeypatch):
    ...
    import korra_cli.main as hmain
