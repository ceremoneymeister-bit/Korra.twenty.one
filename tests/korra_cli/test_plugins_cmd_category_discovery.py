...
class TestPluginStatus:
    ...
    def test_key_in_disabled(self):
        from korra_cli.plugins_cmd import _plugin_status
...
class TestFilterPluginEntries:
    def test_enabled_filter_uses_key(self):
        from korra_cli.plugins_cmd import _filter_plugin_entries
...
class TestCmdListJson:
    ...
    def test_json_status_uses_key(self, mock_user_dir, mock_bundled_dir, tmp_path, capsys):
        ...
        with patch("korra_cli.plugins_cmd._get_enabled_set", return_value={"web/keenable"}):
            args = MagicMock()
            args.json = True
            args.plain = False
            args.no_bundled = False
