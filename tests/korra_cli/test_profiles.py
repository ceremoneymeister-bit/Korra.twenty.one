"""Comprehensive tests for korra_cli.profiles module.
...
class TestDeleteProfile:
    ...
    def test_backend_scan_only_matches_this_profile(self, profile_env, monkeypatch):
        ...
        procs = [
            ...
            FakeProc(101, ["python", "-m", "korra_cli.main", "--profile", "coder", "serve"]),
            # Interactive chat for coder → NOT a backend subcommand, skipped.
            FakeProc(102, ["python", "-m", "korra_cli.main", "--profile", "coder", "chat"]),
            # Backend for a different profile → skipped.
            FakeProc(103, ["python", "-m", "korra_cli.main", "--profile", "other", "serve"]),
            # This very process → skipped even if it matched.
            FakeProc(self_pid, ["python", "-m", "korra_cli.main", "--profile", "coder", "serve"]),
        ]
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        monkeypatch.setattr("korra_cli.profiles.shutil.which", lambda name: "/opt/hermes/bin/hermes")
        from korra_cli.profiles import create_wrapper_script
    ...
    def test_remove_finds_bat_on_windows(self, profile_env):
        from korra_cli.profiles import create_wrapper_script, remove_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
...
class TestFindAliasForProfile:
    ...
    def test_profile_named_alias(self, profile_env):
        from korra_cli.profiles import create_wrapper_script, find_alias_for_profile
    ...
    def test_ignores_unrelated_files(self, profile_env):
        ...
        from korra_cli.profiles import _get_wrapper_dir, find_alias_for_profile
...
class TestOfflineProviderKeySlots:
    ...
    def test_overlay_only_definition_keeps_canonical_key_slot(self, monkeypatch):
        from korra_cli import providers as P
