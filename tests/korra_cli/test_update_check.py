"""Tests for the update check mechanism in korra_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    with patch("korra_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_does_not_cache_none(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "korra_cli.config.detect_install_method", lambda root: "git"
    )
    monkeypatch.setattr(
        "korra_cli.config.get_project_root", lambda: repo_dir
    )
    ...
    def fake_resolve(self, *args, **kwargs):
        s = str(self)
        if "banner.py" in s or s.endswith("korra_cli"):
