...
class _LivenessPatches:
    ...
    def __enter__(self):
        ...
        self._stack.enter_context(
            patch(
                "korra_cli.cron._active_cron_provider_name",
        ...
        self._stack.enter_context(
            patch(
                "korra_cli.gateway.named_profile_served_by_running_multiplexer",
                return_value=False,
            )
        )
        self._stack.enter_context(
...
class TestRuntimeLockFirstLiveness:
    ...
    def test_lock_inactive_falls_back_to_pid_scan(self):
        ...
        with (
            ...
            patch("korra_cli.gateway.find_gateway_pids", return_value=[424242]),
        ):
...
class TestCronStatusLockFirst:
    ...
    def _run_status(self, *, pids, lock_active, lock_pid=None):
        ...
        with (
            patch("korra_cli.cron._active_cron_provider_name", return_value="builtin"),
            patch("korra_cli.gateway.find_gateway_pids", return_value=list(pids)),
            patch(
                "gateway.status.is_gateway_runtime_lock_active",
