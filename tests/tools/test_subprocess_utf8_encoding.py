...
``cron/``, plus 5 more in ``tools/`` and ``korra_cli/`` (main.py,
setup.py, tts_tool.py, transcription_tools.py). The two call sites it
misses — ``korra_cli/onepassword_secrets_cli.py::_op_whoami`` and
...
def test_op_whoami_passes_utf8_encoding(tmp_path):
    ...
    from korra_cli import onepassword_secrets_cli as op_cli
