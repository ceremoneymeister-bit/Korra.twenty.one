...
at startup, by THREE separate code paths:
  ...
  2. gateway/run.py    -> ``_terminal_env_map`` dict (gateway / messaging
                          platforms)
  3. korra_cli/config.py:set_config_value
...
mirrors the pattern used in tests/korra_cli/test_config_drift.py.
"""
...
def _save_config_env_sync_keys() -> set[str]:
    ...
    """
    from korra_cli import config as hc_config
...
def test_save_config_set_supports_critical_bridged_keys():
    ...
    assert not missing, (
        ...
        f"in korra_cli/config.py (set_config_value bridges through it)."
    )
