...
def _restore_tool_and_agent_modules():
    ...
    original_modules = {
        ...
        if name in {"tools", "agent", "korra_cli"}
        or name.startswith("tools.")
        or name.startswith("agent.")
        or name.startswith("korra_cli.")
    }
    try:
        yield
    finally:
        _reset_modules(("tools", "agent", "korra_cli"))
...
def _install_fake_tools_package(*, credential_mounts=None):
    ...
    korra_cli = types.ModuleType("korra_cli")
    korra_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["korra_cli"] = korra_cli
    sys.modules["korra_cli.config"] = types.SimpleNamespace(
