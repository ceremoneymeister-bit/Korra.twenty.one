...
def test_chat_gateways_redact_secret_in_provider_error(platform):
    ...
    assert "сервис модели" in sanitized.lower()
...
def test_telegram_status_sanitizes_raw_provider_security_errors():
    ...
    assert "сервис модели отклонил" in sanitized.lower()
...
def test_telegram_final_response_redacts_auth_secrets():
    ...
    assert "не удалось войти" in sanitized.lower()
    assert 'проверьте подключение и ключ доступа' in sanitized.lower()
