...
def test_first_database_open_retries_only_transient_journal_lock(
    ...
    original = korra_state.apply_wal_with_fallback
    attempts = 0
    ...
    monkeypatch.setattr(korra_state, "apply_wal_with_fallback", transient_lock)
...
def test_room_log_pages_are_bounded_by_serialized_event_bytes(tmp_path, monkeypatch):
    ...
    for index in range(4):
        _append(
            ...
            # Keep JSON timestamp widths equal so this measures page limits.
            now=20 + index,
        )
