...
class TestBuildSessionContextPrompt:
    ...
    def test_local_delivery_path_uses_display_hermes_home(self):
        ...
        with patch("korra_constants.display_hermes_home", return_value="~/.hermes/profiles/coder"):
            prompt = build_session_context_prompt(ctx)
...
class TestSessionStoreRewriteTranscript:
    ...
    def store(self, tmp_path, monkeypatch):
        import korra_state
        monkeypatch.setattr(korra_state, "DEFAULT_DB_PATH", tmp_path / "state.db")
        config = GatewayConfig()
...
class TestGatewaySessionDbRecovery:
    ...
    def test_transcript_reroute_migrates_remaining_backlog_to_child(self):
        import threading
        from types import SimpleNamespace
        from korra_state import CompressionSessionClosedError
