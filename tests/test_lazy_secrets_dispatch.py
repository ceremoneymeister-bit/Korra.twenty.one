...
def _run_hermes(args: list[str], timeout: int = 30) -> subprocess.CompletedProcess[str]:
    ...
    return subprocess.run(
        [sys.executable, "-m", "korra_cli.main"] + args,
        capture_output=True,
        text=True,
        cwd=str(repo_root),
        env=env,
...
# resolves the korra_cli.main module's lazily re-exported attribute —
# so the patch must land on korra_cli.main._cmd_update_check.
with patch('korra_cli.main._cmd_update_check', capture_update_check):
    try:
        m.main()
