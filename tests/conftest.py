...
# `korra_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
...
def _hermes_home_points_at_production(value: str) -> bool:
    ...
    imports (logging handlers, ``korra_state.DEFAULT_DB_PATH``) froze paths
...
# like a real CLI") — which used to disarm korra_state's live-DB guard in
...
# korra_state._running_under_pytest() as a test-context signal. A child
...
_HERMES_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``korra_cli.voice.speak_text`` — real synthesis,
    ...
    # korra_constants home-resolution helpers prefer them over monkeypatched
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    # korra_state's live-DB guard stays armed in them even when the test
    ...
    # 3b. korra_state computes ``DEFAULT_DB_PATH = get_hermes_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from korra_state import ...``) that
    ...
    hermes_state_mod = sys.modules.get("korra_state")
    ...
    try:
        import korra_cli.plugins as _plugins_mod
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from korra_cli import kanban_db as _kb_mod
    except Exception:
        return
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    Only patches when ``korra_cli.kanban_db`` is *already imported* — a
    ...
    """
    _kdb = sys.modules.get("korra_cli.kanban_db")
    if _kdb is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import korra_cli.kanban_db`
...
# ``korra_state._ensure_test_isolation`` (the single choke point every
...
# so subprocess children that import korra_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("korra_state")
...
def _reset_tui_gateway_server_state():
    ...
    # to its default.
    try:
        from korra_constants import get_hermes_home_override, set_hermes_home_override
...
# correlated with a ``tests/korra_cli/`` pytest run starting up.
#
...
def _wal_is_usable() -> bool:
    ...
    IMPORTANT: this must NOT import ``korra_state``. That module computes
    ...
    duplicated from ``korra_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      ``korra_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``korra_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``korra_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``korra_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
def _live_system_guard(request, monkeypatch):
    ...
    # Korra: старые `hermes_cli`-строки оставлены рядом с новыми намеренно.
    # Это распознавание ЖИВОГО процесса по его argv, а живым на машине
    # разработчика ещё долго остаётся шлюз со старого образа. Убрать их —
    # значит тихо снять защиту ровно с того процесса, ради которого она и
    # написана.
    _HERMES_TOKENS = (
        "hermes-gateway",
        "hermes.service",
        "korra_cli.main gateway",
        "korra_cli/main.py gateway",
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                ...
                # whose cmdline contains "python -m korra_cli.main".
                if (
                    "hermes" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        ...
        # equivalent `python -m korra_cli.main update`).  These commands
        ...
        if "update" in low and (
            ...
            or
            # python -m korra_cli.main update --gateway
            ("korra_cli" in low and "update" in low.split())
            or
...
def _audio_playback_guard(request, monkeypatch):
    ...
    try:
        import korra_cli.voice as _voice
    except Exception:
