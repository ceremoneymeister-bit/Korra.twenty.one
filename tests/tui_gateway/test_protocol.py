...
def server():
    ...
    # e.g. korra_cli.active_sessions would bind the mocked get_hermes_home
...
def test_live_session_payload_replays_pending_approval(server, monkeypatch):
    ...
    assert replayed == {
        **first,
        "description": "удаление папки со всем содержимым",
    }
...
def test_skin_live_switch_end_to_end(server, tmp_path, monkeypatch):
    ...
    import korra_cli.skin_engine as skin_engine
