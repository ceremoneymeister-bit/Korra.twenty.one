...
``acp_adapter/entry.py`` (via the shared ``korra_bootstrap.harden_import_path``
...
def test_sys_path_guard_runs_before_cli_import():
    ...
    ``korra_bootstrap.harden_import_path()`` call precedes ``import cli``."""
    ...
    assert harden_call_line is not None, (
        "slash_worker.py must call korra_bootstrap.harden_import_path()"
    )
...
def test_guard_delegates_to_shared_helper_not_inline():
    ...
    assert '{"", "."}' not in src and "{'', '.'}" not in src, (
        "slash_worker.py should delegate to korra_bootstrap.harden_import_path, "
        "not re-implement the guard inline"
    )
    assert "korra_bootstrap.harden_import_path()" in src, (
        "slash_worker.py must call the shared korra_bootstrap.harden_import_path guard"
    )
