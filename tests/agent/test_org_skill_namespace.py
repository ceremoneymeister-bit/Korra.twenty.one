...
class TestOrgPullIsWiredIn:
    ...
    def test_no_internal_jargon_in_user_facing_strings(self):
        ...
        targets = [
            root / "korra_cli" / "subcommands" / "sync.py",
            root / "korra_cli" / "subcommands" / "skills.py",
        ]
        banned = re.compile(
...
class TestSkillSyncIsOneCommand:
    ...
    def test_sync_usage_lists_propose(self):
        main_src = self._src("korra_cli", "main.py")
        usage_start = main_src.index("Использование: korra sync ")
...
class TestLocalEditsSurviveOrgUpdates:
    ...
    def test_auto_propose_defaults_off(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "korra_cli.config.load_config", lambda: {}, raising=False
        )
