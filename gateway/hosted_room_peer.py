...
def gateway_room_grant_secret(root: Path | str | None = None) -> bytes:
    ...
    if root is None:
        ...
        root = korra_env("KORRA_HOME") or get_hermes_home()
...
def catalog_mapping(
    *,
    ...
    persistent_process = bool(persistent_process and korra_env("KORRA_DESKTOP") != "1")
    checked_policy = RoomExecutionPolicy.from_mapping(
        execution_policy
        or execution_policy_mapping(
            target_profile=(
                str(target_profile or "").strip()
                or (korra_env("KORRA_PROFILE") or "default").strip()
                or "default"
            )
        )
    )
