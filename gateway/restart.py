...
EXTERNAL_GATEWAY_SUPERVISOR_ENV = "KORRA_GATEWAY_EXTERNAL_SUPERVISOR"
...
def is_gateway_supervisor_process(
    ...
    if env.get("INVOCATION_ID"):
        return True
    if korra_env("KORRA_S6_SUPERVISED_CHILD", env=env):
        return True
    xpc_service = env.get("XPC_SERVICE_NAME", "")
    if xpc_service and xpc_service != "0":
        return True
    return str(korra_env(EXTERNAL_GATEWAY_SUPERVISOR_ENV, "", env=env)).strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
