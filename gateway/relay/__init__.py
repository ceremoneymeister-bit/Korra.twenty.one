...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        try:
            from korra_cli.skin_engine import get_active_skin  # late import: boot-safe
        ...
        # Korra: дефолт скина теперь "Korra"; старое "Hermes Agent" — из
        # пользовательских skin.yaml прежнего поколения, подавляем оба.
        if value in ("Hermes Agent", "Korra"):
            value = ""
...
def _resolve_relay_identity_token() -> str:
    ...
    if not token_url:
        ...
        from korra_cli.auth import resolve_nous_access_token
